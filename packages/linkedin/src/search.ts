...
/**
 * Every JSON island a Voyager SSR document ships: `<code id="bpr-guid-…">`, `__NEXT_DATA__` and
 * `<script type="application/json">`.
 *
 * Exported because the profile read needs exactly the same islands out of a DIFFERENT page
 * (`/in/<publicId>`), and a second copy of this extraction is a second thing to fix when LinkedIn
 * changes how it embeds its bootstrap.
 */
export function embeddedJsonBlobs(html: string): unknown[] {
  if (!html) return [];
  return [
