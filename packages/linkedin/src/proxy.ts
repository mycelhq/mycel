...
import { cookieFromInit, mergeSetCookie, setCookiesFrom, withCookie } from "./cookies";
...
/**
 * THE PRODUCTION TRANSPORT, when one is registered.
 *
 * Distinct from `fetchOverride` on purpose. That one is named "test seam" and is checked first so a
 * test still wins; conflating them would mean a mocked transport and a shipped one share a variable,
 * and the first confusing failure would be a test that passed because production was plugged in.
 *
 * What registers here is the browser transport (./browser-transport.ts). The reason it exists is
 * measured: LinkedIn answers 403 to this account through the proxy from an undici client and 200 to
 * the same cookies in a real browser. The difference is not the cookies — it is the TLS and HTTP/2
 * fingerprint, which no header can forge. A request issued by an actual Chromium is the only client
 * that reliably looks like one.
 *
 * Null means "use undici", which is what every install without a browser does.
 */
let productionTransport: FetchLike | null = null;

export function setLinkedInTransport(f: FetchLike | null): void {
  productionTransport = f;
}

export function hasLinkedInTransport(): boolean {
  return productionTransport !== null;
}
...
function hopKey(url: string, init: Record<string, unknown>): string {
  ...
  // Cookie value is part of identity: a 302 to the same URL with a fresh `lidc` is a retry, not a loop.
  return `${method} ${normalizeHop(url)} ${cookieFromInit(init)}`;
}
...
async function followLinkedInRedirects(
  ...
  const onJar = typeof init.onJar === "function" ? (init.onJar as (cookies: string) => void) : undefined;
  const initRest = { ...init };
  delete initRest.onJar;
  let reqInit: Record<string, unknown> = { ...initRest, redirect: "manual" as const };
  ...
  for (let hop = 0; hop <= MAX_LINKEDIN_REDIRECTS; hop++) {
    ...
    // Apply Set-Cookie BEFORE deciding whether to follow. A 302 to the same Voyager URL is
    // LinkedIn re-routing via a fresh `lidc`; following (or returning) without merging it is how
    // the next hop — and every subsequent action — keeps sending the stale hint.
    const rotated = setCookiesFrom(res.headers);
    if (rotated.length > 0) {
      const before = cookieFromInit(reqInit);
      const after = mergeSetCookie(before, rotated);
      if (after !== before) {
        reqInit = withCookie(reqInit, after);
        onJar?.(after);
      }
    }
...
export async function proxiedFetch(
  ...
  const send = (target: string, reqInit: Record<string, unknown>): Promise<Response> => {
    ...
    // The browser transport carries the proxy itself — Chromium is launched with it — so it is
    // consulted after the proxy requirement above has already been enforced.
    if (productionTransport) return productionTransport(target, reqInit);
    ...
    return dispatcherFor(resolved).then(
      // `as unknown as` rather than a direct cast. undici ships its OWN Response type, and when
      // packages/linkedin/node_modules/undici is present — which it is inside the image, and is not
      // on a hoisted laptop install — TypeScript sees two unrelated Responses and refuses the
      // conversion. It compiled locally and broke three deploys in a row for exactly that reason.
      (dispatcher) => undiciFetch(target, { ...reqInit, dispatcher } as any) as unknown as Promise<Response>,
    );
  };
