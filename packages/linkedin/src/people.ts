...
export interface LiPerson {
  /** LinkedIn public identifier ("jane-doe"). THE natural key for the `people` collection. */
  public_id?: string;
...
export function vectorImageUrl(v: unknown): string | undefined {
  // The wrapper key is DOTTED in LinkedIn's own JSON — "com.linkedin.common.VectorImage" — and this
  // read the underscored spelling, which exists nowhere. Names parsed fine off the same nodes, so
  // nine real prospects came back from a real account with photo_url: NONE on every one (prod,
  // 2026-08-16) and nothing errored: the unwrap fell through `?? v`, found no rootUrl, and answered
  // undefined. Both spellings are tried because our own tests and any snake_cased proxy layer may
  // legitimately produce the underscored one.
  const rec = asRecord(v);
  const img = asRecord(rec["com.linkedin.common.VectorImage"] ?? rec.com_linkedin_common_VectorImage ?? v);
