 ...
 */
export const WARMUP_GATED = ["follow_person", "react_to_post", "endorse_skill"] as const;

/**
 * Declared in the catalogue, no executor, no sequencer. InMail / accept stay out of the composer
 * until they are wired with pacing caps. Do not plan around them.
 *
 * `endorse_skill` MOVED OUT OF HERE and into WARMUP_GATED: it now has an executor in engage.ts,
 * behind the same kill-switch and with the same INFERRED-endpoint caveat as react/follow. That is a
 * promotion from "does not exist" to "exists and is disarmed" — it is NOT a promotion into
 * SEQUENCE_LIVE, and it must not become one. An endorsement lands under the founder's real name on
 * a stranger's profile, permanently; it is an action an agent may choose deliberately, not a rung
 * on a cadence that fires while nobody is watching.
 */
export const DECLARED_UNWIRED = ["accept_invite", "send_inmail"] as const;
