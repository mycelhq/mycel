...
import { getLinkedInHost, linkedInWedge, type RecordStore } from "./host";
...
export function personRecord(p: LiPerson | LiProfile, field: "search" | "profile", at: string): Record<string, unknown> {
  ...
  return {
    ...defined({
      ...
      location: p.location,
      /**
       * A `public_id` IS reachability. `writePeople` has already refused anything without one, and a
       * LinkedIn public id is exactly how this person gets messaged — the sequencer needs no URL and
       * no address to send them a DM.
       *
       * The first cut passed only `profile_url`, and search rows do not always carry one: the
       * screen then dropped `{ public_id: "dana-okafor", name: "Dana Okafor", title: "VP
       * Engineering", company: "Acme" }` as "no way to contact them". That is a perfectly good lead
       * and the exact failure a hygiene filter must not have — quietly eating real prospects is
       * worse than the junk it was added to remove.
       */
      linkedin_url: p.profile_url ?? `https://www.linkedin.com/in/${p.public_id}`,
      urn: p.urn,
...
export async function writePeople(
  ...
  for (const p of people) {
    ...
    /**
     * ═══ THE GARBAGE COLLECTOR, AT THE DOOR ═══
     *
     * A discovery sweep files whatever it finds, and what it finds includes rows that are not
     * people. A founder's pipeline showed, between real prospects: "Agora Software",
     * "Altena-Software", "International Software Company", "Dlubal Software FR" — the company's own
     * name in the person's name field, no face, no headline, nothing to send to.
     *
     * They are not harmless. Each is a card to read and dismiss, a slot in the "26 on file" count
     * that implies work nobody did, and — if a campaign enrols one — a message addressed by name to
     * a limited company.
     *
     * HERE, because this is the one door every path writes people through: keyword search, the
     * company People tab, amplification. Screening at the callers would mean screening at three
     * places and forgetting the fourth.
     *
     * `profile_url` is the reachability signal for a LinkedIn find — the profile IS how they are
     * contacted. See `judgeLead` for what else counts and, more importantly, for what is protected:
     * a real headline overrides the name-shape test, because "Elena Debbaut · Turnaround • Strategic
     * Execution" is a person whose name simply looks unusual.
     */
    const judge = getLinkedInHost().judgeLead;
    const verdict = judge?.({
      name: p.name,
      company: p.company,
      company_domain: p.company_domain,
    ...
    if (verdict && !verdict.keep) {
      // Logged with its reason, never silent: a filter nobody can audit is one nobody will trust
      // with their pipeline.
      console.info(`[mycel] not a lead, not filed — ${p.public_id}: ${verdict.reason}`);
      continue;
    }
    try {
