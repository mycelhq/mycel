...
//
// ═══ DEFAULT IS BRIGHT DATA. DECODO CANNOT SERVE SELF-SERVE, AT ANY PRICE ═══
//
// Decodo sells dedicated ISP by the COUNTRY, from a dashboard, with no purchase API. So the set of
// countries we can egress from is a list somebody bought in advance and typed into
// MYCEL_DECODO_COUNTRIES, and `allocateDecodo` throws `noResidentialLine(cc)` for anything else.
//
// That is fine for a handful of seats we operate. It cannot work for self-serve: a stranger signs
// up, connects their LinkedIn, and we do not know what country they are in until they do. Nobody
// pre-buys 195 countries from a dashboard. The choice there is a hard refusal or a foreign IP, and
// a foreign IP is the single thing LinkedIn scores hardest against.
//
// Bright Data takes the country as a URL parameter against a pool that already covers the world
// (US ~5.8M, GB ~323k, DE ~377k, FR ~225k), with no per-country purchase and geo-targeting free.
// Elastic by construction, which is what self-serve needs.
//
// Cost, checked 7 September 2026 — and the old note here had this backwards:
//   Bright Data ISP   from $1.30/IP/mo
//   Decodo ISP        $3.33/IP at 3 · $2.90 at 10 · $2.00 at 20
// The previous ladder recorded Decodo at ~$1.20–1.60 and called it "the cost default". Decodo's
// list price has roughly doubled at low volume since; Bright Data's figure was accurate. So Bright
// Data is now both cheaper per IP AND the only one that can allocate a country on demand. Check
// Bright Data's monthly minimum before committing — that is the one place Decodo could still win
// for a very small deployment, and it is the number this comment does not have.
//
//   `MYCEL_LINKEDIN_PROXY_PROVIDER=brightdata` + `MYCEL_BRIGHTDATA_CUSTOMER` / `ZONE` / `PASSWORD`.
//   Sticky per LinkedIn member via `-session-{stickyKey}`; the ledger keeps a member on one IP.
//
// DECODO IS RETAINED, NOT REMOVED, and only until the subscription is cancelled. The founder seat
// runs on a Decodo URL stored in `growth.secrets`, on an IP its session was born on. Moving a warm
// account to a new IP is the change LinkedIn punishes most, so it moves when the account is next
// re-authenticated anyway, not before. `MYCEL_LINKEDIN_PROXY_PROVIDER=decodo` still selects it.
//
// Alternate: **static pool** of dedicated ISP URLs from any vendor
// (`MYCEL_LINKEDIN_PROXY_POOL`, tag `us|http://…` / JSON `{url,country}`). Still wins over both.
//
// Ads stay out of this loop (later channel). Composio = agent toolkit broker only.
//
...
export interface ProxyPoolStatus {
  ...
  countries?: string[];
  /**
   * Can this provider egress from a country we have not bought in advance?
   *
   * The connect form asks the operator which country their LinkedIn account lives in, and then
   * offered a HARDCODED list of three — France, Spain, the UK — because those are the Decodo IPs
   * somebody bought. The menu was a purchase order pretending to be a question: a self-serve user
   * in Brazil either cannot connect, or picks a country they are not in and gets the foreign IP the
   * form promises never to use.
   *
   * Bright Data allocates by URL parameter against a worldwide pool, so for it the honest answer is
   * "any". Decodo and a static pool can only offer what is already paid for, and `countries` says
   * which. The UI reads this instead of carrying its own list, so buying a fourth Decodo country
   * shows up in the dropdown without anybody editing a component.
   */
  anyCountry: boolean;
}
...
/**
 * Which pool is active. Explicit PROVIDER wins; otherwise prefer static list → Bright Data → Decodo.
 *
 * Bright Data comes before Decodo now. It allocates any country on demand, which is what self-serve
 * requires, and as of 7 September 2026 it is also the cheaper of the two per IP. Decodo stays last
 * so an existing deployment that only has Decodo configured keeps working until it is cancelled.
 */
...
export function proxyPoolStatus(): ProxyPoolStatus {
  ...
  if (provider === "none") {
    return { provider: "none", enabled: false, free: null, total: null, anyCountry: false };
  }
  if (provider === "brightdata") {
    // Worldwide pool, allocated per connect. Nothing to buy per country, so nothing to list.
    return { provider, enabled: true, free: null, total: null, anyCountry: true };
  }
  if (provider === "decodo") {
    ...
    return {
      ...
      free_by_country,
      countries,
      // Only what was bought. `allocateDecodo` throws noResidentialLine(cc) for anything else.
      anyCountry: false,
    };
  }
  ...
  return {
    ...
    total: slots.length,
    // A fixed list of URLs somebody pasted; the countries are whatever those slots are tagged with.
    anyCountry: false,
    countries: [...new Set(slots.map((x) => x.country).filter(Boolean) as string[])].sort(),
    free_by_country,
  };
}
