...
# ── the console ─────────────────────────────────────────────────────────────
#
# ═══ AN INSTALL THAT ENDS AT A CURL COMMAND IS AN INSTALL NOBODY FINISHES ═══
#
# Everything below this line used to be three curl invocations. They were correct, they had been
# fixed twice after a stranger-install walkthrough found them wrong, and they are still the wrong
# last thing for somebody to read: a person who has just spent four minutes installing an "operating
# system for service businesses" and is handed an HTTP request has not been shown a product. They
# have been shown an API, and the honest conclusion to draw from an API is "come back when there is
# something to look at".
#
# The console is a SEPARATE REPOSITORY on purpose, and that is not an accident of packaging — it
# talks to the kernel over /v1 and nothing else, so anything it does, an app you write can do. See
# docs/OPEN-CORE.md. Which means it is a second clone, so it is offered rather than assumed, and the
# offer says how long it takes before somebody commits to it.
CONSOLE_REPO="${MYCEL_CONSOLE_REPO:-https://github.com/mycelhq/console.git}"
CONSOLE_DIR="$(cd "$HERE/.." && pwd)/mycel-console"
CONSOLE_READY=
if [ -d "$CONSOLE_DIR" ]; then
  ok "console already present at $CONSOLE_DIR"
  CONSOLE_READY=1
elif have git && { [ -n "$NONINTERACTIVE" ] || yesno "Set up the console too? (a browser UI for approving and running work — one clone plus a minute of npm install)" "y"; }; then
  say ""
  say "${B}Setting up the console${N}"
  if git clone --depth 1 "$CONSOLE_REPO" "$CONSOLE_DIR" >/dev/null 2>&1; then
    ( cd "$CONSOLE_DIR" && npm install >/dev/null 2>&1 ) && { ok "console ready"; CONSOLE_READY=1; } \
      || warn "console cloned but npm install had issues — run it manually in $CONSOLE_DIR"
  else
    # Never silent. A failed clone here used to be indistinguishable from not having asked, and the
    # person would spend the rest of the session assuming there was no UI.
    warn "could not clone the console from $CONSOLE_REPO — the kernel below works without it"
  fi
fi
...
# THE ORDER IS THE MESSAGE. Two commands and a URL first, because that is the whole of what somebody
# has to do; the API is underneath for the person who wants it, where it reads as depth rather than
# as the only door.
if [ -n "$CONSOLE_READY" ]; then
  say "  ${B}1.${N} Start the kernel:  ${C}cd $(basename "$HERE") && npm run dev${N}"
  say "  ${B}2.${N} Start the console: ${C}cd $(basename "$CONSOLE_DIR") && npm run dev${N} ${D}(another terminal)${N}"
  say "  ${B}3.${N} Open:              ${C}http://localhost:3000${N}"
  say "                        ${D}sign in with the owner login the kernel prints on boot${N}"
else
  say "  Start the kernel:   ${C}cd $(basename "$HERE") && npm run dev${N}"
  say "                      ${D}it prints an API key and an owner login on boot${N}"
