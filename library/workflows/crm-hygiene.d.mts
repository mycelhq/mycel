...
export interface CrmHygieneResult {
  ...
  detail?: string;
  /**
   * The likelier explanation, when the result is technically true and probably means something else.
   *
   * Set when EVERY record has no way to reach it and there are at least three — possible, and more
   * often a field-name mismatch. Declared here because a caller that cannot see the field in the
   * type is a caller that will not render it, which is how a workflow comes to compute an answer
   * nobody is shown. See the argument in `crm-hygiene.mjs`.
   */
  warning?: string;
}
