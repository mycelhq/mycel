// SHARED PRIMITIVE — the dunning ladder. Kernel-owned, verified, referenced by any wedge that bills
// and chases (invoice-chaser today; any generated service that declares a money model tomorrow).
//
// This looks like judgment but it is POLICY — given the facts, the step is fixed. Moving it out of
// the model makes it consistent, auditable and testable, and it stops the agent inventing an
// escalation the contract doesn't permit. A wedge references this by `lib: "dunning-ladder"`; it
// does not, and cannot, author it. Prose (skills/chase-politely.md) still decides HOW to write;
// this decides WHICH step.
//
// Founder-grade code. Pure: no I/O, no clock (today is passed in), no randomness.

export default function dunningLadder(args) {
