...
async function main(): Promise<void> {
  ...
  if (run.has("agency")) {
     ...
     *
     * ═══ TAKE THE TOP *TAKEABLE* MOVE, NOT THE TOP MOVE ═══
     *
     * This took `moves[0]` unconditionally, and it went red twice on two different legitimate
     * refusals. First `move.wedge_disabled` (the note at the top of this file), then
     * `chase_invoice` against a business with no mailbox connected: "the reminder could be written
     * but never sent".
     *
     * Both times the product was right. `takeable: false` is a DECISION — the ranking proposes a
     * chase because it is derived from the invoice rather than from what is configured, and refuses
     * to start one that structurally cannot reach a client. Hiding the row instead would mean a
     * founder with an overdue invoice and no mailbox sees nothing at all, which is the worse bug.
     *
     * A regression test that is knowingly red is a regression test nobody reads, and the next real
     * failure hides behind "oh, that one is expected". So this now asserts BOTH halves of the gate,
     * which is a stronger check than the one it replaces: the top takeable move goes through, and a
     * non-takeable one is refused WITH ITS REASON rather than with a bare 409.
     */
    const moves = await founder.get<{
      moves: { id: string; kind: string; entity_id?: string; takeable?: boolean; unavailable_reason?: string }[];
    }>("moves");
    const all = moves.moves ?? [];
    const move = all.find((m) => m.takeable !== false);
    if (move) {
      const taken = await founder.tryPost(`moves/take`, { move_id: move.id });
      expect(`the top takeable move ("${move.kind}") can be taken`, taken.ok, `${taken.status} ${taken.text.slice(0, 120)}`);
      ...
      skip("every proposed move needs something connected first — a real state for a new business");
    }

    /**
     * THE OTHER HALF. A move the ranking marked unavailable must be refused, and the refusal must
     * carry the sentence the row already showed the founder — not a bare status code.
     */
    const blocked = all.find((m) => m.takeable === false);
    if (blocked) {
      const refused = await founder.tryPost(`moves/take`, { move_id: blocked.id });
      expect(
        `an unavailable move ("${blocked.kind}") is refused rather than half-started`,
        !refused.ok,
        `it was accepted: ${refused.status}`,
      );
      /**
       * THE SAME SENTENCE, not merely A sentence. The row already told the founder "connect the
       * email account you invoice from and this will start on its own"; a refusal that says
       * something different is a second explanation of one fact, and the founder has to work out
       * which one is true.
       */
      const said = blocked.unavailable_reason ?? "";
      // The PARSED error, not the raw body: the reason contains a quoted mailbox name, so a
      // substring check against the JSON compares against backslash-escaped quotes and fails on a
      // product that is behaving correctly. Cost one red run to notice.
      let refusedSaid = refused.text;
      try {
        refusedSaid = String((JSON.parse(refused.text) as { error?: unknown }).error ?? refused.text);
      } catch {
        /* not JSON — compare the raw body, which is the honest fallback */
      }
      expect(
        "and the refusal repeats the row's own sentence rather than inventing a second one",
        said.length > 40 && refusedSaid.includes(said),
        `row said:      ${said.slice(0, 110)}\n         refusal said: ${refusedSaid.slice(0, 110)}`,
      );
    } else {
      skip("nothing was blocked on a missing connection this run");
    }
  }
  ...
  if (run.has("portal")) {
    ...
    if (waiting) {
      ...
      /**
       * ═══ THE LOOP THAT MAKES THIS A BUSINESS RATHER THAN A TOOL ═══
       *
       * Everything above proves work reaches a client and comes back signed off. None of it proves
       * anybody gets PAID, and this script has been creating invoices out of thin air — five of them,
       * at five ages, none connected to any work. So the chain that actually matters had never run:
       *
       *     accepted deliverable → the money-plan line it satisfies → a draft invoice → payment
       *
       * Every link exists and has unit tests. The chain does not, which is a different thing: unit
       * tests prove the pieces, and a business is the joins.
       */
      const kase = W.clients.long!.caseId;
      const plan = await founder.tryPut(`cases/${kase}/money-plan`, {
        currency: "USD",
        lines: [{ label: "Q3 sales tax return", kind: "milestone", amount_minor: usd(1200) }],
      });
      if (plan.ok) {
        agency("priced the engagement: one milestone, $1,200 on the return");
        const drafted = await founder.tryPost<{ invoice?: { id: string; number: string; status: string } }>(
          `deliverables/${waiting.id}/draft-invoice`,
          {},
        );
        expect("accepted work becomes a draft invoice from the engagement's own price", drafted.ok, `${drafted.status} ${drafted.text.slice(0, 160)}`);

        const raised = drafted.body?.invoice;
        if (raised) {
          agency(`raised ${raised.number} for the accepted return`);
          /**
           * A DRAFT, never issued automatically. The founder decides when a client is billed — this
           * is the one place in the loop where the machine stopping is the feature.
           */
          expect("and it is a draft, because billing a client is the founder's decision", raised.status === "draft", raised.status);

          await founder.post(`invoices/${raised.id}/status`, { to: "sent" });
          const paid = await founder.tryPost(`invoices/${raised.id}/payments`, { amount_minor: usd(1200) });
          expect("the invoice can be paid in full", paid.ok, `${paid.status} ${paid.text.slice(0, 120)}`);

          const settled = await founder.get<{ totals?: { amount_due?: number }; effective_status?: string; status?: string }>(
            `invoices/${raised.id}`,
          );
          client(longName, `paid ${raised.number} in full`);
          expect(
            "and it settles — nothing is left owing on work that has been paid for",
            (settled.totals?.amount_due ?? -1) === 0,
            `amount_due=${settled.totals?.amount_due} status=${settled.effective_status ?? settled.status}`,
          );

          /**
           * THE LINE IS SPENT. A money-plan line that stays `planned` after it has been invoiced is
           * how a client gets billed twice for one piece of work — the worst bug this loop can have,
           * because the founder finds out from the client.
           */
          /**
           * `data.money_plan`, not a top-level `money_plan`. The PUT answers with one at the top and
           * the GET returns the case as stored — so a check reading the shape the WRITE returns
           * silently sees an empty list and reports the healthiest possible state as a failure.
           * Cost one red run; worth the comment, because "billed twice" is the assertion here and an
           * assertion that cannot see its own data is worse than none.
           */
          const after = await founder.get<{ data?: { money_plan?: { lines?: { status?: string }[] } } }>(`cases/${kase}`);
          const statuses = (after.data?.money_plan?.lines ?? []).map((l) => l.status);
          expect(
            "the priced line is marked invoiced, so nobody is billed twice for it",
            statuses.length > 0 && !statuses.includes("planned"),
            `lines: ${JSON.stringify(statuses)}`,
          );
        }
      } else {
        skip(`this deployment cannot price an engagement (${plan.status}) — fulfilment may be off on this plan`);
      }
    } else {
  ...
  if (run.has("followup")) {
    ...
    if (W.deliverableIds.mid) {
      ...
      /**
       * ═══ THE STAGE THAT HAD NEVER RUN ANYWHERE ═══
       *
       * `simulation/lifecycle.ts` lists fifteen stages and its own note singles this one out: "a
       * business that can deliver but cannot revise has no retainer, because the second month is
       * entirely revisions." Against this script before this beat existed, `lifecycle-report` said
       * 8/15 — delivered ✓, revision_requested ✓, revised ✓, revision_accepted ✗. Forty checks
       * passed and the one transition a retainer is made of had never been executed, here or in
       * production, or in any test.
       *
       * That is not an oversight in a script. It is the difference between selling a deliverable
       * and selling a service, and it was invisible precisely because everything either side of it
       * worked: version 2 was created, version 2 was released, and then the simulation changed the
       * subject.
       *
       * ── WHY THE SECOND ACCEPTANCE IS A DIFFERENT TEST FROM THE FIRST ──
       *
       * `long` accepts a FIRST version, from `with_client`. This accepts a SECOND, from a
       * deliverable that has already been round the loop once — a different starting state, a
       * different transition in the table, and the one that has to be re-armed after a change
       * request rather than merely allowed. A client whose objection was answered and who then
       * cannot press accept is a client the product has trapped at exactly the moment it earned
       * their trust.
       */
      const midRow = await mid.portal!.get<{ deliverables: { id: string; can_act?: boolean; state?: string; version?: number }[] }>(
        "portal/deliverables",
      );
      const revised = (midRow.deliverables ?? []).find((d) => d.id === W.deliverableIds.mid);
      expect("the revision reaches the client's portal", !!revised, `${(midRow.deliverables ?? []).length} deliverables visible`);
      expect(
        "and the client may act on it again after their objection was answered",
        revised?.can_act === true,
        `state="${revised?.state}" can_act=${revised?.can_act}`,
      );

      const signedOff = await mid.portal!.tryPost<{ ok?: boolean }>(`portal/deliverables/${W.deliverableIds.mid}/accept`, {
        note: "That is the commission in the right month now. Happy with this one.",
      });
      expect(
        "A REVISION CAN BE ACCEPTED — the stage a retainer is made of",
        signedOff.ok,
        `${signedOff.status} ${signedOff.text.slice(0, 160)}`,
      );
      /*
        `CLIENTS[1]!.name` and not `midName`: that binding is a `const` inside beat 5's block, and
        this is beat 6. `tsc --noEmit` passed on the reference — which is worth remembering, because
        it means a typecheck is not a scope check here, and only running the thing found it.
      */
      client(CLIENTS[1]!.name, "accepted the revised October accounts");
    }
