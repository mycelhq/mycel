...
export class Session {
  ...
  /**
   * A multipart upload — the founder's own file, through the founder's own route.
   *
   * `FormData` is NOT serialised here and the content-type header is NOT set. Both are deliberate:
   * `fetch` sets `multipart/form-data; boundary=…` from the body itself, and a hand-written
   * content-type produces a boundary that does not match the payload, which the server reads as an
   * empty form. That failure looks like "a 'file' part is required" and sends the reader hunting in
   * entirely the wrong place.
   *
   * So this cannot go through `call()`, which sets `content-type: application/json` on everything.
   */
  async upload<T>(path: string, form: FormData): Promise<T> {
    ...
    const text = await res.text();
    if (!res.ok) die(`[${this.label}] POST /v1/${path} → ${res.status}\n    ${text.slice(0, 400)}`);
  ...
  /**
   * The soft PUT. Added for pricing an engagement, which a deployment may legitimately refuse — the
   * money-plan route is behind the fulfilment entitlement, so a free plan answers 402 and that is a
   * real product state rather than a broken script.
   */
  tryPut = <T,>(path: string, body: unknown) => this.attempt<T>(path, { method: "PUT", body: JSON.stringify(body) });
}
