...
test("distillation: an approve-with-edit becomes a scoped rule with the founder's own words as evidence", async () => {
  ...
  await store.createTask({
    ...
    actor: { kind: "system", id: "test" }, input: { client_id: "brightline" }, constraints: {}, tools: [],
  ...
  // Brightline is not a house rule, and applying it to everyone is how a "learning" system makes a
  // business worse.
  assert.equal(body!.client_id, "brightline");
...
test("retrieval: a rule reaches the prompt only for the tenant, client and task type it was learned on", async () => {
  ...
  const otherClient = ruleOf({ id: "other-client", client_id: "brightline" });
  ...
  // On Brightline's own work the exception applies — and outranks the house rule, because the
  // exception is the expertise.
  const forClient = retrieveRules(all, ctxFor({ client_id: "brightline" }));
...
test("retrieval: a rule distilled for one client is never inlined into another client's run", async () => {
  ...
  const forA = ruleOf({
    id: "for-a",
    ...({ sensitivity: "client", client_id: "brightline" } as const),
    text: "Brightline settle at 60 days and asked us not to chase before then.",
  });
  ...
  assert.ok(!atB.markdown.includes("Brightline"), "and its text is nowhere in what B's prompt would say");

  const atA = retrieveRules([forA, unattributed], ctxFor({ client_id: "brightline" }));
...
test("retrieval: an unlabelled legacy rule reaches nobody until something says whose lesson it was", async () => {
  ...
  assert.deepEqual(retrieveRules([legacy], ctxFor({ client_id: "brightline" })).selected.map((r) => r.id), []);
...
test("distillation: every write site labels the rule, so a new row is never the unlabelled shape", async () => {
  ...
  const forClient = distillFromApprovalEdit({
    project_id: "p1", wedge: "books-keeper", task_type: "chase", client_id: "brightline",
  ...
  for (const r of forClient) {
    ...
    assert.equal(r.client_id, "brightline");
  }
...
test("retrieval: knowledge files are ranked and capped, not mounted wholesale", async () => {
  ...
  const files = [
    ...
    {
      name: "this-client.md",
      content: "brightline wants friday summaries",
      created_at: "2025-12-20T00:00:00.000Z",
      kind: "correction",
      source: "feedback",
      metadata: { sensitivity: "client", client_id: "brightline", task_type: "chase" },
    },
    {
  ...
  const g = await groundRun(store, {
    ctx: ctxFor({ client_id: "brightline" }),
    files,
