...
//
//   · a fire runs find → propose, approved on the audience's standing autopilot consent —
//     the cases are queued and NOTHING has been sent by the time the fire returns;
...
test("a fire runs find → propose, and autopilot's standing consent approves it; nothing is sent yet", async () => {
  ...
  /**
   * AUTOPILOT DECIDES. The audience switch is the consent — it is what starts this loop and the only
   * thing that stops it — so asking again per campaign asked the same question twice and blocked on
   * the second one. Unattended loops produced campaigns that sat until they expired.
   *
   ...
   * The reason records that a standing authority decided it rather than a click.
   */
  ...
  assert.match(String(approval!.policy_reason ?? ""), /autopilot/i, "an autopilot decision must say so on the row");
