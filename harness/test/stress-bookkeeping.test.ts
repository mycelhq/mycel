...
test("stress test: a bookkeeping agency is expressible end to end", async () => {
  ...
  const wf = await (await app.request("/v1/internal/workflows/reconcile", { method: "POST", headers: AH, body: JSON.stringify({ bank_transactions: readRes.data.transactions, ledger_entries: [{ amount_cents: 12000, date: "2026-10-02", reference: "A1" }, { amount_cents: -3500, date: "2026-10-05", reference: "B2" }], sales_tax_rate_pct: 20 }) })).json();
  ...
  const upd = await (await app.request("/v1/internal/case/update", { method: "POST", headers: AH, body: JSON.stringify({ stage: "reconciling", data: { difference_cents: wf.data.difference_cents, unmatched: wf.data.unmatched_bank }, note: "off by 4500p — C3 unmatched" }) })).json();
  ...
  const filing = await Promise.race([
    (async () => (await (await app.request("/v1/internal/actions/file_sales_tax", { method: "POST", headers: AH, body: JSON.stringify({ connection_id: mailConn.id, amount: 2750 }) })).json()))(),
    new Promise((r) => setTimeout(() => r({ suspended: true }), 600)),
  ]) as any;
