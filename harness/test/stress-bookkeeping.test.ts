...
test("stress test: a bookkeeping agency is expressible end to end", async () => {
  ...
  const bank = [
    { amount_cents: 12000, date: "2026-10-02", reference: "A1", counterparty: "Shopify" },
    { amount_cents: -3500, date: "2026-10-05", reference: "B2", counterparty: "Royal Mail" },
    { amount_cents: 4500, date: "2026-10-09", reference: "C3", counterparty: "Etsy" },
  ];
  ...
  ok("schedule: quarterly sales tax deadline expressible", nextRun({ kind: "monthly", day: 7, hour: 9, minute: 0 }, new Date("2026-11-01")).toISOString().startsWith("2026-11-07"));
  ...
  const readRes = await jsonBody(await app.request("/v1/internal/reads/list_transactions", { method: "POST", headers: AH, body: JSON.stringify({ connection_id: bankConn.id, path: "transactions", query: { period: "2026-10" } }) }));
  ...
  const wf = await jsonBody(await app.request("/v1/internal/workflows/reconcile", { method: "POST", headers: AH, body: JSON.stringify({ bank_transactions: readRes.data.transactions, ledger_entries: [{ amount_cents: 12000, date: "2026-10-02", reference: "A1" }, { amount_cents: -3500, date: "2026-10-05", reference: "B2" }], sales_tax_rate_pct: 20 }) }));
  ok("workflow: reconciles in integer cents, exactly", wf.ok && wf.data.difference_cents === 4500 && wf.data.sales_tax_due_cents === 2750, `diff ${wf.data?.difference_cents}p, vat ${wf.data?.sales_tax_due_cents}p`);
  ...
  const upd = await jsonBody(await app.request("/v1/internal/case/update", { method: "POST", headers: AH, body: JSON.stringify({ stage: "reconciling", data: { difference_cents: wf.data.difference_cents, unmatched: wf.data.unmatched_bank }, note: "off by 4500p — C3 unmatched" }) }));
  ok("case: agent advanced the stage and recorded state", upd.ok && upd.case.stage === "reconciling" && upd.case.data.difference_cents === 4500);
  ...
  const chase = await jsonBody(await app.request("/v1/internal/actions/send_receipt_chase", { method: "POST", headers: AH, body: JSON.stringify({ connection_id: mailConn.id, to: "acme@client.co", body: "Missing 1 receipt for October" }) }));
  ...
  const gated = await Promise.race([
    (async () => (await jsonBody(await app.request("/v1/internal/actions/send_receipt_chase", { method: "POST", headers: AH, body: JSON.stringify({ connection_id: mailConn.id, to: "acme@client.co", body: "again" }) }))))(),
  ...
  const filing = await Promise.race([
    (async () => (await jsonBody(await app.request("/v1/internal/actions/file_sales_tax", { method: "POST", headers: AH, body: JSON.stringify({ connection_id: mailConn.id, amount: 2750 }) }))))(),
    new Promise((r) => setTimeout(() => r({ suspended: true }), 600)),
  ]) as any;
  ok("policy: tax filing always requires a human", filing.suspended === true);
  ...
  const bigData = Object.fromEntries(Array.from({ length: 50 }, (_, i) => [`tx_${i}`, { amount_cents: i * 100 }]));
