...
test("a machine-generated PDF gives up its text, compressed or not", () => {
  ...
  for (const compress of [true, false]) {
    const out = ok(extractText("statement.pdf", "application/pdf", pdfWith(["Brightline Ltd", "Balance due 4,200.00 USD"], { compress })));
    assert.match(out.text, /Brightline Ltd/);
...
test("an XLSX workbook yields tab-separated cell text", () => {
  ...
  assert.match(out.text, /Brightline\t450/);
});
