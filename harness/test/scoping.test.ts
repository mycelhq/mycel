...
test("listCases: a project sees only its own cases, and never an unscoped row", async () => {
  ...
  // There is no longer any way to ASK without a project: `CaseFilter.project_id` is required, so
  // the operator-wide read that used to return all three — including the ownerless legacy row —
  // does not typecheck. This line is the runtime half of that: the closest thing a caller can
  // still express is an empty scope, and an empty scope owns nothing.
  assert.equal((await d.listCases({ project_id: "" })).length, 0);
...
test("queryRecords/countRecords: same fail-closed tenant filter, and count agrees with rows", async () => {
  ...
  // Same as listCases: an unscoped count no longer compiles, and an empty scope counts nothing.
  assert.equal(await d.countRecords({ project_id: "", wedge: "books-keeper" }), 0);
});
