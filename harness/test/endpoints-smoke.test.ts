...
/** Every mutation route, with its verb — the surface where actions (and money) live. */
function registeredMutations(): { verb: "post" | "put" | "patch" | "delete"; path: string }[] {
  const found = new Map<string, { verb: "post" | "put" | "patch" | "delete"; path: string }>();
  for (const f of routeSources(SRC)) {
    for (const m of readFileSync(f, "utf8").matchAll(
      /app\.(post|put|patch|delete)\(\s*"(\/v1\/[^"]+)"/g,
    )) {
      const verb = m[1] as "post" | "put" | "patch" | "delete";
      const path = m[2]!;
      found.set(`${verb} ${path}`, { verb, path });
    }
  }
...
/**
 * The mutation surface — POST/PUT/PATCH/DELETE — hit UNAUTHENTICATED with an empty body.
 *
 * This is the "we shipped it and it just crashes" guard, made side-effect-free: with no bearer
 * token and no body, a correctly-wired route rejects at its auth/validation preamble — 400, 401,
 * 403, 404, 405, 415, 422 are all honest answers. A 500 is the bug: it means the handler threw
 * before it ever checked auth (a destructure of an absent body, a store read on undefined), which
 * is exactly the unhandled crash a founder meets as a dead button. No token means no guarded route
 * can perform its action, so nothing is sent, charged, or written.
 *
 * Webhooks and other deliberately-public routes reject an empty/garbage body with a 4xx too, so
 * they need no allowlist — the invariant is only "never 500", not "always 401".
 */
test("every mutation route rejects an unauthenticated empty request without a 500", async () => {
  const { app } = makeApp();
  const routes = registeredMutations();
  assert.ok(routes.length > 120, `expected the full mutation surface, found ${routes.length}`);

  const failures: string[] = [];
  let hit = 0;
  for (const { verb, path: route } of routes) {
    if (SKIP(route)) continue;
    const path = concrete(route);
    hit += 1;
    const res = await app.request(path, {
      ...
      body: verb === "delete" ? undefined : "{}",
    });
    const text = await res.text().catch(() => "");
    if (res.status === 500) {
      failures.push(`${verb.toUpperCase()} ${route} (${path}) → 500 ${text.slice(0, 200)}`);
    }
  }
  assert.ok(hit > 100, `mutation smoke skipped too much of the surface (${hit} routes)`);
  assert.equal(failures.length, 0, `mutation 500s on unauthenticated empty body:\n${failures.join("\n")}`);
});
