...
// The shims these used to call were one-liners over these, dropping `.skipped` — the half that
// says which rows could not be read. Deleted; the behaviour they assert lives here.
import { normaliseStripeCharges, normaliseStripeInvoices } from "../src/capabilities.normalise.ts";
...
test("payments: Stripe amounts are read from the field that means settled money", async () => {
  ...
  const [p] = normaliseStripeInvoices({ data: [stripeInvoice({ total: 400_000, amount_paid: 100_000 })] }).items;
  ...
  assert.deepEqual(normaliseStripeCharges({ data: [{ id: "ch_1", status: "pending", amount: 5000 }] }).items, []);
  const [c] = normaliseStripeCharges({ data: [{ id: "ch_2", status: "succeeded", amount_captured: 5000, amount_refunded: 2000, currency: "usd", created: 1 }] }).items;
