...
async function engagement(title = "Acme — October close") {
  ...
  /**
   * A mailbox, because `startNudge` now refuses `no_channel` before the pacing ladder — a chase with
   * nowhere to go no longer gets a sandbox. Without this the tests below that assert the REQUEST
   * LADDER is chasing would silently assert the new refusal instead, and pass for the wrong reason.
   */
...
test("waits: a wait must name a condition, a reason and something to resume", async () => {
  ...
  const base = { project_id: projectId, case_id: kase.id, reason: "r", condition: good, resume: { task_type: RESUME_TYPE , input: {} } };
  ...
  assert.equal((await armWait(domain, { ...base, resume: { task_type: "" , input: {} } })).ok, false, "a wait with no resume is a dead end");
...
test("waits: a wait is armed under the CASE's project, and another tenant's case is not found", async () => {
  ...
  const armed = await armWait(domain, { project_id: projectId, case_id: kase.id, reason: "ok", condition: cond, resume: { task_type: RESUME_TYPE , input: {} } });
...
test("waits: one live wait per case, and a closed case cannot wait on anything", async () => {
  ...
  const cond = { kind: "date", at: new Date(Date.now() + DAY).toISOString() } as const;
  const arm = () => armWait(domain, { project_id: projectId, case_id: kase.id, reason: "first", condition: cond, resume: { task_type: RESUME_TYPE , input: {} } });
  ...
  const viaRoute = await api(app, `cases/${kase.id}/wait`, {
    method: "POST",
    body: JSON.stringify({ reason: "third", condition: cond, resume: { task_type: RESUME_TYPE , input: {} } }),
  });
  ...
  const onClosed = await armWait(domain, {
    project_id: projectId, case_id: kase.id, reason: "x", condition: cond, resume: { task_type: RESUME_TYPE , input: {} },
  });
...
test("waits: an unknown resume task_type is refused at the ROUTE, when a human can still fix it", async () => {
  ...
  const res = await api(app, `cases/${kase.id}/wait`, {
    ...
    body: JSON.stringify({
      ...
      resume: { task_type: "no_such_task" , input: {} },
    }),
  });
...
test("waits: a client_reply is only satisfied by a message that arrived AFTER we started waiting", async () => {
  ...
  const armed = await armWait(domain, {
    ...
    condition: { kind: "client_reply", thread_id: thread.id }, resume: { task_type: RESUME_TYPE , input: {} },
  });
  assert.ok(armed.ok);
...
test("waits: an invoice or thread belonging to another tenant is UNRESOLVABLE, never satisfaction", async () => {
  ...
  const armed = await armWait(domain, {
    ...
    condition: { kind: "invoice_paid", invoice_id: foreignInvoice.id }, resume: { task_type: RESUME_TYPE , input: {} },
  });
  assert.ok(armed.ok);
...
test("waits: a condition that can never come true fails CLOSED and loudly, it does not hang", async () => {
  ...
  const armed = await armWait(domain, {
    ...
    condition: { kind: "request_resolved", request_id: req.id }, resume: { task_type: RESUME_TYPE , input: {} },
  });
  assert.ok(armed.ok);
  const wait = armed.ok ? armed.wait : null!;
...
test("waits: two replicas claiming the same wait concurrently produce exactly ONE resume", async () => {
  ...
  const armed = await armWait(domain, {
    ...
    condition: { kind: "date", at: new Date(Date.now() - DAY).toISOString() }, resume: { task_type: RESUME_TYPE , input: {} },
  });
  assert.ok(armed.ok);
  const wait = armed.ok ? armed.wait : null!;
  const spawned = recordingDeps();
...
test("waits: nothing resumes when resumption is not wired up, and a disabled wedge does not run", async () => {
  ...
  const { domain, projectId, kase } = await engagement();
  const cond = { kind: "date", at: new Date(Date.now() - DAY).toISOString() } as const;
  const armed = await armWait(domain, { project_id: projectId, case_id: kase.id, reason: "due", condition: cond, resume: { task_type: RESUME_TYPE , input: {} } });
  assert.ok(armed.ok);
  const wait = armed.ok ? armed.wait : null!;
...
test("waits: a resume is an ordinary queued Task — no send, no invoice, no way around a gate", async () => {
  ...
  await api(app, `cases/${kase.id}/wait`, {
    method: "POST",
    body: JSON.stringify({ reason: "waiting on Acme", condition: { kind: "client_reply", thread_id: thread.id }, resume: { task_type: RESUME_TYPE , input: {} } }),
  });
  await tick();
...
test("waits: one project's sweep never touches another project's waits", async () => {
  const { domain, projectId, kase } = await engagement();
  const cond = { kind: "date", at: new Date(Date.now() - DAY).toISOString() } as const;
  await armWait(domain, { project_id: projectId, case_id: kase.id, reason: "mine", condition: cond, resume: { task_type: RESUME_TYPE , input: {} } });
  const spawned = recordingDeps();
...
test("waits: a parked engagement leaves /next's move list and appears as HELD instead", async () => {
  ...
  await armWait(domain, {
    project_id: projectId, case_id: kase.id, reason: "waiting on Acme to confirm the scope",
    condition: { kind: "date", at: new Date(Date.now() + 30 * DAY).toISOString() }, resume: { task_type: RESUME_TYPE , input: {} },
  });
...
test("waits: a wait with no conditions, both spellings, a duplicate or an unknown mode is refused at arm time", async () => {
  ...
  const base = { project_id: projectId, case_id: kase.id, reason: "r", resume: { task_type: RESUME_TYPE , input: {} } };
