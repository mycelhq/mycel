 ...
 * This is the FIRST rung of the profile ladder (see profile.ts), so it is the shape the warm-up view
 * and the invite's urn resolution actually meet now that `profileView` answers 410.
 */
const dashProfilePayload = (publicId: string, first: string, last: string, urnId: string) => ({
  elements: [
    {
      publicIdentifier: publicId,
      firstName: { text: first },
      lastName: { text: last },
      entityUrn: `urn:li:fsd_profile:${urnId}`,
      headline: { text: "VP Engineering at Acme" },
      geoLocation: { defaultLocalizedName: "Austin" },
        ...
        elements: [
          {
            ...
            company: { universalName: "acme", companyPageUrl: "https://acme.com" },
          },
        ],
      },
    },
...
function mockVoyager(): Wire {
  ...
  _setFetch(async (url, init) => {
    ...
    // Either invitation endpoint: `MYCEL_LINKEDIN_DASH_INVITE` decides which, and the default
    // moved once already. A mock keyed to one of them silently stops intercepting when it flips.
    if (url.includes("/growth/normInvitations") || url.includes("verifyQuotaAndCreate")) {
    ...
    if (url.includes("/identity/dash/profiles")) {
      const id = decodeURIComponent(new URL(url).searchParams.get("memberIdentity") ?? "");
      return jsonResponse(dashProfilePayload(id, id.split("-")[0], id.split("-")[1] ?? "x", `ACoAAA${id}`));
    }
...
test("search → graph → enrol → paced → warm-up → invitation: the whole path a prospect takes", async () => {
  ...
  try {
    ...
    // The ladder's first rung, not the 410'd `profileView` this used to assert. What matters to this
    // file is unchanged — a warm-up view spends a touch and reaches the wire — but WHICH request it
    // makes is now the strategy ladder's decision, so this asserts the rung that answered.
    assert.ok(
      wire.urls.some((u) => u.includes("/identity/dash/profiles?") && u.includes("memberIdentity=dana-okafor")),
      wire.urls.join("\n"),
    );
    assert.ok(
      !wire.urls.some((u) => u.includes("/profileView")),
      "the retired endpoint is not called at all unless MYCEL_LINKEDIN_LEGACY_PROFILEVIEW=1",
    );
    ...
    // WHAT happened, not WHICH path it took. This asserted `/growth/normInvitations` — the
    // retired pre-Dash endpoint — so it passed for as long as every invitation failed, and
    // broke the moment they started working.
    assert.ok(
      wire.urls.some((u) => u.includes("normInvitations") || u.includes("verifyQuotaAndCreate")),
      `no invitation endpoint was called:\n${wire.urls.join("\n")}`,
    );
...
test("the dispatcher and the executor table cannot disagree about what is wired", () => {
  ...
  // `endorse_skill` MOVED here from the unwired list above: d0d6bd6 built its executor ("You were
  // right that the action should exist"), the package extraction dropped the kernel-side wiring,
  // and for a while this file pinned the accident while playbooks.test pinned the intent — two
  // suites asserting opposite worlds, both green never possible. The doctrine is the verbs.ts one:
  // dispatchable as a deliberate act on a chosen lead, never a sequence step, never auto-approved.
  for (const gated of ["follow_person", "react_to_post", "endorse_skill"]) {
