...
test("isMessagingSend: LinkedIn reads are not sends, and the exemption fails closed", () => {
  // This used to be `isMessagingSend(li, "anything") === true`, on the comment "a LinkedIn session
  // connection only ever sends". True when LinkedIn's one executor was a DM; false the day it became
  // seven. The stale belief refused `search_people` with `no_target` — a search has no recipient and
  // the guard insisted it must — which blocked the prospecting half of the go-to-market loop.
  for (const read of ["search_people", "view_profile", "get_profile", "get_company"]) {
    assert.equal(isMessagingSend(li, read), false, `${read} contacts nobody`);
  }
  // Retracting an invitation reduces contact; it does not initiate any.
  assert.equal(isMessagingSend(li, "withdraw_invite"), false);

  // The two that genuinely reach a stranger stay guarded, and `send_invite` is the single most
  // restriction-prone action LinkedIn offers.
  assert.equal(isMessagingSend(li, "send_message"), true);
  assert.equal(isMessagingSend(li, "send_invite"), true);

  // FAILS CLOSED. The exemption is an enumerated set, not a "looks like a read" pattern, so an
  // executor nobody has written yet is guarded by default rather than exempt by accident.
  assert.equal(isMessagingSend(li, "anything"), true);
  assert.equal(isMessagingSend(li, "fetch_something_new"), true);
});

test("a connection\'s profile composes with the guard in one call", () => {
  // `guardOutreach` was a two-line wrapper over exactly this and nothing called it. The composition
  // is what matters and it is still worth asserting; the wrapper was not.
  const guardOutreach = (conn: Parameters<typeof capabilitiesForConnection>[0], msg: Parameters<typeof guardSend>[1]) =>
    guardSend(capabilitiesForConnection(conn), msg);
