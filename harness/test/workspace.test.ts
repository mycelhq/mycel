...
test("product-builder seeds business-template and loads its skills", () => {
  ...
  /**
   * The checks moved to `business-template/scripts/verify-build.sh` so two wedges can share them
   * without duplicating seven kilobytes of shell. So the assertion follows them: what matters is
   * that the invariant is still ENFORCED, not which file spells it. Asserting on the manifest string
   * after the move would have passed for as long as the string mentioned tsc, and gone quiet the day
   * it stopped — which is the class of test this suite has already been bitten by.
   */
  assert.match(ws?.verify ?? "", /verify-build\.sh/, "the wedge runs the shared verify script");
  /**
   * `business-template/` is a SIBLING app and is not part of the published kernel, so in a clone
   * this threw ENOENT. The README's first instruction is `npm test`, promising "green. no keys",
   * with "if it is red, the clone is broken — not your machine" underneath it. A newcomer got red
   * and was told to distrust their own checkout.
   *
   * Absent means "not distributed", which is a pass: the assertions below are about that app's
   * script and travel with it. The manifest checks around them still run, here and in a clone.
   */
  const scriptPath = join(process.cwd(), "..", "business-template", "scripts", "verify-build.sh");
  if (existsSync(scriptPath)) {
    const script = readFileSync(scriptPath, "utf8");
    assert.match(script, /tsc --noEmit/, "the kernel must still typecheck the workspace");
    assert.match(script, /MycelMark/, "and still require attribution on our own sites");
    assert.match(script, /npm run dev/, "the kernel must still boot the app and curl it");
  }
  assert.equal(
  ...
  // THE WHOLE VERIFY ARRIVES, NOT A PREFIX OF IT. The assertions above all match inside the first
  // 500 characters, which is exactly why they stayed green through twenty-one consecutive failed
  // builds: `resolveWorkspace` truncated at 500 and the checks that got cut off — the Tailwind byte
  // count and the component-library count — are at the END of the string. Anchoring on the last
  // gate rather than the first is the difference between a test that watches this and one that
  // merely appears to.
  const declared = wedge.manifest.task_types!.build_feature!.workspace!.verify as string;
  assert.equal(ws?.verify, declared, "the verify must reach the sandbox whole, never a prefix");
  /**
   * The same guard, following the checks into the file they now live in. The point was never the
   * manifest string — it was that the LAST gate survives, because a truncation cuts the end and a
   * test asserting on the first gate would watch nothing.
   *
   * It matters more now, not less: the script has to reach the sandbox through the SEED, and the
   * seed silently truncating at 500 files is exactly how `scripts/` stopped arriving at all. The
   * seed's own regression test names these three scripts; this one asserts the content is whole.
   */
  if (existsSync(scriptPath)) {
    const script = readFileSync(scriptPath, "utf8");
    assert.match(script, /component library/, "the LAST gate must survive — truncation cuts the end");
    assert.match(script, /MYCEL_SITE_VERIFY_OK/, "and the script must reach its final line");
  }
});

test("a verify longer than the cap is refused by name, never silently shortened", () => {
  // The bug, exactly: `slice(0, 500)` cut product-builder's verify in the middle of a double-quoted
  // string. What reached the sandbox was not a shorter script, it was an INVALID one —
  // `unexpected EOF while looking for matching '"'`, exit 2, before a single check ran. Every build
  // then failed with "the app does not build, so it is not a deliverable": a verdict about the
  // agent's work, rendered by a gate that had not looked at it.
  //
  // A prefix of a shell script is not a shell script. So over the limit must be a refusal that
  // names the field, at resolve time, where a founder can act on it.
  assert.throws(
    () => resolveWorkspace({ workspace: { dir: "app", verify: "x".repeat(MAX_VERIFY_CHARS + 1) } }, "t"),
    /workspace\.verify is \d+ characters/,
  );
  const atLimit = resolveWorkspace({ workspace: { dir: "app", verify: "y".repeat(MAX_VERIFY_CHARS) } }, "t");
  assert.equal(atLimit?.verify?.length, MAX_VERIFY_CHARS, "the limit itself must be allowed through");
});

test("every verify a shipped wedge declares is a script bash can actually parse", async () => {
  // A gate that cannot compile is not a gate, and its failure is indistinguishable in the output
  // from the app being broken. Parsing every shipped manifest here means a malformed verify is
  // caught by `npm test` rather than by a customer's build failing for a reason nobody can read.
  const { sb, mkdir } = sandboxWithHome();
  mkdir("app");
  for (const name of ["product-builder"]) {
    const wedge = loadWedge(name);
    assert.ok(wedge, `${name} must load`);
    for (const taskType of Object.keys(wedge.manifest.task_types ?? {})) {
      const ws = resolveWorkspace(wedge.manifest, taskType);
      if (!ws?.verify) continue;
      await sb.writeFile(VERIFY_SCRIPT_PATH, `set -o pipefail\ncd ~/app || exit 2\n{ ${ws.verify}; } 2>&1 | tail -c 4000`);
      const r = await sb.exec(`bash -n ~/${VERIFY_SCRIPT_PATH}`);
      assert.equal(r.code, 0, `${name}/${taskType} verify does not parse: ${r.stderr}`);
    }
  }
});
...
test("a build run is told, in both the prompt and AGENTS.md, where the work goes", async () => {
  ...
  /**
   * The marking scheme moved into a script in the seed so two wedges could share it. What the agent
   * has to know did not change — it must be told the command it is graded by, and, when that command
   * is a file, told to READ it. A filename alone is strictly less than the seven thousand characters
   * of gates it replaced, and each of those gates names a real failure and how not to repeat it.
   */
  assert.match(md, /verify-build\.sh/, "the agent must know which command it will be marked on");
  assert.match(md, /Read it before you start/, "and be told to read it, since it is now a file");
...
// ═════════════════════════════════════════════════════════════════════════════════════════════════
// THE SEED HAS TO CARRY THE TEMPLATE IT SHIPS WITH
// ═════════════════════════════════════════════════════════════════════════════════════════════════
//
// This is the regression test for a production failure that ran for a week and looked like something
// else entirely. `business-template` grew to 620 files against a cap of 500 — 470 of them the
// component library — so `readSeed` spent its budget and stopped at
// `component-library/shadcn/spinner.json`. Everything after that in directory order, including ALL
// of `scripts/`, never reached the sandbox.
//
// Nothing said so. The app compiled, because those scripts are tooling rather than source, so the
// remote build reported `MYCEL_VERIFY_OK`. The kernel's own verify gate then died on
// `Cannot find module '/root/app/scripts/check-styles.mjs'` — an error naming a missing file, with
// no hint that the kernel had failed to put it there. Every build that got that far failed at the
// last step for a reason nowhere near the last step.

test("the shipped scaffold fits the seed budget, with the scripts the verify gate needs", () => {
  const seed = readSeed("business-template");
  if (!seed.root) return; // not packaged in this checkout; `seedWorkspace` has its own error for that

  assert.equal(
    seed.truncated,
    false,
    `the scaffold no longer fits: ${seed.files.length} files carried before the walk stopped. ` +
      `Whatever sorts last is now missing from every build, and the run will fail somewhere ` +
      `unrelated. Raise MAX_SEED_FILES/MAX_SEED_BYTES or shrink the template.`,
  );

  // Named explicitly rather than trusting the count, because these three are what the verify gate
  // and the agent's own instructions invoke by path — `node scripts/check-styles.mjs`,
  // `node scripts/component.mjs search`, `node scripts/template.mjs list`. A scaffold that carries
  // 619 of 620 files is fine unless the one it drops is one of these.
  for (const needed of ["scripts/check-styles.mjs", "scripts/component.mjs", "scripts/template.mjs"]) {
    assert.ok(
      seed.files.some((f) => f.name === needed),
      `${needed} is not in the seed. The verify gate runs it by path, so every build fails on ` +
        `MODULE_NOT_FOUND after the app has already compiled.`,
    );
  }
});

test("running out of budget is truncation, and one oversized file is not", () => {
  // The distinction that made the failure invisible. `skipped` means "this file could not come and
  // the rest is intact"; `truncated` means "we stopped partway and what is missing depends on
  // directory order, which nobody chose".
  const seed = readSeed("business-template");
  if (!seed.root) return;
  assert.equal(typeof seed.truncated, "boolean");
  assert.equal(typeof seed.skipped, "number");
});
