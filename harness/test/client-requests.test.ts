...
// ── a door on the room the ask was raised into ───────────────────────────────────────────────────
//
// MEASURED IN PRODUCTION before this existed: forty-one open client asks, three answered ever, and
// nothing since 27 August. Not apathy — the asks were unreachable. `thread_id` was null on all
// fifty-one asks ever written, because a thread needs a channel and one project of twenty-one had
// one. And `mintPortalLink` had a single non-test call site, a per-client route a founder has to
// know exists, so nine portal links had been issued in the product's life, for five clients.
//
// `party-link` refuses client asks with "they use the client portal", which assumed something put
// clients in front of it. This is that something.

test("requests: a client ask mints a portal link that actually opens their ask", async () => {
  const { app, a } = await world();
  const raised = await api(app, "requests", {
    method: "POST",
    body: JSON.stringify({ client_id: a.client.id, kind: "document", ask: "last month's bank statement" }),
  });
  assert.equal(raised.status, 201, "the ask was not raised");

  const minted = await api(app, `requests/${raised.json.id}/portal-link`, { method: "POST" });
  assert.equal(minted.status, 200, "no link for an open client ask");
  assert.ok(minted.json.portal_link?.token, "no token returned");

  // THE POINT. A token that does not open the ask is a link to nowhere, which is what we already
  // had. Exchange it and read the ask back through the resulting session.
  const session = await api(app, "portal/session", {
    method: "POST",
    body: JSON.stringify({ token: minted.json.portal_link.token }),
  });
  assert.equal(session.status, 200, "the minted link did not exchange for a session");
  const h = { authorization: `Bearer ${session.json.token}` };
  const mine = await api(app, "portal/requests", { headers: h });
  assert.ok(
    (mine.json as { id: string }[]).some((r) => r.id === raised.json.id),
    "the client opened the link and their own ask was not there",
  );
});

test("requests: the two link doors do not overlap", async () => {
  const { app, a } = await world();
  const clientAsk = await api(app, "requests", {
    method: "POST",
    body: JSON.stringify({ client_id: a.client.id, kind: "answer", ask: "which entity signs?" }),
  });
  // A client ask has no party link — that route says so itself.
  const asParty = await api(app, `requests/${clientAsk.json.id}/party-link`, { method: "POST" });
  assert.equal(asParty.status, 409, "a client ask should be refused a party link");

  const thirdParty = await api(app, "requests", {
    method: "POST",
    body: JSON.stringify({
      client_id: a.client.id, kind: "answer", ask: "confirm the year-end figure",
      party_role: "assessor", party_label: "Their accountant",
    }),
  });
  const asPortal = await api(app, `requests/${thirdParty.json.id}/portal-link`, { method: "POST" });
  assert.equal(asPortal.status, 409, "a third-party ask must not mint a CLIENT portal link");
});

test("requests: a closed ask mints nothing, and neither does another project's", async () => {
  const { app, a } = await world();
  const r = await api(app, "requests", {
    method: "POST",
    body: JSON.stringify({ client_id: a.client.id, kind: "answer", ask: "still needed?" }),
  });
  await api(app, `requests/${r.json.id}/cancel`, { method: "POST" }).catch(() => undefined);
  const after = await api(app, `requests/${r.json.id}/portal-link`, { method: "POST" });
  assert.notEqual(after.status, 200, "a closed ask handed out a live link to the client portal");
});
