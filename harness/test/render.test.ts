...
test("branding: a tenant's accent and name reach the PDF, and two tenants differ", () => {
  ...
  const other = resolveBrandKit({ display_name: "Sunset & Co", accent: "#0f766e" }, "proj name");
  ...
  assert.ok(b.includes("Sunset & Co"));
  assert.equal(a.includes("Sunset"), false);
...
test("a project still called `default` never becomes a letterhead", () => {
  ...
  // AND THE REPAIR IS BLANK, NOT "Invoice". The old fallback was a document TYPE, from when the only
  // document was one — and a monthly close then reached a client with "Invoice" printed where the
  // firm's name goes. A blank masthead reads as unbranded; a wrong one reads as a mistake, on the
  // first line of the first thing they open. Templates draw this as text, so empty draws nothing.
  assert.equal(resolveBrandKit(undefined, "default").display_name, "");
  assert.equal(resolveBrandKit(undefined, "  Default  ").display_name, "");
  assert.equal(resolveBrandKit(undefined, "").display_name, "");
...
test("invoice document: FROM prints the registered address, company number and VAT", () => {
  const text = invoiceText({
    invoice: invoice(),
    seller: { address: ["14 Willow Lane", "Bristol BS1 4DJ"], company_number: "12345678", vat_number: "GB123456789" },
  });
  assert.match(text, /FROM/);
  assert.match(text, /14 Willow Lane/);
