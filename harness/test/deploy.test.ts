...
/**
 * The old one-step `startDeploy`, expressed through the two steps that replaced it.
 *
 * Every property below — refuses an unattributed task, records a row before the upload, hands
 * CodeBuild nothing from the tarball, records the build id — is still true and still worth asserting.
 * What changed is that reaching CodeBuild now takes a person's yes, so the tests go through the real
 * path rather than a renamed shortcut. A helper and not a rewrite of thirty assertions, because the
 * thing under test is the handoff, not the gate; the gate has its own tests in `deploy-review.test.ts`.
 */
async function proposeAndPublish(
  d: InMemoryDomainStore,
  req: { projectId: string; slug: string; taskId?: string; base64: string },
  cfg: typeof CFG,
) {
  const dep = await proposeDeploy(d, req, cfg);
  return (await publishDeploy(d, dep.id, req.projectId, cfg)) ?? dep;
}

test("a deploy refuses a task with no project rather than deploying it somewhere", async () => {
  ...
  await assert.rejects(
    () => proposeAndPublish(d, { projectId: "", slug: "acme", base64: "" }, CFG),
    /no project/,
  );
});
...
test("nothing the pipeline is told about a tenant comes from the tarball", async () => {
  ...
  const d = new InMemoryDomainStore();
  const dep = await proposeAndPublish(d, { projectId: "proj-1", slug: "acme", taskId: "task-9", base64: "" }, CFG);
...
test("a deployment row exists BEFORE the upload, so a crash mid-deploy leaves evidence", async () => {
  ...
  const d = new InMemoryDomainStore();
  await assert.rejects(
    () => proposeAndPublish(d, { projectId: "proj-1", slug: "acme", base64: "" }, CFG),
...
test("a successful handoff records the build id — the only handle that leads to the log", async () => {
  fakeClient();
  const d = new InMemoryDomainStore();
  const dep = await proposeAndPublish(d, { projectId: "proj-1", slug: "acme", base64: "" }, CFG);
...
test("a project never has two live deployments — going live supersedes the previous one", async () => {
  ...
  const first = await proposeAndPublish(d, { projectId: "proj-1", slug: "acme", base64: "" }, CFG);
  await reconcileDeployment(d, first, CFG);
  const second = await proposeAndPublish(d, { projectId: "proj-1", slug: "acme", base64: "" }, CFG);
...
test("superseding never rewrites a failed deployment — that would erase the evidence", async () => {
  ...
  await d.supersedeDeployments("proj-1", "acme", live.id);
...
test("a failed build marks the row failed and does not supersede what is already live", async () => {
  ...
  const bad = await proposeAndPublish(d, { projectId: "proj-1", slug: "acme", base64: "" }, CFG);
...
test("the deployment routes never serve one tenant's URL to another tenant's key", async () => {
  ...
  const tok = (await jsonBody(login)).token as string;
...
test("a wedge that declares a workspace still runs in mock mode without an object store", async () => {
  ...
  const r = await api(app, "tasks", {
    method: "POST",
    body: JSON.stringify({ wedge: "product-builder", task_type: "build_feature", input: { brief: "a landing page" } /* `brief`, the field the contract and moves.ts both name */ }),
  });
...
// ═════════════════════════════════════════════════════════════════════════════════════════════════
// TWO CLIENTS, TWO SITES — the thing an agency could not do
// ═════════════════════════════════════════════════════════════════════════════════════════════════

test("an agency's second client going live does not retire their first", async () => {
  // THE BUG THIS RELEASE EXISTS TO FIX, in one assertion. Superseding was scoped to the PROJECT, and
  // a project is the agency — so publishing a site for Harborline retired the site for Brightline,
  // silently, because a superseded row is not an error anywhere. Add the unique index on
  // `(project_id) WHERE status='live'` and the second deploy could not even be recorded.
  const d = new InMemoryDomainStore();
  const brightline = await d.createDeployment({
    project_id: "proj-1", site_id: "case-brightline", slug: "ridgeline-1111aaaa", url: "u1", status: "live",
  });
  const harborline = await d.createDeployment({
    project_id: "proj-1", site_id: "case-harborline", slug: "ridgeline-2222bbbb", url: "u2", status: "live",
  });

  await d.supersedeDeployments("proj-1", "ridgeline-2222bbbb", harborline.id);

  assert.equal((await d.getDeployment(brightline.id, "proj-1"))?.status, "live", "the first client's site stays up");
  assert.equal((await d.getDeployment(harborline.id, "proj-1"))?.status, "live");
});

test("a redeploy of one site still retires that site's previous version", async () => {
  // The other half: narrowing the scope must not stop superseding working at all, or "what is my
  // URL" gets several answers for one site — which is how a customer is shown a URL a later deploy
  // replaced.
  const d = new InMemoryDomainStore();
  const old = await d.createDeployment({
    project_id: "proj-1", site_id: "case-brightline", slug: "ridgeline-1111aaaa", url: "u", status: "live",
  });
  const fresh = await d.createDeployment({
    project_id: "proj-1", site_id: "case-brightline", slug: "ridgeline-1111aaaa", url: "u", status: "live",
  });
  const n = await d.supersedeDeployments("proj-1", "ridgeline-1111aaaa", fresh.id);
  assert.equal(n, 1);
  assert.equal((await d.getDeployment(old.id, "proj-1"))?.status, "superseded");
});

test("a slug from a caller cannot retire another tenant's site", async () => {
  // `project_id` stays in the WHERE clause even though the slug is globally unique. Tenancy filters
  // are not an optimisation, and the day a slug is not as unique as we believed is the day this line
  // is the only thing standing.
  const d = new InMemoryDomainStore();
  const theirs = await d.createDeployment({
    project_id: "proj-2", site_id: "case-x", slug: "shared-slug", url: "u", status: "live",
  });
  const mine = await d.createDeployment({
    project_id: "proj-1", site_id: "case-y", slug: "shared-slug", url: "u", status: "live",
  });
  await d.supersedeDeployments("proj-1", "shared-slug", mine.id);
  assert.equal((await d.getDeployment(theirs.id, "proj-2"))?.status, "live", "another tenant is untouched");
});

test("`current` means the founder's own site, not whichever deployed last", async () => {
  // This asked for "the project's live deployment, limit 1", which was a complete question while a
  // project could only have one site. With client sites it is an ambiguous one, and the ambiguity
  // resolves by recency — so a founder checking their own address would sometimes be shown a
  // client's, depending on who was published most recently.
  const d = new InMemoryDomainStore();
  await d.createDeployment({
    project_id: "proj-1", site_id: "proj-1", slug: "ridgeline", url: "https://ridgeline.apps.test", status: "live",
  });
  await d.createDeployment({
    project_id: "proj-1", site_id: "case-brightline", slug: "ridgeline-1111aaaa", url: "https://ridgeline-1111aaaa.apps.test", status: "live",
  });

  const own = await d.listDeployments({ project_id: "proj-1", site_id: "proj-1", status: "live", limit: 1 });
  assert.equal(own[0]?.url, "https://ridgeline.apps.test");

  const client = await d.listDeployments({ project_id: "proj-1", site_id: "case-brightline", status: "live", limit: 1 });
  assert.equal(client[0]?.url, "https://ridgeline-1111aaaa.apps.test");
});

test("a deployment from before sites existed still answers as the project's own", async () => {
  // The migration case, and the one that would break silently: every row already in the database has
  // no site_id, and every one of them IS the founder's own site. If the filter did not read a
  // missing site_id as the project, "what is my URL" would 404 for every existing customer the day
  // this shipped.
  const d = new InMemoryDomainStore();
  await d.createDeployment({ project_id: "proj-1", slug: "ridgeline", url: "https://ridgeline.apps.test", status: "live" });
  const own = await d.listDeployments({ project_id: "proj-1", site_id: "proj-1", status: "live", limit: 1 });
  assert.equal(own.length, 1, "a null site_id reads as the project");
  assert.equal(own[0]?.url, "https://ridgeline.apps.test");
});

test("the site filter does not weaken the tenant filter", async () => {
  const d = new InMemoryDomainStore();
  await d.createDeployment({ project_id: "proj-2", site_id: "case-x", slug: "other", url: "u", status: "live" });
  const mine = await d.listDeployments({ project_id: "proj-1", site_id: "case-x", status: "live", limit: 10 });
  assert.equal(mine.length, 0, "another tenant's site is not visible by naming its id");
});
