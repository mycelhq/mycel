...
test("leak: an intake answer about client A is never mounted into client B's run", async () => {
  ...
  const forA = await recordAnswer(domain, {
    ...
    answer: "Brightline are withholding $4,200 pending the FCA complaint. Do not chase until it clears.",
    sensitivity: "client",
    clientId: "brightline",
  });
  assert.equal(forA.metadata.sensitivity, "client");
  assert.equal(forA.metadata.client_id, "brightline");

  const files = asFiles(await domain.listKnowledge(WEDGE, PROJECT));

  const forBrightline = retrieveFiles(files, ctx("brightline")).mounted.map((f) => f.name);
  assert.ok(forBrightline.includes(forA.name), "the client it is about still gets its own answer");
...
test("leak: a fact with no attribution at all is retrieved for nobody", async () => {
  ...
  for (const who of ["brightline", "acme", undefined]) {
...
test("leak: a house fact reaches every client, and a label only ever narrows", async () => {
  ...
  const narrowed = { ...house, metadata: { sensitivity: "house", client_id: "brightline" } };
  assert.equal(mayMount(narrowed, "brightline"), true);
...
test("leak: the +15 intake bonus can no longer promote another client's answer", async () => {
  ...
  const theirs: GroundingFile = {
    name: "theirs.md",
    content: "brightline are disputing invoice 118",
    // Newer AND carrying the intake bonus: under the old scorer it won the single slot outright.
    created_at: "2026-06-01T00:00:00.000Z",
    metadata: { intake: true, sensitivity: "client", client_id: "brightline" },
  };
...
test("leak: an answer to a gap raised on a client's task inherits that client", async () => {
  ...
  const client = (
    await api(app, "clients", { method: "POST", body: JSON.stringify({ name: "Brightline", handle: "brightline" }) })
  ).json;
  const kase = (
    await api(app, "cases", { method: "POST", body: JSON.stringify({ wedge: WEDGE, title: "Brightline", client_id: client.id }) })
  ).json;
