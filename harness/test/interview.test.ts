...
test("interview: questions drafted for one project are invisible to another", async () => {
  ...
  await store.addArtifact({
    ...
    content: JSON.stringify({
      questions: [
        {
          id: "brightline-terms",
          ask: "What did you agree with Brightline about Friday summaries?",
  ...
  assert.ok(
    forA.json.questions.some((q: { id: string }) => q.id === "brightline-terms"),
  ...
  assert.ok(
    !forB.json.questions.some((q: { id: string }) => q.id === "brightline-terms"),
    "another project never sees a question drafted from someone else's business",
  );
  assert.ok(
    !JSON.stringify(forB.json).includes("Brightline"),
