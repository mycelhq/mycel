// UPSTREAM TESTS, RUN VERBATIM. This is the whole point of keeping them: 94 cases written against
// open-design's own linter, executed against our copy of it, so "we ported it" is a claim the suite
// checks rather than something I assert. Only the harness is adapted — a shim over `node:test`,
// because the three matchers these use (toBe / toBeDefined / toBeUndefined) are a few lines and
// adding vitest to the kernel to avoid writing them would be the worse trade.
import { describe, it as nodeIt } from "node:test";
import assert from "node:assert/strict";

/**
 * `it`, plus the one vitest extra these tests use. `it.each(table)(name, fn)` expands a table into
 * one case per row, substituting `%s` in the name — table-driven regression pins, which is exactly
 * what you want for a list of banned hex values.
 */
type ItFn = ((name: string, fn: () => void | Promise<void>) => void) & {
  each: (rows: readonly unknown[][]) => (name: string, fn: (...args: never[]) => void | Promise<void>) => void;
};
const it = ((name: string, fn: () => void | Promise<void>) => nodeIt(name, fn)) as ItFn;
it.each = (rows) => (name, fn) => {
  for (const row of rows) {
    let i = 0;
    const title = name.replace(/%[sdifjo#%]/g, () => String(row[i++]));
    nodeIt(title, () => (fn as (...a: unknown[]) => void | Promise<void>)(...row));
  }
};

const expect = (actual: unknown) => ({
  toBe: (want: unknown) => assert.strictEqual(actual, want),
  toBeDefined: () => assert.notEqual(actual, undefined),
  toBeUndefined: () => assert.equal(actual, undefined),
});

import { lintArtifact, type LintFinding } from "../src/design-lint";
