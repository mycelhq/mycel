...
/**
 * The schedules this file is about: the ones a business EARNS by doing something — raising an
 * invoice turns the money clocks on, opening an engagement arms the ignition sweep.
 *
 * The three self-improvement reviews are excluded because they are the opposite kind: every project
 * carries them from creation, unconditionally, because a business reviewing its own work is true
 * from the first day. Counting them here would turn "a project that has done nothing carries no
 * clocks" — the actual invariant — into an assertion about how many schedules exist in total.
 */
const IMPROVEMENT_TASK_TYPES = new Set(["reflect_memory", "review_work", "review_artifacts"]);
const schedulesFor = async (projectId: string) =>
  (await getDomainStore().listSchedules())
    .filter((s) => s.project_id === projectId)
    .map((s) => s.task_type)
    .filter((t) => !IMPROVEMENT_TASK_TYPES.has(t));
