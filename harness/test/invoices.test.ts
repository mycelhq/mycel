...
import { api, connectMailbox, makeApp, jsonBody } from "./helpers";
...
async function world() {
  ...
  return { app, projectId, domain, memberToken: (await jsonBody(login)).token as string, a: await mk("acme"), b: await mk("beta") };
}
...
test("invoices: rails are project-scoped and travel on the by-id read", async () => {
  ...
  const put = await api(app, "payments/rails", {
    ...
    body: JSON.stringify({
      ...
      currency: "GBP",
      seller: { address: ["14 Willow Lane"], company_number: "12345678" },
    }),
  });
...
/**
 * THE INVOICE THAT COULD NEVER BE PAID BY CARD.
 *
 * A checkout used to be minted in exactly one place — the transition to `sent` — and that call is
 * best-effort, so an invoice issued before the business connected Stripe kept `payment_link_url`
 * null for ever. Nothing retried. The founder believed their client could pay; the client was
 * looking at a page with no button; the invoice aged into a chase.
 *
 * The order below is the real one, and it is the common one: send the first bill, THEN go and wire
 * up how to take the money.
 */
test("portal: a card link is minted on view for an invoice issued before Stripe was connected", async () => {
  const { app, projectId, a } = await world();

  // Issued with nothing connected. This is the state that used to be terminal.
  const inv = (await draft(app, a.client.id, { currency: "GBP" })).json as Invoice;
  await api(app, `invoices/${inv.id}/status`, { method: "POST", body: JSON.stringify({ to: "sent" }) });
  const before = await api(app, `portal/invoices/${inv.id}`, { headers: a.h });
  assert.equal(before.status, 200);
  assert.equal(before.json.payment_link_url ?? null, null, "no rail, no link — nothing has changed about that");

  // The founder goes and connects Stripe, after the fact.
  await api(app, "payments/rails", {
    method: "PUT",
    body: JSON.stringify({ rails: [{ kind: "stripe", enabled: true, lines: [] }], currency: "GBP" }),
  });
  ...
  setCheckoutDeps({
    ...
    execute: async () => {
      created += 1;
      return { ok: true, data: { url: "https://checkout.stripe.com/c/pay/cs_test_repair" } };
    },
    publicUrl: () => "https://app.example",
  });
  try {
    const after = await api(app, `portal/invoices/${inv.id}`, { headers: a.h });
    assert.equal(after.status, 200);
    assert.equal(after.json.payment_link_url, "https://checkout.stripe.com/c/pay/cs_test_repair");
    assert.equal(after.json.how_to_pay.online.url, "https://checkout.stripe.com/c/pay/cs_test_repair");
    assert.equal(created, 1);

    // IDEMPOTENT, which is what makes a write on a read defensible. A client refreshing the page
    // must not open a second checkout for the same debt.
    const again = await api(app, `portal/invoices/${inv.id}`, { headers: a.h });
    assert.equal(again.json.payment_link_url, "https://checkout.stripe.com/c/pay/cs_test_repair");
    assert.equal(created, 1, "the second view mints nothing");
  } finally {
    setCheckoutDeps({
      listConnections: async () => [],
      execute: async () => ({ ok: false, detail: "no stripe in this test" }),
...
/** A Stripe that will not answer must cost the client a button, never the page. */
test("portal: a refused checkout still renders the invoice and the other rails", async () => {
  const { app, projectId, a } = await world();
  await api(app, "payments/rails", {
    method: "PUT",
    body: JSON.stringify({
  ...
  try {
    ...
    await api(app, `invoices/${inv.id}/status`, { method: "POST", body: JSON.stringify({ to: "sent" }) });
    const detail = await api(app, `portal/invoices/${inv.id}`, { headers: a.h });
    assert.equal(detail.status, 200, "the page survives");
    assert.equal(detail.json.payment_link_url ?? null, null);
    assert.ok(
      detail.json.how_to_pay.blocks.some((b: { kind: string }) => b.kind === "bank_transfer"),
      "and the rail this business is actually paid by still prints",
    );
  } finally {
    setCheckoutDeps({
      listConnections: async () => [],
