...
import { getIdentityStore, INACTIVE_LIMITS, PLAN_LIMITS } from "../src/identity";
...
test("plan: running it yourself is unmetered, and that is the default", async () => {
  ...
  assert.deepEqual(r.json.limits, {
    seats: null, projects: null, tasks_per_month: null, model_spend_usd_per_month: null, meeting_joins_per_month: null,
    // `images_per_month` joins the list rather than being exempted from it: a self-hoster generating
    // images is spending their own AWS credits on their own account, and metering someone else's
    // bill would be exactly the paywall this test exists to keep out of a clone.
    images_per_month: null,
    linkedin_sessions: null, fulfillment: true, invoicing: true,
  });
...
test("plan: cancelling stops new work without becoming a free tier by another name", () => {
  // This used to assert `PLAN_LIMITS.free`, and that WAS the bug. Falling back to the free tier gave
  // every lapsed org 100 jobs and $2 of model spend a month, forever, for nothing — the free tier we
  // removed, reachable by cancelling. It is now INACTIVE_LIMITS: no new work, and nothing deleted.
  const id = getIdentityStore();
  ...
  // This is also where Stripe puts a subscription whose FIRST charge fails when the trial ends.
  id.setPlan(org.id, { status: "past_due" });
  assert.deepEqual(id.limitsFor(org.id), PLAN_LIMITS.growth, "past_due still works");
  assert.equal(id.workBlockedBy(org.id), null, "a declined card does not stop the business");

  id.setPlan(org.id, { status: "cancelled" });
  assert.deepEqual(id.limitsFor(org.id), INACTIVE_LIMITS);
  // Stopped, but never silently: the refusal names the status, so the product says what happened
  // instead of rendering "your plan includes 0 jobs a month".
  assert.equal(id.workBlockedBy(org.id), "cancelled");
});
...
test("plan: setting someone else's plan needs a credential of its own", async () => {
  ...
  // Compared against what the org was created with rather than against `undefined`. Orgs are now
  // stamped with a plan at creation (see `createOrgWithOwner`), so "nothing was written" means
  // "unchanged" — and asserting `undefined` would have started passing for the wrong reason the
  // moment any default appeared.
  assert.equal(id.getOrg(theirs.id)?.plan, "self_hosted", "and nothing was written");
...
test("plan: a spend ceiling stops the run that a job count would have allowed", async () => {
  ...
  /**
   * WHICH CEILING IS THE `code`'s JOB, NOT THE PROSE'S.
   *
   * This asserted the message said "model spend", and the message deliberately no longer does. A
   * founder whose client work has just stopped is not helped by learning our unit of account, and
   * naming compute in dollars invites them to price us against an API rather than against the hire
   * they are not making — which is the whole positioning argument.
   *
   * So the discriminator is `code`, which is what support and the console read, and the prose says
   * the two things a person needs: nothing is charged, and here is when it clears.
   */
  assert.match(refused.json.error, /clears on/);
  assert.match(refused.json.error, /nothing is charged/i);
  assert.ok(!/\$\d/.test(refused.json.error), "a founder-facing ceiling must not quote a compute price");

  // The job count is untouched — this is a second, independent ceiling, and the CODE says which one
  // you hit so the fix is obvious.
