...
test("patching context merges preferences instead of replacing the object", async () => {
  ...
  const client = (await api(app, "clients", {
    method: "POST",
    body: JSON.stringify({ display_name: "Acme", handles: ["a@acme.test"], preferences: { tone: "formal", tz: "Europe/London" } }),
  })).json;
  ...
  assert.deepEqual(patched.json.client.preferences, { tone: "warm", tz: "Europe/London" });
