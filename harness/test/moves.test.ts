...
test("a stated never-chase rule holds that client's invoice at rank time, and shows itself", async () => {
  ...
  const other = await makeClient(project, "Brightline");
...
/**
 * RESOLVED manifests, not the raw JSON on disk.
 *
 * This read `wedge.json` directly, and that stopped being the same thing on 6 September: the spine
 * — `nudge_client_request`, `check_in_case`, `deliverable_verdict` — is merged in `loadWedge` for
 * any wedge that ships to a client, so a trade inherits chasing without declaring it.
 *
 * Reading the file therefore answers a question no caller asks. `takeability` resolves through
 * `loadWedge`, so the assertion below has to as well, or it fails the day a trade correctly gains a
 * job for free — which is exactly how it failed on `content-desk`.
 */
async function manifests(): Promise<Map<string, Manifest>> {
  ...
  for (const slug of await readdir(dir)) {
    ...
    if (loaded) out.set(slug, loaded.manifest as unknown as Manifest);
  }
  return out;
}
 ...
 */
test("the three kinds refused on purpose stay refused, and the fourth has its precondition", async () => {
  const all = await manifests();
  const declaredAnywhere = new Set<string>();
  for (const m of all.values()) for (const t of Object.keys(m.task_types ?? {})) declaredAnywhere.add(t);

  /**
   * `rewrite_losing_arm` WAS the fourth, and it is now takeable. What it was refused FOR is still
   * asserted, one layer down.
   *
   * The old refusal was not "this has no carrier" — `product-builder` really declares
   * `build_feature`. It was "taking this would publish a marketing page with no approval between the
   * click and the internet", because `orchestrator.ts` called `startDeploy` on run completion. That
   * is the thing that had to stop being true, and it has: a finished build now leaves its deployment
   * at `awaiting_review` and CodeBuild is not asked for anything until a person answers.
   *
   * So this checks the PRECONDITION rather than the flag. If somebody makes a finished build publish
   * itself again, this test fails on the commit that does it — which is the only version of this
   * assertion worth having, because "is it takeable" is a one-line answer and "may it be" is the
   * question.
   */
  assert.ok(declaredAnywhere.has("build_feature"), "product-builder no longer declares build_feature");
  assert.ok(takeability("rewrite_losing_arm").takeable, "the gate exists, so this may be taken");
  const orchestrator = readFileSync(new URL("../src/orchestrator.ts", import.meta.url).pathname, "utf8")
    .replace(/\/\*[\s\S]*?\*\//g, "")
    .replace(/^\s*\/\/.*$/gm, "");
  assert.match(orchestrator, /await proposeDeploy\(/, "a finished build must propose, not publish");
  assert.doesNotMatch(
    orchestrator,
    /\bstartDeploy\b/,
    "a finished build publishes itself again - rewrite_losing_arm must go back to takeable: false",
  );
...
test("a decided experiment is proposed, quotes the kernel's own verdict, and may be taken", async () => {
  ...
  /**
   * TAKEABLE, and it was not until the deploy grew a review gate.
   *
   * The old assertion here was `takeable === false` with a reason matching /publish/i, because a
   * completed `build_feature` run published itself: `orchestrator.ts` called `startDeploy` and the
   * page was live with no person in between. Taking the move would have meant "publish a new
   * homepage" while the control beside it only drafted an email.
   *
   * The gate is in (`proposeDeploy` stops at `awaiting_review`), so the click now means what every
   * other take means — start the work — and the publishing decision is a second, separate answer on
   * the deployment. The precondition is asserted directly in "the three kinds refused on purpose",
   * which fails if a finished build ever publishes itself again.
   */
  assert.equal(move.takeable, true, "the review gate exists, so this may be taken");
  assert.equal(move.unavailable_reason, undefined, "a takeable move needs no refusal sentence");
