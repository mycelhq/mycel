...
/**
 * A response body, in a test.
 *
 * `Response.json()` is typed `Promise<unknown>`, which is correct — a server can return anything —
 * and in a test it means every assertion about a field needs a cast at the call site. Sixty-nine of
 * those were never written, because `harness/test` sat outside `tsconfig.json`'s `include` and
 * nothing type-checked it.
 *
 * That gap was not cosmetic. `deploy.test.ts` called `supersedeDeployments(projectId, keepId)` after
 * the signature grew a `slug` in the MIDDLE; the call went through with `slug = keepId` and
 * `keepId = undefined`, nothing was superseded, and a test asserting that a failed row stays failed
 * passed for exactly the opposite of the reason it claimed.
 *
 * Named `jsonBody` rather than `body` because half these files already have a local `body` — the
 * request payload they are about to send — and shadowing it would be a rename that reads as a bug.
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export const jsonBody = async (res: { json(): Promise<unknown> }): Promise<any> => res.json();
