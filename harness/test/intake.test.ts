...
test("intake: a question only the client can answer becomes a client request, not a founder gap", async () => {
  ...
  const projectId = (await api(app, "me")).json.projects[0].id;
  /**
   * A client who can actually be reached, which this test did not previously need.
   *
   * `clientReachable` now redirects an undeliverable client question to the founder — the fix for
   * all fifty-one production asks having `thread_id` NULL. So a test asserting the question BECOMES
   * a client request has to set up a client who can be asked one. Before this it passed against an
   * unreachable client, which is exactly the state that was shipping.
   */
  ...
  const reachable = await getDomainStore().createClient({
  ...
  assert.equal(raised.client_id, reachable.id, "the request must be scoped to the client it was raised for");
...
test("intake: routing is the agent's declaration, never a guess from the question's wording", async () => {
  ...
  const ctx = { project_id: "p1", wedge: WEDGE, task_id: "t1", client_id: "brightline" };
