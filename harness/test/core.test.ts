...
test("task: validation, clamp, and a successful mock run", async () => {
  ...
  assert.equal((await api(app, "tasks", { method: "POST", body: JSON.stringify({ wedge: "books-keeper", task_type: "nope" }) })).status, 400);

  // constraints clamped to server ceilings
  const created = await api(app, "tasks", {
    method: "POST",
    body: JSON.stringify({ wedge: "books-keeper", task_type: "daily_sync", input: { message: "hi" }, constraints: { max_cost_usd: 99999 } }),
  });
...
test("SSE replays the ordered event stream to a terminal event", async () => {
  const { app } = makeApp();
  const { json: task } = await api(app, "tasks", { method: "POST", body: JSON.stringify({ wedge: "books-keeper", task_type: "daily_sync", input: { message: "hi" } }) });
