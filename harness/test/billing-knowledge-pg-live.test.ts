...
test("postgres billing: only one of two racing transitions wins", { skip }, async () => {
  ...
  const store = await PostgresBillingStore.connect(URL!);
  const p = project();
  const i = await store.createInvoice({ project_id: p, client_id: "c", currency: "GBP", status: "sent", lines: [] });
...
test("postgres billing: listInvoices never crosses a tenant", { skip }, async () => {
  ...
  const a = project();
  const b = project();
  const mine = await store.createInvoice({ project_id: a, client_id: "c", currency: "GBP", status: "draft", lines: [] });
  await store.createInvoice({ project_id: b, client_id: "c", currency: "GBP", status: "draft", lines: [] });
