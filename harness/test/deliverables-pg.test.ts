...
test("deliverables.pg: a version number is allocated inside the write, under a row lock", () => {
  ...
  /**
   * THE OUTGOING VERSION IS STAMPED WITH THE TIMESTAMP, and this asserts the PROPERTY rather than a
   * placeholder number — because the literal is what pinned a production bug in place.
   *
   * This line used to read `/superseded_at = \$9/`. `$9` is `author`; the timestamp is `$10`. So the
   * statement assigned a text parameter to a `timestamptz` column, Postgres refused to deduce a
   * consistent type for `$9`, and the whole statement failed to PREPARE — every deliverable version
   * submitted on Postgres. This suite passed throughout, because it asserts the string against a
   * FakeDb and the assertion had been updated to match the break.
   *
   * Asserting "the same parameter `created_at` uses" cannot be satisfied by the wrong number.
   */
  const tsParam = /created_at\)\s*\n?\s*SELECT[^\n]*?(\$\d+)\s*$/m.exec(sql)?.[1]
    ?? sql.match(/,\s*(\$\d+)\s*\n\s*FROM parent/)?.[1];
  assert.ok(tsParam, "could not find which parameter carries the timestamp");
  assert.ok(
    sql.includes(`superseded_at = ${tsParam}`),
    `superseded_at must use the timestamp parameter ${tsParam}, not a text one`,
  );
  assert.ok(sql.includes(`updated_at = ${tsParam}`), `updated_at must use the timestamp parameter ${tsParam}`);
...
// ── the pin, which is the field most likely to work in tests and not in production ───────────────

test("the house style is written to Postgres, not just to the in-memory twin", () => {
  // THE TRAP THIS EXISTS FOR. `createDeliverable`'s INSERT names its columns explicitly, so a new
  // field on `NewDeliverable` type-checks, passes every test that runs against the memory store, and
  // is silently dropped by the only store production uses. The result is a pin that exists in the
  // type system, in the tests and nowhere a client's report is actually rendered from.
  const db = new FakeDb(() => [ROW]);
  const store = new PostgresDeliverableStore(db);
  void store.createDeliverable({
    project_id: "p1",
    case_id: "c1",
    client_id: "cl1",
    title: "Q1 accounts",
    kind: "document",
    style: { system: "brutalism", evidence: "chosen", digest: "abc123", at: "2026-09-07T00:00:00.000Z" },
  });
  const insert = db.calls.find((c) => /INSERT INTO deliverables/.test(c.sql));
  assert.ok(insert, "no insert was issued");
  assert.match(insert.sql, /\bstyle\b/, "the column is not in the INSERT — the pin is dropped on write");
  const stored = insert.vals.find((v) => typeof v === "string" && v.includes("brutalism"));
  assert.ok(stored, "the style value never reached the parameter list");
  assert.equal(JSON.parse(String(stored)).evidence, "chosen", "provenance must survive the round trip");
});

test("a deliverable with no pin inserts NULL rather than the string 'undefined'", () => {
  // Every row written before pins existed has none, and `JSON.stringify(undefined)` is `undefined`,
  // not `"null"` — passing that to `pg` for a jsonb column is an error at insert time, which would
  // turn "this business has no house style yet" into "this business cannot create deliverables".
  const db = new FakeDb(() => [ROW]);
  void new PostgresDeliverableStore(db).createDeliverable({
    project_id: "p1", case_id: "c1", client_id: "cl1", title: "x", kind: "document",
  });
  const insert = db.calls.find((c) => /INSERT INTO deliverables/.test(c.sql))!;
  assert.equal(insert.vals[6], null);
});

test("a style stored as text still reads back as an object", () => {
  // `jsonb` comes back parsed from `pg`; a text column, a hand-inserted row or a restore from a
  // dump does not. A pin read as a string keeps every type check happy and silently stops matching.
  const asObject = rowToDeliverable({ ...ROW, style: { system: "brutalism", evidence: "chosen", digest: "d", at: "t" } });
  assert.equal(asObject.style?.system, "brutalism");
  const asText = rowToDeliverable({ ...ROW, style: '{"system":"brutalism","evidence":"chosen","digest":"d","at":"t"}' });
  assert.equal(asText.style?.system, "brutalism");
  assert.equal(rowToDeliverable({ ...ROW, style: null }).style, undefined);
});

test("the column is created by the store's own migration", () => {
  // A field the INSERT names and no migration adds is an insert that fails on every existing
  // install — the opposite failure, and just as invisible from the memory store.
  const src = readFileSync(new URL("../src/deliverables.pg.ts", import.meta.url), "utf8");
  assert.match(src, /ALTER TABLE deliverables ADD COLUMN IF NOT EXISTS style jsonb/);
});
