...
test("postgres: tasks + service surface survive a restart", { skip: URL ? false : "set MYCEL_TEST_DATABASE_URL to run" }, async () => {
  ...
  const taskRes = await app1.request("/v1/tasks", { method: "POST", headers: H, body: JSON.stringify({ wedge: "books-keeper", task_type: "daily_sync", input: { message: "durable?" } }) });
  assert.equal(taskRes.status, 201);
  const taskId = (await jsonBody(taskRes)).id as string;
  await app1.request("/v1/connections", { method: "POST", headers: H, body: JSON.stringify({ kind: "email", name: `pg-${Date.now()}`, config: {} }) });
  await app1.request("/v1/wedges/books-keeper/knowledge", { method: "POST", headers: H, body: JSON.stringify({ name: `pg-${Date.now()}.md`, content: "survives", kind: "fact" }) });
  ...
  const conns = await jsonBody(await app2.request("/v1/connections", { headers: H }));
  assert.ok(conns.length > 0, "connections persisted and are in scope");

  const knowledge = await jsonBody(await app2.request("/v1/wedges/books-keeper/knowledge", { headers: H }));
