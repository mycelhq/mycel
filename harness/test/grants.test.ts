...
test("the TTL is a backstop, not the mechanism: it outlives the runtime ceiling", () => {
  ...
  // THE REAL CEILING, from the config that decides it — not a third copy of its default.
  //
  // This line used to read `process.env.MYCEL_MAX_RUNTIME_S ?? 1800`, which is config.ts's number
  // written out again with a different fallback. When the ceiling moved to three hours the TTL
  // stayed at one, every grant a long run holds would have expired mid-run, and THIS TEST PASSED —
  // because it compared the stale TTL against its own equally stale copy of the ceiling. A
  // duplicated constant does not just drift; it disables the check that would have caught it.
  const ceilingMs = loadConfig().maxRuntimeCeilingS * 1000;
