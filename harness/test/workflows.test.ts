...
test("workflows: a throwing workflow fails loudly instead of returning junk", async () => {
  ...
  /**
   * ═══ THE PROPERTY, NOT THE SENTENCE — AND THE SENTENCE CHANGED FOR A GOOD REASON ═══
   *
   * This matched `/integer cents/`, which was the LIBRARY's own message. Declaring an `items` shape
   * on `reconcile.bank_transactions` moved the refusal earlier, to the schema boundary, where it
   * now reads:
   *
   *     invalid args: $.bank_transactions[0].amount_cents: expected integer, got number
   *
   * That is strictly better — refused at the contract rather than inside the maths — and it failed a
   * test that was pinning a spelling. What matters is that it refuses AND names the field, so a
   * caller knows which line to fix. Both messages satisfy that; only one satisfied the old regex.
   */
  assert.match(r.error!, /amount_cents/, "the refusal no longer names the field that is wrong");
  assert.match(r.error!, /integer/i, "the refusal no longer says what was expected");
});
