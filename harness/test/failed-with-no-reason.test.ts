...
/**
 * Backdated, and that is the scenario rather than a convenience.
 *
 * These tests describe a run "reclaimed on boot" — a row that has been sitting since a worker died.
 * `listUnfinished()` only ever returns rows untouched for `STALE_TASK_MS`, so a fixture stamped
 * `now()` is not the thing being tested; it is a healthy run.
 *
 * They passed anyway, because the MEMORY store had no default for `staleAfterMs` while the Postgres
 * one defaulted to ten minutes — so on memory the reaper saw every non-terminal row regardless of
 * age. That divergence cancelled live runs on every install without Postgres, and these tests were
 * encoding it. Backdating is what makes them describe the case their names claim.
 */
