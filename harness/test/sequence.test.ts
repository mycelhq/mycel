...
import { inboxHealth, recordEmailSend } from "../src/outreach/inbox-health";
...
/**
 * ═══ ENROL ON THE TEST'S CLOCK, NOT THE WALL CLOCK ═══
 *
 * `enrollProspect` defaults `now` to `new Date()` and derives the case's `due_at` from it. Every
 * test below then advances the sequencer at a FIXED instant — `2026-09-02T11:00:00Z`. So whether a
 * case was due depended on what time of day the suite ran: before 11:00 UTC it was due and the tests
 * passed, after 11:00 UTC it was not and `advanceSequences` reported `processed: 0`, which reads as
 * "the cap was not enforced" when nothing was ever evaluated.
 *
 * Diagnosed by printing the summary: `{processed: 0, sent: 0}` on a test asserting a mailbox cap.
 * The suite went red at 11:00 UTC and would have gone green again the next morning, which is the
 * worst shape a failure can have — it looks like flakiness and it is a fixture reading the clock.
 *
 * `at` is the same instant the test advances at, so the case is due the moment the test looks.
 */
async function enrolled(
  campaign: Campaign,
  over: Partial<Case["data"]> = {},
  stage = "connected",
  at: Date = FIXED_NOW,
): Promise<Case> {
  const kase = await enrollProspect(
    domain(),
    campaign,
...
// ── The paused campaign, and the starvation it used to cause ─────────────────────────────────────
//
// THE FAILURE THESE ENCODE, which happened in the founder's own outbound engine before it could
// happen to a customer: 478 live enrolments, 316 of them on a single PAUSED campaign, ten slots per
// tick. The campaign was consulted inside the loop, after the batch had been picked — so the paused
// rows, whose `due_at` values were the OLDEST precisely because they had been deferred longest,
// sorted to the front and filled every slot. 948 denials over two days, counters reading
// `claimed=10 executed=10`. Busy, achieving nothing, and nothing red anywhere.
//
// The mechanism is what makes it such a reliable trap: the fairness rule (oldest first) is what
// hands the batch to the rows that will be refused. A queue with worse manners would have starved
// less.

test("a paused campaign does not eat the batch", async () => {
  const store = new InMemoryStore();
  const project = "p-paused-starve";
  const conn = await account(project);
  spyDispatcher();

  const dead = await campaignWith(store, project, conn, { name: "paused" });
  const live = await campaignWith(store, project, conn, { name: "live" });
  await setCampaignStatus(domain(), project, dead.campaign.id, "paused", "the copy was wrong");

  // The paused campaign's cases are OLDER — which is exactly the situation that produced the
  // outage, because deferral is what makes them old.
  const old = new Date(Date.now() - 6 * 3_600_000).toISOString();
  for (let i = 0; i < 4; i++) {
    const k = await enrollProspect(domain(), dead.campaign, { profile_id: `dead-${i}`, name: `Dead ${i}`, thread: `urn:t:d${i}`, copy: { send_message: "no" } });
    await domain().updateCase(k.id, { stage: "connected", due_at: old, data: { ...k.data, connected: true } });
  }
  const wanted = await enrollProspect(domain(), live.campaign, { profile_id: "live-1", name: "Live", thread: "urn:t:l1", copy: { send_message: "hi" } });
  await domain().updateCase(wanted.id, { stage: "connected", data: { ...wanted.data, connected: true } });

  // A batch of two. Under the old ordering both slots went to the paused campaign and the live
  // prospect was never reached — which is the whole bug, and why the limit here is smaller than the
  // number of paused rows.
  const summary = await advanceSequences(store, domain(), { project_id: project, limit: 2 });

  assert.equal(summary.processed, 1, "only the live campaign's case was even considered");
  assert.ok(summary.sent + summary.paced + summary.parked >= 1, "and it was actually worked");
  assert.match(summary.note ?? "", /4 prospects held: their campaign is paused/);
});

test("pausing keeps the work — resuming does not restart anybody", async () => {
  const store = new InMemoryStore();
  const project = "p-pause-keeps";
  const conn = await account(project);
  spyDispatcher();
  const { campaign } = await campaignWith(store, project, conn);
  const kase = await enrolled(campaign);

  await setCampaignStatus(domain(), project, campaign.id, "paused");
  const held = await advanceSequences(store, domain(), { project_id: project });
  assert.equal(held.processed, 0, "nothing is due work while nobody wants it sent");

  const during = (await domain().getCase(kase.id))!;
  assert.equal(during.stage, "connected", "the stage is untouched — a pause is not a cancel");

  const resumed = await setCampaignStatus(domain(), project, campaign.id, "active");
  assert.equal(resumed?.status, "active");
  assert.equal(resumed?.paused_reason, undefined, "a stale reason must not sit beside a running campaign");

  const after = await advanceSequences(store, domain(), { project_id: project });
  assert.equal(after.processed, 1, "it picks up where it stopped");
});

test("a founder taking one touch by hand is refused while the campaign is paused", async () => {
  // `advanceSequences` filters the population, so the belt in `advanceCase` is unreachable from the
  // tick — but `startNextTouch` is the "send this one now" button and goes through the same
  // function. Without the belt, pausing would stop the machine and leave the button live.
  const store = new InMemoryStore();
  const project = "p-pause-byhand";
  const conn = await account(project);
  const calls = spyDispatcher();
  // Without this `startNextTouch` refuses `not_configured` before it reaches anything worth
  // testing — which is a green test about nothing.
  setTouchDeps({ store });
  const { campaign } = await campaignWith(store, project, conn);
  const kase = await enrolled(campaign);
  // Due in the PAST, or `startNextTouch` refuses on its own pacing rule and never reaches the belt —
  // which would make this test pass for the wrong reason.
  await domain().updateCase(kase.id, { due_at: new Date(Date.now() - 60_000).toISOString() });
  await setCampaignStatus(domain(), project, campaign.id, "paused", "bad list");

  // `(domain, case)` — the store comes from `setTouchDeps`, not from the call.
  const fresh = (await domain().getCase(kase.id))!;
  const r = await startNextTouch(domain(), fresh);
  assert.equal(calls.length, 0, "nothing went out");
  assert.equal(r.ok, false);
  assert.equal(r.reason, "parked");
  assert.match(r.message ?? "", /paused — bad list/, "and it says whose decision it was and why");
  setTouchDeps(null);
});

test("a campaign with no status is live — every row written before the field existed keeps running", () => {
  assert.equal(campaignIsLive({ status: undefined }), true);
  assert.equal(campaignIsLive({ status: "active" }), true);
  assert.equal(campaignIsLive({ status: "paused" }), false);
  assert.equal(campaignIsLive(undefined), false, "a missing campaign is not a live one");
});

// ── cold email has its own ceiling now, and it is not LinkedIn's ────────────────────────────────

/** A mail connection, so `send_email` dispatches on something with a `from` the ramp can key on. */
/**
 * ═══ THE INSTANT EVERY CLOCK-SENSITIVE TEST HERE RUNS AT ═══
 *
 * Derived from the real clock, not hardcoded, and it has to satisfy three things at once:
 *
 *   · a WEEKDAY, MID-MORNING — several tests assert a mailbox cap or a spacing rule, and outside the
 *     sending window the refusal is `outside_window` instead, which is a different assertion. The
 *     comment on the first of them says so: "Mid-morning on a Wednesday, so the window is not what
 *     is being tested here."
 *   · IN THE PAST, because campaigns, grants and cases are created at the REAL clock by helpers that
 *     do not take a `now`. An instant in the future finds them not yet live. (Measured: a fixed
 *     2027 date fails 15 of 29 for exactly this reason.)
 *   · NOT SO FAR PAST that those same real-clock rows post-date it.
 *
 * A hardcoded date satisfied all three on the day it was written and decayed. `2026-09-02T11:00:00Z`
 * passed all morning and went red at 11:00 UTC — after which every run created its fixtures later
 * than the instant it advanced at, so `advanceSequences` reported `processed: 0` and a mailbox-cap
 * test read as "the cap was not enforced" when nothing had been evaluated at all.
 *
 * So: the most recent weekday at 11:00 UTC, which is always a valid window and always just behind
 * whatever the real clock is doing.
 */
const FIXED_NOW = (() => {
  const d = new Date();
  d.setUTCHours(11, 0, 0, 0);
  // Before 11:00 UTC today, 11:00 today is still ahead of the fixtures — step back a day.
  if (d.getTime() >= Date.now()) d.setUTCDate(d.getUTCDate() - 1);
  // Sunday (0) and Saturday (6) are outside every sending window.
  while (d.getUTCDay() === 0 || d.getUTCDay() === 6) d.setUTCDate(d.getUTCDate() - 1);
  return d;
})();

async function mailbox(project: string, from = "hello@hartley.co.uk"): Promise<Connection> {
  return domain().createConnection({
    project_id: project,
    kind: "email",
    name: "mail",
    owner: { kind: "founder", id: "founder" },
    config: { api_url: "http://127.0.0.1:1/never-called", from },
  });
}

const EMAIL_FIRST: SequenceStep[] = [{ from: "connected", action: "send_email", advance_to: "em1", only_if: "connected AND !replied" }];

test("cold email is paced by the mailbox's warm-up, not waved through", async () => {
  // ═══ THE HOLE THIS CLOSES ═══
  //
  // `CROSS_CHANNEL_ACTIONS` correctly exempted email from LinkedIn's invite budget — `touchFor`
  // defaults unknowns to "invite", so every em1 send was silently throttling the founder's LinkedIn
  // account. The exemption was replaced with `{ allowed: true }`, which meant cold outreach email
  // had NO ceiling of any kind: no warm-up, no daily cap, no spacing, no working hours. A brand-new
  // mailbox could open a hundred conversations on its first afternoon, which reads to a receiver
  // exactly like a compromised account.
  const store = new InMemoryStore();
  const conn = await mailbox("p-mail-cap");
  const { campaign } = await campaignWith(store, "p-mail-cap", conn, { steps: EMAIL_FIRST });
  await enrolled(campaign, { email: "dana@corp.com", copy: { send_email: "hi Dana", email_subject: "Quick note" } });
  const calls = spyDispatcher();
  try {
    // Mid-morning on a Wednesday, so the window is not what is being tested here.
    const now = FIXED_NOW;
    // A brand-new mailbox is at week one: five a day. Spend them.
    for (let i = 0; i < 5; i++) {
      await recordEmailSend({ project_id: "p-mail-cap", address: "hello@hartley.co.uk", now });
    }
    const summary = await advanceSequences(store, domain(), { project_id: "p-mail-cap", now });
    assert.equal(summary.sent, 0, "at cap, so nothing goes out");
    assert.equal(calls.length, 0, "and nothing reached the transport");

    // Parked with a reason a founder reads, and a wait that matches: come back tomorrow.
    const parked = (await domain().listCases({ project_id: "p-mail-cap" })).find((k) => k.data?.email === "dana@corp.com")!;
    assert.match(String(parked.data?.paused_reason ?? parked.data?.note ?? JSON.stringify(parked.data)), /sent its 5 for today/);
  } finally {
    _setDispatcher(null);
  }
});

test("a successful cold email counts against the mailbox; a failed one does not", async () => {
  // Counting on intent would let a run of transport failures eat a mailbox's whole allowance without
  // one message reaching anybody — and the warm-up would then advance on volume never delivered,
  // which is the fiction the earned-week rule exists to refuse.
  const store = new InMemoryStore();
  const conn = await mailbox("p-mail-count", "count@hartley.co.uk");
  const { campaign } = await campaignWith(store, "p-mail-count", conn, { steps: EMAIL_FIRST });
  await enrolled(campaign, { email: "dana@corp.com", copy: { send_email: "hi Dana", email_subject: "Quick note" } });
  const now = FIXED_NOW;
  const calls = spyDispatcher();
  try {
    await advanceSequences(store, domain(), { project_id: "p-mail-count", now });
    assert.equal(calls.length, 1);
    const board = await inboxHealth("p-mail-count", now);
    assert.equal(board[0]?.sentToday, 1, "the send that happened is counted");
  } finally {
    _setDispatcher(null);
  }

  // Now a transport failure on a fresh mailbox: dispatched, refused, and NOT counted.
  const conn2 = await mailbox("p-mail-fail", "fail@hartley.co.uk");
  const { campaign: c2 } = await campaignWith(store, "p-mail-fail", conn2, { steps: EMAIL_FIRST });
  await enrolled(c2, { email: "dana@corp.com", copy: { send_email: "hi Dana", email_subject: "Quick note" } });
  _setDispatcher(async () => ({ ok: false, detail: "HTTP 502" }));
  try {
    await advanceSequences(store, domain(), { project_id: "p-mail-fail", now });
    assert.deepEqual(await inboxHealth("p-mail-fail", now), [], "nothing delivered, so nothing counted");
  } finally {
    _setDispatcher(null);
  }
});

test("a LinkedIn step still spends LinkedIn's budget and never the mailbox's", async () => {
  // The original bug, asserted from the other side: the two ceilings must stay separate, or email
  // volume throttles the founder's LinkedIn account exactly as it used to.
  const store = new InMemoryStore();
  const conn = await account("p-li-only");
  const { campaign } = await campaignWith(store, "p-li-only", conn);
  await enrolled(campaign);
  const calls = spyDispatcher();
  try {
    await advanceSequences(store, domain(), { project_id: "p-li-only", now: FIXED_NOW });
    assert.equal(calls.length, 1);
    assert.equal(calls[0].action, "send_message");
    assert.deepEqual(await inboxHealth("p-li-only"), [], "a LinkedIn send is not a mailbox send");
  } finally {
    _setDispatcher(null);
  }
});
