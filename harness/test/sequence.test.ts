...
import { advanceSequences, ensureSequenceSchedule, factsFor, _setDispatcher } from "../src/gtm/sequence";
...
test("a pacing refusal reschedules the case instead of sending — with a sentence the founder can read", async () => {
  const store = new InMemoryStore();
  // Weekend/3am, expressed the way pacing sees it: outside this account's working hours.
  const conn = await account("p-paced", { utc_offset: workingHoursOffset() + 24 * 5 });
