...
async function twoReplicas() {
  ...
  const me = await jsonBody(await a.request("/v1/me", { headers: { authorization: "Bearer testkey" } }));
...
test("multi-replica: cancel on instance B cancels a task created on instance A", async () => {
  const { store, a, b, projectId } = await twoReplicas();
  const now = new Date().toISOString();
  await store.createTask({
    id: "cancel-task", project_id: projectId, wedge: "w", task_type: "x",
    actor: { kind: "system", id: "s" }, input: {},
    constraints: { max_runtime_s: 300, max_cost_usd: 1, approval_required: false },
    tools: [], status: "running", cost_usd: 0, created_at: now, updated_at: now,
  } as never);
  ...
  const res = await b.request("/v1/tasks/cancel-task/cancel", {
    method: "POST",
    headers: { authorization: "Bearer testkey" },
  });
  assert.equal(res.status, 200);

  const t = await store.getTask("cancel-task");
  assert.equal(t?.status, "cancelled", "cancel on instance B must persist status to store");
});
