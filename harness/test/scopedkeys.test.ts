...
test("the deploy path mints the scoped key and never carries the product key", async () => {
  ...
  /**
   * THE PROPERTY, not the variable name.
   *
   * This read `brandKeyFor(req.projectId)` and failed when the review gate split `startDeploy` into
   * propose-then-publish: the CodeBuild call moved into `fireBuild`, which takes the Deployment ROW
   * rather than a request, so the same value is now spelled `dep.project_id`. The test was pinning
   * the spelling of a local variable, and the change it objected to had strengthened the property —
   * a row read back out of the kernel's own database is a narrower source than a function argument.
   *
   * What has to hold is that the key is minted from a project id the kernel owns. So: minted at all,
   * and minted from something whose name says it came from us. A literal, a header, a field off the
   * tarball, or anything reached through `body`/`input` would all fail this.
   */
  const mint = deploy.match(/MYCEL_BRAND_KEY: brandKeyFor\(([^)]*)\)/);
  assert.ok(mint, "deploy.ts must mint the scoped brand key");
  assert.match(
    mint![1],
    /^(dep\.project_id|req\.projectId)$/,
    `the brand key must be derived from the kernel's own project id, not from ${mint![1]}`,
  );
