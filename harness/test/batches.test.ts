...
import { api, makeApp, waitTask, jsonBody } from "./helpers";
...
test("batches: agent opens a batch over the internal endpoint; children join the parent", async () => {
  ...
  /**
   * Join is async in the orchestrator's `finally`, and the parent now takes an extra hop:
   * `awaiting_batch` -> `queued` -> `running` -> terminal. It used to go straight to `succeeded`,
   * which is exactly the bug — the parent was declared finished with its plan half executed and its
   * deliverable never written. So this waits for a TERMINAL state rather than for "no longer
   * parked", and asserts the parent was actually put back to work.
   */
  const TERMINAL = new Set(["succeeded", "failed", "rejected", "expired", "cancelled"]);
  const start = Date.now();
  let parent = await store.getTask(parentId);
  while (parent && !TERMINAL.has(parent.status) && Date.now() - start < 15000) {
    await new Promise((r) => setTimeout(r, 20));
    parent = await store.getTask(parentId);
  }
  const events = await store.eventsAfter(parentId, 0);
  assert.equal(parent?.status, "succeeded", `parent stuck as ${parent?.status}; events: ${events.map((e) => e.type).join(" > ")}`);
  assert.ok(
    events.some((e) => e.type === "batch.resumed"),
    "the parent must be re-run after its children join, not marked finished where it parked",
  );
