...
test("records: writes are idempotent on the natural key — never double-post", async () => {
  ...
  const first = await api(app, "records", { method: "POST", body: body({ amount_pence: 12000, status: "unmatched" }) });
  ...
  assert.equal(second.json.data.amount_pence, 12000, "merge, not truncate");
...
test("records: the agent batches an ingest and queries it back, scoped to its wedge", async () => {
  ...
  const records = Array.from({ length: 300 }, (_, i) => ({
    ...
    key: `tx_${i}`,
    data: { amount_pence: (i + 1) * 10, status: i < 7 ? "unmatched" : "matched" },
  }));
