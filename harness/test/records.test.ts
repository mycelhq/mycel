...
test("records: the agent batches an ingest and queries it back, scoped to its wedge", async () => {
  ...
  const up = await jsonBody(await app.request("/v1/internal/records/upsert", { method: "POST", headers: H, body: JSON.stringify({ records }) }));
  assert.equal(up.ok, true);
  assert.equal(up.count, 300);

  const q = await jsonBody(await app.request("/v1/internal/records/query", { method: "POST", headers: H, body: JSON.stringify({ collection: "transactions", where: { status: "unmatched" } }) }));
  ...
  const byCase = await jsonBody(await app.request("/v1/internal/records/query", { method: "POST", headers: H, body: JSON.stringify({ case_id: kase.id, limit: 5 }) }));
