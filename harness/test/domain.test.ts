...
test("knowledge: add, list, and feedback becomes a grounding example", async () => {
  const { app } = makeApp();
  const add = await api(app, "wedges/books-keeper/knowledge", { method: "POST", body: JSON.stringify({ name: "pricing.md", content: "$1450/mo", kind: "fact" }) });
  assert.equal(add.status, 201);
  const list = await api(app, "wedges/books-keeper/knowledge");
  assert.ok(list.json.some((k: any) => k.name === "pricing.md"));

  // a task, then feedback with a correction → new knowledge item
  const t = (await api(app, "tasks", { method: "POST", body: JSON.stringify({ wedge: "books-keeper", task_type: "daily_sync", input: {} }) })).json;
  const fb = await api(app, `tasks/${t.id}/feedback`, { method: "POST", body: JSON.stringify({ rating: "bad", correction: "always name the period the figure covers" }) });
  assert.equal(fb.json.ok, true);
  assert.ok(fb.json.knowledge_id, "correction stored as knowledge");
  const after = await api(app, "wedges/books-keeper/knowledge");
...
test("schedules: pausing a schedule must not erase the rest of it", async () => {
  ...
  assert.equal(paused.task_type, "daily_sync");
