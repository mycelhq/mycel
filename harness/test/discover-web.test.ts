...
import { buildFilters, looksLikeJobTitle, mapPerson, webDiscoverPeople } from "../src/gtm/discover-web";
...
describe("FullEnrich web discovery", () => {
  it("buildFilters maps the loose composer input to FullEnrich filters", () => {
    ...
    // title + a short query merge into one titles field, deduped
  ...
  it("a sentence about who they sell to is not a job title", () => {
    assert.equal(looksLikeJobTitle("independent bakeries who need a new site"), false);
    assert.equal(looksLikeJobTitle("Founder"), true);
    assert.equal(looksLikeJobTitle("Co-Founder"), true);
    const f = buildFilters({
      query: "independent bakeries who need a new site",
      title: "Founder",
      location: "Bristol",
    });
    assert.deepEqual(f.current_position_titles, [{ value: "Founder" }]);
    assert.equal("current_company_names" in f, false);
  });

  it("cheap-found companies constrain the paid people-search", () => {
    const f = buildFilters({ title: "Owner", companies: ["Hart's Bakery", "Mark's Bread"] });
    assert.deepEqual(f.current_company_names, [{ value: "Hart's Bakery" }, { value: "Mark's Bread" }]);
  });
