      ...
      Invoice: [
        {
          ...
          CustomerRef: { name: "Brightline Joinery" },
          TotalAmt: 1500.5,
          Balance: 500.25,
        },
      ],
  ...
  assert.deepEqual(items, [
    {
      external_id: "130",
      number: "1037",
      client_name: "Brightline Joinery",
      total_minor: 150_050,
...
test("capabilities: every provider that claims a read declares one this kernel can run", () => {
  ...
  //
  // A read is a tool SLUG or an API PATH — Xero's toolkit is four tools and none of them reads an
  // invoice, so `read_invoices` on Xero goes at `/api.xro/2.0/Invoices` through the proxy. What a
  // path GRANTS is the capability, not the URL: the founder consented to us reading their invoices,
  // and a grant tied to a path would silently lapse the day Xero versions their API.
  for (const capability of ALL_CAPABILITIES) {
    for (const p of capabilityProviders(capability)) {
      const slugs = readToolsFor(capability, p.toolkit);
      assert.deepEqual(
        slugs,
        (p.reads ?? []).map((r) => r.slug ?? capability),
        `${capability}/${p.toolkit}`,
      );
      // Every read is one door or the other, never neither — a read with no slug and no path is a
      // provider that looks readable and cannot be read.
      for (const r of p.reads ?? []) {
        assert.ok(r.slug || r.path, `${capability}/${p.toolkit}: a read declares neither tool nor path`);
        assert.ok(!(r.slug && r.path), `${capability}/${p.toolkit}: a read declares both`);
      }
