...
test("harness: the prompt differs by task type too", () => {
  ...
  // Strict output is enforced by opencode's own `format: json_schema` (which retries), so restating
  // the schema in prose only spends tokens telling the model what it is already constrained by.
  assert.ok(!pDecide.includes("conforming to this schema"), "the schema is not also inlined");
...
test("harness: a task type may narrow which of the wedge's procedures it sees", () => {
  ...
  const wedge: LoadedWedge = {
    ...
    skills: [
      ...
      { name: "file-vat.md", content: "---\ndescription: how we file VAT\n---\nbody" },
    ],
  };
  ...
  assert.ok(!md.includes("skills/file-vat.md"), "a task type only sees the procedures it needs");
});
