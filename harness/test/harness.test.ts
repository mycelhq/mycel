...
import { buildAgentsMdForTest, buildPromptForTest, capabilityConnections } from "../src/runtime";
...
function fakeWedge(manifest: Record<string, unknown>): LoadedWedge {
  return { manifest: manifest as never, dir: "/tmp", skills: [], knowledge: [], exemplars: [] };
}
...
test("harness: a decide profile keeps the action proxy and loses the filesystem", () => {
  ...
  // `webfetch`/`websearch` ARE allowed on decide: it runs every fulfillment wedge, not just dunning,
  // and denying the web tool bought no safety while `bash` (curl) stayed open. See the profile note.
  assert.equal(toolEnabled(profile.permission, "webfetch"), true, "fulfillment can research the open web");
  assert.equal(toolEnabled(profile.permission, "websearch"), true);
...
test("draft_shape has the budget its production failures say it needs", () => {
  // Of the draft_shape runs that did not succeed in production, NINE of nineteen died on
  // `max_runtime_exceeded` — the largest single cause, ahead of every model and schema failure
  // combined. The step that decides whether the product understands somebody's business was being
  // cut off mid-answer about half the time it struggled.
  //
  // Pinned because the number has a twin: `cloud/app/onboarding/shape-working.tsx` HARD_MS mirrors
  // it, and a panel that gives up before the kernel throws away a run that was about to succeed.
  const wedge = loadWedge("business-shaper");
  assert.ok(wedge);
  const profile = resolveHarnessProfile({
    task: taskOf("business-shaper", "draft_shape"),
    wedge,
    ceilings: CEILINGS,
  });
  assert.equal(profile.max_runtime_s, 720, "keep in step with HARD_MS in shape-working.tsx");
  assert.ok(profile.steps !== undefined && profile.steps <= 12, "the first screen answers, it does not wander");
  assert.equal(toolEnabled(profile.permission, "webfetch"), false, "a good answer in twenty seconds — no site crawl");
  assert.equal(toolEnabled(profile.permission, "websearch"), false);
});

test("a capability whose data is in the task input is not missing", () => {
  /**
   ...
   * shape of every migration: a founder who exports a year of ledgers should not be told to connect
   * a bank feed before we will read them.
   */
  const caps = ["read_bank_transactions"];
  const withoutInput = capabilityConnections(caps, [], "p1");
  assert.equal(withoutInput.missing.length, 1, "no connection and no data is a genuine gap");

  const withInput = capabilityConnections(caps, [], "p1", ["read_bank_transactions"]);
  assert.deepEqual(withInput.missing, [], "supplied data settles it");
  assert.deepEqual(withInput.ids, [], "and grants no connection — the run reads the input");
});
