...
test("artifacts: a client sends a document through the portal and work starts", async () => {
  ...
  const fd = new FormData();
  fd.append("file", new File([PNG], "july.png", { type: "image/png" }));
...
test("artifacts: a customer can list their thread's files without waiting for a reply", async () => {
  ...
  const fd = new FormData();
  fd.append("file", new File([PNG], "statement.png", { type: "image/png" }));
...
// ── the file the ask is actually waiting for ─────────────────────────────────────────────────────
//
// The thread door next to this one needs a `thread_id`, a thread needs a channel, and one project
// of twenty-one has one — so `thread_id` is null on every ask the product has ever written. Since
// `blocked.tsx` renders its file input only when `thread_id` is set, all twenty-eight open
// `document` asks showed a client the question, a textarea and a Send button, and no way to attach
// the one thing being requested. Keyed on the REQUEST, which always exists, instead.

async function anAsk(
  app: ReturnType<typeof makeApp>["app"],
  store: ReturnType<typeof makeApp>["store"],
  opts: { taskId?: string | null; clientId: string; projectId: string; status?: "open" | "resolved" } = {} as never,
) {
  return getRequestStore().createRequest({
    ...
    ...(opts.status && opts.status !== "open" ? { status: opts.status } : {}),
  } as never);
}

test("portal: a client can send the file an ask is waiting for, with no thread anywhere", async () => {
  const { app, store } = makeApp();
  /**
   * TWO runs, and the ask belongs to the SECOND.
   *
   * With one task in the world, "landed on the run that raised the ask" and "landed on whatever
   * task exists" are the same sentence, and the assertion below cannot fail. A decoy makes the
   * claim falsifiable — which is the only reason it is worth asserting.
   */
  await aTask(app, store);
  const { id: taskId, projectId } = await aTask(app, store);
  const domain = getDomainStore();
  const clientRow = await domain.createClient({
    project_id: projectId, display_name: "Acme", handles: ["acme@x.test"], metadata: {},
  });
  const ask = await anAsk(app, store, { taskId, clientId: clientRow.id, projectId });
  const session = (await exchangePortalLink(
    mintPortalLink({ project_id: projectId, client_id: clientRow.id }).token,
  ))!.token;

  const res = await app.request(`/v1/portal/requests/${ask.id}/attachments`, {
    method: "POST",
    headers: { authorization: `Bearer ${session}` },
    body: upload("july.png", "image/png", PNG),
  });
  assert.equal(res.status, 201, await res.clone().text());
  const art = (await res.json()) as { id: string; task_id: string };
  assert.ok(art.id, "no artifact came back to hand to `respond` as artifact_ids");
  // It lands on the run that raised the ask, because that is where the resuming episode looks.
  assert.equal(art.task_id, taskId, "the file landed somewhere other than the run that asked for it");
});

test("portal: a kickoff ask says why it cannot take a file, rather than losing it", async () => {
  const { app, store } = makeApp();
  const { projectId } = await aTask(app, store);
  const domain = getDomainStore();
  const clientRow = await domain.createClient({
    project_id: projectId, display_name: "Beta", handles: ["beta@x.test"], metadata: {},
  });
  // Raised when the engagement opened, before any run — so there is no task to hang bytes off.
  const ask = await anAsk(app, store, { clientId: clientRow.id, projectId });
  const session = (await exchangePortalLink(
    mintPortalLink({ project_id: projectId, client_id: clientRow.id }).token,
  ))!.token;
  ...
  const res = await app.request(`/v1/portal/requests/${ask.id}/attachments`, {
    method: "POST",
    headers: { authorization: `Bearer ${session}` },
    body: upload("x.png", "image/png", PNG),
  });
  assert.equal(res.status, 409, "a file with nowhere to belong was accepted anyway");
  assert.match((await res.json()).error, /type your answer/i, "the refusal does not say what to do instead");
});

test("portal: one customer cannot send a file into another customer's ask", async () => {
  const { app, store } = makeApp();
  const { id: taskId, projectId } = await aTask(app, store);
  const domain = getDomainStore();
  const mine = await domain.createClient({ project_id: projectId, display_name: "Mine", handles: ["m@x.test"], metadata: {} });
  const theirs = await domain.createClient({ project_id: projectId, display_name: "Theirs", handles: ["t@x.test"], metadata: {} });
  const theirAsk = await anAsk(app, store, { taskId, clientId: theirs.id, projectId });
  const session = (await exchangePortalLink(
    mintPortalLink({ project_id: projectId, client_id: mine.id }).token,
  ))!.token;

  const res = await app.request(`/v1/portal/requests/${theirAsk.id}/attachments`, {
    method: "POST",
    headers: { authorization: `Bearer ${session}` },
    body: upload("nosy.png", "image/png", PNG),
  });
  assert.equal(res.status, 404, "a customer wrote into another customer's ask");
});
