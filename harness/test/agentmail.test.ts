...
test("an unsigned webhook is refused: a missing header must not read as 'nothing to check'", async () => {
  ...
  assert.equal((await jsonBody(res)).reason, "missing_headers");
});
...
test("spam and unauthenticated senders never become messages from the client", async () => {
  ...
  for (const eventType of ["message.received.spam", "message.received.blocked", "message.received.unauthenticated"]) {
...
test("a bounce is no longer ignored — it is HANDLED, which is a different 200", async () => {
  // `message.bounced` used to be in the loop above, acknowledged with `{ ok: true, ignored }`. So
  // this kernel saw every bounce against every agency running on it and learned nothing from any of
  // them: the address stayed on the list and the next sequence step mailed it again, which is the
  // one behaviour that feeds the bounce rate a sending domain gets blocked over.
  //
  // The old assertion still holds and matters — a bounce must never become a message from the client
  // or start a run. What is new is that something happens instead of nothing.
  const { app } = await makeFreshApp();
  const projectId = await firstProject(app);
  const { inboxId } = await mailbox(projectId);
  ...
  const res = await deliver(app, {
    ...
    message: { inbox_id: inboxId, to: "gone@harlow.co.uk", bounce_type: "Permanent", reason: "550 5.1.1 no such user" },
  });
  assert.equal(res.status, 200);
  assert.equal(res.json.ok, true);
  assert.equal(res.json.task_id, undefined, "a bounce must not start a run");
  assert.equal(res.json.ignored, undefined, "and it is not ignored either");
  assert.equal(res.json.delivery.suppressed, true, "the address is on this business's list now");
  assert.match(res.json.delivery.detail, /does not exist/);

  // A transient bounce is handled and suppresses nobody: the address is still real and the person
  // behind it did nothing wrong.
  const soft = await deliver(app, {
    event_type: "message.bounced",
    message: { inbox_id: inboxId, to: "busy@harlow.co.uk", reason: "452 4.2.2 mailbox full" },
  });
  assert.equal(soft.json.delivery.suppressed, false);
  assert.match(soft.json.delivery.detail, /still real/);
});
