...
test("a run is told the facts the firm already holds about the client", async () => {
  // `Client.preferences` has said, in the contract, since it was written: "operational settings
  // (tone, timezone, billing day) — the agent should be able to read one without a search." Nothing
  // on the run path had ever fetched the client, so it could not read one at all.
  //
  // A close ran nine times and told the same client nine times that their VAT scheme was unknown and
  // their deadlines could not be stated, while the platform held a row saying otherwise. A firm that
  // asks the same question every month is not one anybody keeps.
  ...
  const md = buildAgentsMdForTest(
    task, null, [], undefined, undefined, undefined, undefined, undefined, [], undefined,
    {
      display_name: "Harlow & Finch",
      metadata: { vat_scheme: "flat rate, 12%" },
      preferences: { vat_return_period: "quarterly, ending 30 September", tone: "plain" },
    },
  );
  assert.match(md, /Harlow & Finch/);
  assert.match(md, /flat rate, 12%/, "the scheme it kept saying was unknown");
  assert.match(md, /quarterly, ending 30 September/);
  assert.match(md, /already know about this client/);

  // A client with nothing recorded prints nothing. "No preferences recorded" reads as a finding.
  const bare = buildAgentsMdForTest(
    task, null, [], undefined, undefined, undefined, undefined, undefined, [], undefined,
    { metadata: {}, preferences: {} },
  );
  assert.equal(/already know about this client/.test(bare), false);
});
