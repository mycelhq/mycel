...
test("geo-monitor names weekly_report as production, not the first probe", () => {
  const spec = fulfillmentOf("geo-monitor");
  assert.equal(spec?.production_task_type, "weekly_report");
  assert.ok(spec?.deliverable_shapes?.includes("document"));
  assert.ok(spec?.deliverable_shapes?.includes("link"));
});
