...
test("billing.pg: the list filter binds project_id and never interpolates", async () => {
  // `project_id` is required now, so there is no filter shape that produces an empty WHERE. The
  // clause is always emitted — which is the point: a query with no tenant clause cannot be built.
  assert.deepEqual(invoiceWhere({ project_id: "proj-a" }), { clause: "WHERE project_id=$1", vals: ["proj-a"] });
