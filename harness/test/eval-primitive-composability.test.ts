...
test("EVAL: the shared library is a CLOSED vocabulary — an unknown primitive is refused, never pointed at absent code", () => {
  ...
  assert.deepEqual([...SHARED_WORKFLOWS].sort(), ["close-figures", "content-calendar", "crm-hygiene", "dunning-ladder", "lookalike", "next-touch", "reconcile", "schedule", "signal-backtest", "signal-score", "win-patterns"]);

  // THE SAME REFUSAL FOR A SHIPPED WEDGE, which it did not have. `authoredFaults` has always checked
  // this; `manifestFaults` had not, so books-keeper could declare `lib: "close-figures"` before the
  // name existed, pass every gate, and fail mid-close after spending a sandbox — then do the
  // arithmetic by hand, which is the exact thing the workflow existed to prevent.
  const shipped = manifestFaults("books-keeper", {
    wedge: "books-keeper",
    title: "Books",
    task_types: {},
    workflows: [{ name: "x", lib: "make-money-fast" }],
  });
  assert.ok(shipped.some((f) => /make-money-fast/.test(f.message)));
});
