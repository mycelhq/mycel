...
test("a result is matched back by the round-tripped key, never by array position", async () => {
  ...
  await withKey({ [FULLENRICH_KEY_ENV]: "test-key", FULLENRICH_POLL_MS: "0", FULLENRICH_BASE_URL: "https://fullenrich.test/api/v2" }, async () => {
    ...
    try {
      ...
      const ruiEmail = (rui.provenance as { email?: { attempts?: Array<{ by?: string; credits?: number }> } }).email;
      const danaEmail = (dana.provenance as { email?: { attempts?: Array<{ by?: string; credits?: number }> } }).email;
      const hopCredits = (p?: { attempts?: Array<{ by?: string; credits?: number }> }) =>
        (p?.attempts ?? []).filter((a) => a.by === FULLENRICH_RESOLVER).reduce((s, a) => s + (a.credits ?? 0), 0);
      assert.equal(hopCredits(ruiEmail) + hopCredits(danaEmail), 4, "batch credits must not be billed once per face");
...
// ── PORTRAIT CAPTURE ───────────────────────────────────────────────────────────────────────────
// A real run (prod, 2026-08) filed nine FullEnrich-discovered people with photo_url:null — the
// person's face was in the provider row but nothing read it, so the CRM showed company logos where
// faces belong. These guard that the field is mapped from wherever FullEnrich puts it.
test("photoFromRow extracts a portrait from the top-level provider field", () => {
  assert.equal(
    photoFromRow({ profile_picture_url: "https://media.licdn.com/dms/image/abc.jpg" }),
    "https://media.licdn.com/dms/image/abc.jpg",
  );
  assert.equal(photoFromRow({ photo_url: "https://cdn.example.com/p.png" }), "https://cdn.example.com/p.png");
});

test("photoFromRow reads the professional-network social profile too", () => {
  const social = { url: "https://linkedin.com/in/x", picture: "https://media.licdn.com/dms/image/soc.jpg" };
  assert.equal(photoFromRow({}, social), "https://media.licdn.com/dms/image/soc.jpg");
});

test("photoFromRow drops non-https (an http or missing portrait is no portrait — it lands in an img src)", () => {
  assert.equal(photoFromRow({ photo_url: "http://insecure.example/p.jpg" }), undefined);
  assert.equal(photoFromRow({}), undefined);
});

test("parseRichProfile surfaces the portrait alongside role and company", () => {
  const rich = parseRichProfile({
    headline: "Head of Finance",
    profile_picture_url: "https://media.licdn.com/dms/image/rich.jpg",
    employment: { current: { title: "CFO", company: { name: "Acme", domain: "acme.com" } } },
  });
