...
async function businessWithOneDebt(amount = 1_400_00) {
  ...
  const client = await makeClient(project, "brightline");
...
test("a question and a request that differ by one verb are routed to different places", async () => {
  ...
  assert.equal(heuristicIntent("please send Brightline a reminder").kind, "request");
  // A polite order is still an order. "can you chase them" opens with a question word and is work.
  assert.equal(heuristicIntent("can you chase Brightline").kind, "request");
});
...
test("an unreachable classifier routes on the heuristic rather than failing the turn", async () => {
  ...
  assert.equal((await classifyIntent("chase Brightline", { classify: explodes })).kind, "request");
...
test("an attachment becomes context for the turn and never a knowledge row", async () => {
  ...
  const r = await chat(
    ...
    auths,
    {
      ...
      attachments: [{ name: "brightline-statement.pdf", text: "Balance carried forward 4,200.00 USD. Overdue since March." }],
    },
  ...
  assert.ok(
    r.answer.cited.some((c) => c.source === "attachment" && c.label === "brightline-statement.pdf"),
