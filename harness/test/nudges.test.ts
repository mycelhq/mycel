...
async function engagement(projectId: string, wedge = WEDGE) {
  /**
   * A MAILBOX, because a reminder with nowhere to go no longer gets a run.
   *
   * `startNudge` refuses `no_channel` before the pacing ladder — see the note there. Without this
   * every test below would exercise that refusal instead of the claim, the ladder and the budget it
   * is actually about, and would go green for the wrong reason on the day the ladder broke.
   */
 ...
 * ═════════════════════════════════════════════════════════════════════════════════════════════════
 *
 * MEASURED IN PRODUCTION at 22:30 tonight: the sweep spawned SEVEN `nudge_client_request` runs.
 * Each booted a Daytona sandbox, was told by `compile` "no mailbox is connected to this business,
 * so nothing can be emailed to a client", wrote a perfectly good reminder anyway, correctly
 * answered `channel: "none"`, and reported success. Seven sandboxes, nothing sent, four times a day.
 *
 * `compile`'s refusal is advisory, and that is RIGHT for a job that produces something a founder can
 * still use — a report is worth writing even if it has to be delivered by hand. A chase is not: its
 * only artefact is the sending.
 */
/** `engagement()` without the mailbox. The three tests below are about exactly that difference. */
async function engagementWithNoMailbox(projectId: string) {
  const client = await getDomainStore().createClient({
    project_id: projectId, display_name: "Acme", handles: ["a@x.co"], metadata: {},
  });
  const kase = await getDomainStore().createCase({
    project_id: projectId, wedge: WEDGE, title: "March close", client_id: client.id,
    stage: "gathering", status: "open", data: {},
  });
  return { client, kase };
}

test("a chase is not started when the business has no mailbox connected", async () => {
  const pid = `p-${randomUUID()}`;
  const deps = recordingDeps();
  try {
    // Deliberately NOT using `engagement()` — that helper connects a mailbox precisely because
    // every other test in this file needs one.
    const { client, kase } = await engagementWithNoMailbox(pid);
    const r = await ask(pid, { client_id: client.id, case_id: kase.id });

    const out = await startNudge(getDomainStore(), r, { pacing: "ladder" });
    assert.equal(out.ok, false);
    assert.equal((out as { reason: string }).reason, "no_channel");
    assert.match((out as { message: string }).message, /no mailbox is connected/i);
    assert.equal(deps.spawned.length, 0, "a sandbox was booted to write a message that cannot leave");
  } finally {
    deps.restore();
  }
});

test("...and it does not spend one of the client's reminders finding that out", async () => {
  // Checked BEFORE the pacing ladder on purpose. Burning a nudge on an unsendable reminder would
  // exhaust the client's patience against messages they never received, and the ask would run out
  // of budget while still open.
  const pid = `p-${randomUUID()}`;
  const deps = recordingDeps();
  try {
    const { client, kase } = await engagementWithNoMailbox(pid);
    const r = await ask(pid, { client_id: client.id, case_id: kase.id });
    await startNudge(getDomainStore(), r, { pacing: "ladder" });

    const fresh = await getRequestStore().getRequest(pid, r.id);
    assert.equal(fresh?.nudge_count ?? 0, 0, "the claim was spent on a reminder that never went");
    assert.equal(fresh?.last_nudged_at, undefined, "the ladder was advanced by a send that did not happen");
    assert.equal(fresh?.status, "open", "the ask stopped being open without being answered");
  } finally {
    deps.restore();
  }
});

test("connect a mailbox and the same ask sends on the next sweep", async () => {
  const pid = `p-${randomUUID()}`;
  const deps = recordingDeps();
  try {
    const { client, kase } = await engagementWithNoMailbox(pid);
    const r = await ask(pid, { client_id: client.id, case_id: kase.id });
    assert.equal((await startNudge(getDomainStore(), r, { pacing: "ladder" })).ok, false);

    await getDomainStore().createConnection({
      project_id: pid, kind: "email", name: "mailbox", owner: { kind: "founder", id: "f" },
      config: { from: "hello@practice.test", api_url: "https://api.postmark.test/email" }, secret_ref: "env:MAIL",
    });

    /**
     * `override` — what a founder pressing "chase now" does. On `ladder` this same call correctly
     * answers `too_soon`, because the ask was raised seconds ago, and that IS the proof the ordering
     * is right: `no_channel` is checked first, and once a mailbox exists the pacing ladder is what
     * decides. Asserting through `override` isolates the gate from the clock.
     */
    const out = await startNudge(getDomainStore(), r, { pacing: "override" });
    assert.equal(out.ok, true, `still refused after connecting: ${JSON.stringify(out)}`);
