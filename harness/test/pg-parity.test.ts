...
test("parity: listInvoices scopes identically in both backends", async () => {
  ...
  /**
   * Two of these are TYPE-ILLEGAL ON PURPOSE. `InvoiceFilter.project_id` is required precisely so a
   * caller cannot forget the tenant, and the point of this parity sweep is that both stores fail
   * CLOSED when one arrives without it anyway — through JavaScript, from an untyped boundary, or
   * from a future refactor. Casting them here keeps the type strict while still exercising the case
   * it exists to make impossible; deleting them would remove the only coverage of the leak this
   * table's required field was added to prevent.
   */
  const filters: InvoiceFilter[] = [
    ...
    { project_id: "nobody" },
    { project_id: "" },
    { client_id: "cli-1" } as InvoiceFilter,
  ];
