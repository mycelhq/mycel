...
test("blueprints: provisioning creates a whole business and reports what's missing", async () => {
  ...
  /**
   * The crux: BLUEPRINT schedules must not be live yet, or a credential-less business starts failing
   * on a timer.
   *
   * This used to filter out three self-improvement schedules — `reflect_memory`, `review_work` and
   * `review_artifacts` — that a project carried alongside the blueprint's own. 59f1dd83 deleted that
   * whole system on 6 September, having measured it: 261 sandbox-hours, four proposals, none
   * adopted, and not one of the 22 rules in production traceable to it.
   *
   * The filter outlived it and became a no-op, which is worse than untidy in two ways. It quietly
   * weakened this assertion — with nothing to remove, "the blueprint's three" and "all of them"
   * stopped being different claims, so the test could no longer tell a stray schedule from none.
   * And the comment read as a live description of a system that no longer exists: it cost a later
   * session a full investigation into why those three had "stopped running", down to the point of
   * nearly re-provisioning them. That deletion's own commit message warned about exactly this —
   * leaving the plumbing behind is the half-measure that produces the drift.
   */
  const scheds = (await api(app, "schedules")).json as { name: string; enabled: boolean }[];
  assert.equal(scheds.length, 3, "a project should carry the blueprint's three schedules and nothing else");
  assert.ok(scheds.every((s) => s.enabled === false), "provisioned disabled");
