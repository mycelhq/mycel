...
test("the summary says which arm won, what a conversion is, and what to do", async () => {
  ...
  /**
   * A SENTENCE, not a slug. This line used to read `assert.match(metric, /client portal/)`, which
   * restated the constant asserted one line above and pinned the wording of a definition that was
   * always going to change — and did, when the template's call to action stopped pointing at the
   * sign-in page. That version failed for the right reason at the wrong place: the definition moving
   * is the event this test should survive, not the one it should report.
   *
   * What has to hold is that the number arrives with a description a model can read. An event name
   * (`$convert:outcome`) in this field would be a definition only this codebase can decode.
   */
  assert.ok(s.experiment!.metric.includes(" "), "the metric must be prose, not an event name");
  assert.doesNotMatch(s.experiment!.metric, /\$|_/, "no raw event token in a described metric");
