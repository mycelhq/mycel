...
test("act: calendar normalisers keep Meet / Zoom / Teams join links on the event", () => {
  const g = normaliseGoogleCalendarEvents({
    items: [
      {
        id: "m1",
        ...
        end: { dateTime: "2026-03-05T14:30:00Z" },
      },
      {
        id: "m2",
        summary: "Zoom sync",
        ...
        start: { dateTime: "2026-03-05T15:00:00Z" },
        end: { dateTime: "2026-03-05T15:30:00Z" },
      },
    ],
  });
  assert.equal(g.items.find((e) => e.external_id === "m1")?.meeting_url, "https://meet.google.com/abc-defg-hij");
  assert.equal(g.items.find((e) => e.external_id === "m2")?.meeting_url, "https://zoom.us/j/123456789");

  const o = normaliseOutlookEvents({
    value: [
      {
        id: "t1",
        subject: "Teams with Acme",
        onlineMeeting: { joinUrl: "https://teams.microsoft.com/l/meetup-join/19%3ameeting" },
        start: { dateTime: "2026-03-05T16:00:00.0000000", timeZone: "UTC" },
        end: { dateTime: "2026-03-05T16:30:00.0000000", timeZone: "UTC" },
      },
    ],
  });
  assert.match(o.items[0]!.meeting_url ?? "", /teams\.microsoft\.com/);
});
