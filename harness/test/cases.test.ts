...
test("cases: the agent can read and advance its own case, scoped to its run", async () => {
  ...
  const read = await jsonBody(await app.request("/v1/internal/case", { headers: H }));
  assert.equal(read.ok, true);
  assert.equal(read.case.id, kase.id);
  assert.ok(read.case.stages.length, "the agent is told which stages exist");

  const upd = await jsonBody(await app.request("/v1/internal/case/update", { method: "POST", headers: H, body: JSON.stringify({ stage: "collecting", data: { receipts_missing: 3 }, note: "3 receipts outstanding" }) }));
  ...
  const badStage = await jsonBody(await app.request("/v1/internal/case/update", { method: "POST", headers: H, body: JSON.stringify({ stage: "invented" }) }));
...
test("cases: a case can be linked to a client after it was opened, once", async () => {
  ...
  const client = (await api(app, "clients", { method: "POST", body: JSON.stringify({ display_name: "Brightline" }) })).json;
