...
test("pacing: a connected free account gets the documented dogfood pace, not 1 invite a day", () => {
  ...
  assert.equal(invites, 100, `free invites/week: ${invites}`);
  ...
  assert.equal(premiumMessages, 350, "Premium messages 50/day × 7 — Premium does not buy invite volume");
  assert.equal(premiumInvites, invites, "Premium invite ceiling matches free (2026 consensus)");
...
test("pacing: it never exceeds the published ceiling, even at its best", () => {
  // Reply bonus can reach 1.15 — cap at the tier ceiling so a free account never inherits Sales Nav.
  const best = budgetFor("invite", at({ engagement: { sent: 500, accepted: 300, replied: 100, flagged: 0 } }));
  assert.equal(best, 100, `free tier ceiling is 100/week; budget was ${best}`);
});
...
test("pacing: a connection with no account_age_days is treated as established", async () => {
  ...
  assert.equal(v.budget, 100);
  assert.equal(v.remaining, 100);
  const msg = await assertSendAllowed(domain, "c1", "message");
  assert.equal(msg.budget, 175);
});

test("pacing: invite spread refuses dumping the weekly budget on day one", () => {
  // Fresh window: one safe day only (~20 for a 100/week ceiling).
  assert.equal(inviteSpreadCap(100, undefined), 20);
  const day1 = evaluate(
    "invite",
    at({ used: { invite: 20 }, windowStartedAt: { invite: new Date().toISOString() } }),
  );
  assert.equal(day1.allowed, false);
  assert.match(day1.reason!, /spreading invitations/i);

  const early = evaluate(
    "invite",
    at({ used: { invite: 5 }, windowStartedAt: { invite: new Date().toISOString() } }),
  );
