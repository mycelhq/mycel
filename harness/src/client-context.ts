 ...
 * one JSON response.
 */
import type { Artifact, Case, Client, Invoice, KnowledgeItem, Message, Thread } from "./contract";
...
export interface ClientContext {
  ...
  /**
   * Invoices for this client — money belongs on the same timeline as outreach and delivery, or
   * chase feels like a separate app from the relationship.
   */
  invoices: Invoice[];
}
...
export async function getClientContext(
  ...
  //
  // `?? ""` is a fail-closed sentinel, not a formality: `Client.project_id` is optional, and while
  // this filter was optional too, an unscoped client read EVERY tenant's cases carrying a matching
  // client_id. Making `CaseFilter.project_id` required is what surfaced that — no row has a
  // project_id of "", so an unattributed client now assembles a context with no cases in it.
  ...
  const invoices =
    client.project_id
      ? await getBillingStore().listInvoices({
  ...
  return { client, threads: threadsWithMessages, cases, deliverables, knowledge, invoices };
}
