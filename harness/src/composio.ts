 ...
 */
/**
 * ═══ THE VENDOR'S OWN API, THROUGH THE BROKER'S OAUTH ═══
 *
 * Composio's Xero toolkit is four tools: `GET_CONNECTIONS`, `GET_CONTACTS`, `GET_QUOTES` and
 * `POST_INVOICE_UPDATE`. Its own metadata advertises fifty-three; the API returns four, one page,
 * no cursor. Not one of them reads a bank transaction, an invoice list or a report — which is to
 * say, not one of them is any use to a bookkeeper. The picture is the same next door: QuickBooks
 * exposes thirteen tools of which two are balance reports, FreshBooks two, and Wave, Zoho Books and
 * Sage none at all.
 *
 * So the brokered-tool path cannot close a month, and waiting for a toolkit to grow is not a plan.
 *
 * `/tools/execute/proxy` is the way through. Composio keeps doing the part that is genuinely hard
 * and genuinely theirs — the OAuth dance, the refresh, the per-tenant token we must never hold —
 * and we address the vendor's real REST API. All of Xero rather than four of it:
 * `/api.xro/2.0/BankTransactions`, `/api.xro/2.0/Invoices`, `/api.xro/2.0/Reports/BalanceSheet`.
 * `capabilities.normalise.ts` already parses those exact response shapes.
 *
 * WHAT THIS DOES NOT CHANGE. The token still never enters this process, the connection is still
 * resolved per project by `resolveCapability`, and reads are still refused unless the connection
 * lists the path — see `isReadTool`. A proxy is a different way to phrase the request, not a wider
 * grant, and anything that reaches a person outside the business still stops at a human.
 */
export async function proxyRequest(
  cfg: ComposioConfig,
  args: {
    connectedAccountId: string;
    /** Relative to the vendor's base URL, which Composio resolves from the toolkit. */
    endpoint: string;
    method: "GET" | "POST" | "PUT" | "PATCH" | "DELETE";
    /** Query string parameters, as `[{name, value}]` — the shape the proxy takes. */
    parameters?: { name: string; value: string; in?: "query" | "header" }[];
    body?: unknown;
  },
): Promise<ExecuteResult> {
  const body = await call<{ status?: number; data?: unknown; error?: string; log_id?: string }>(
    cfg,
    `/tools/execute/proxy`,
    {
      method: "POST",
      body: JSON.stringify({
        ...
        method: args.method,
        ...(args.parameters?.length ? { parameters: args.parameters } : {}),
        ...(args.body === undefined ? {} : { body: args.body }),
      }),
      timeoutMs: 60_000,
    },
  );
  // The proxy answers with the vendor's HTTP status rather than a `successful` flag, so success is
  // 2xx and nothing else. Treating a 401 as data is how an expired token becomes an empty month.
  const status = Number(body.status ?? 0);
  return {
    successful: status >= 200 && status < 300,
    data: body.data,
    error: body.error || (status && (status < 200 || status >= 300) ? `the provider answered ${status}` : undefined),
    log_id: body.log_id,
  };
}
