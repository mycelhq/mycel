...
function hasRefusal(spec: TaskSpec, checks: ShipCheck[]): boolean {
  ...
  for (const prop of Object.values(spec.output_schema?.properties ?? {})) {
    const en = prop?.enum;
...
function clientFacing(m: WedgeManifest): Array<[string, TaskSpec]> {
  ...
  return Object.entries((m.task_types ?? {}) as Record<string, TaskSpec>).filter(([name, spec]) => {
    if (spec.internal || isOperationalTaskType(name) || RESPONSIVE_TASKS.has(name)) return false;
...
export function gradeDeliverables(
  ...
  if (isAuthoredSlug(m.wedge) && fulfillment) {
    ...
    if (!plan?.lines?.length) {
      add({
        ...
        fix: "Add a deposit and/or a retainer line to the service's money plan.",
        because:
  ...
  if (opts.knownToolkits) {
    for (const conn of m.fulfillment?.client_connections ?? []) {
      ...
      add({
        ...
        severity: "blocking",
        says: `This service asks the client to connect "${toolkit}", which is not an integration that exists.`,
  ...
  for (const [name, spec] of jobs) {
    ...
    if (!required.length && !requires.length) {
      add({
        ...
        severity: "blocking",
        task: name,
        says: `"${name}" is what a client receives, and nothing says the work has to contain anything.`,
      ...
      add({
        ...
        severity: "weak",
        task: name,
        says: `Something must be in what "${name}" produces, and nothing checks whether it is right.`,
    ...
    if (looksNumeric(spec.output_schema) && !checks.some((c) => ARITHMETIC.has(c.kind))) {
      add({
        ...
        severity: "weak",
        task: name,
        says: `"${name}" puts numbers in front of a client and nothing checks them against each other.`,
    ...
    if (lists.length && !checks.some((c) => EVIDENCE.has(c.kind))) {
      add({
        ...
        severity: "weak",
        task: name,
        says: `"${name}" produces a list of ${lists[0]} and nothing makes each entry name where it came from.`,
    ...
    if (!hasRefusal(spec, checks)) {
      add({
        ...
        severity: "weak",
        task: name,
        says: `"${name}" has no way to say it could not do the job, so when it has nothing it will produce something anyway.`,
    ...
    if (checks.length && !checks.some((c) => ARITHMETIC.has(c.kind) || EVIDENCE.has(c.kind))) {
      add({
        ...
        severity: "note",
        task: name,
        says: `The checks on "${name}" are all about how the work reads, not whether it is right.`,
    ...
    if (!spec.input_schema?.properties) {
      add({
        ...
        severity: "note",
        task: name,
        says: `"${name}" does not say what it is given, so a run cannot tell a measurement from a suggestion.`,
  ...
  if (jobs.length && !(m.capabilities ?? []).length && !(m.intake ?? []).length) {
    add({
      ...
      fix: "Declare the capability it needs to reach the client's data, or the questions the founder must answer before it can start.",
      because:
  ...
  return { findings, blocking, weak, verdict: verdictFor(jobs.length, blocking, weak) };
}

function verdictFor(jobs: number, blocking: number, weak: number): string {
  if (blocking) {
  ...
  if (weak >= 3) {
    return "It will produce work, and almost nothing checks whether the work is right. Fine to try on your own business, not on a client who is paying.";
  }
  if (weak) {
