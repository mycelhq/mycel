...
import { redactCalendar } from "./calendar-privacy";
...
export async function syncCalendar(args: { project_id: string; now?: Date }): Promise<CalendarSyncSummary> {
  ...
  /**
   * ═══ THE TITLES ARE NOT OURS UNLESS THE MEETING IS ═══
   *
   * This wrote `data: { ...event }` for every event it read. A founder connects a calendar so we can
   * see when their client call is, and we were storing "Oncology follow-up, 09:30" along with it —
   * and nothing in the product ever read it.
   *
   * `redactCalendar` keeps the TIMES of everything (a booking desk needs them and they disclose
   * nothing) and the WORDS of only what it can attribute: a name already in the book, or a join
   * link we would be asked to sit in. See calendar-privacy.ts for why attribution is by name and
   * URL rather than by attendee.
   *
   * The known names are read fresh on every sync rather than cached, because a client added this
   * morning should make this morning's meeting attributable — and the cost is one list read against
   * a calendar round trip.
   */
  const known = (await deps.domain.listClients().catch(() => []))
    .filter((c) => c.project_id === projectId)
    .map((c) => c.display_name ?? "")
    .filter(Boolean);
  const redaction = redactCalendar(read.items, known);
  ...
  // Busy intervals from the REDACTED set: they only ever used the times, and computing them from the
  // unredacted list would be one more place that touches words we decided not to keep.
  const upcoming = busyIntervals(redaction.events).find((b) => Date.parse(b.to) > now.getTime());
  ...
  return finish({
    ...
    detail:
      base.observed === 0
        ...
        : // SAID OUT LOUD. A founder who connects a calendar and is told "read 48, kept the words of
          // 6" learns exactly what the trade is, in one sentence, from a number we had to compute
          // anyway. Leaving it out would be the same behaviour with the promise left implicit.
          `read ${base.observed} event(s), kept the details of ${redaction.kept}` +
          (redaction.redacted > 0 ? ` and only the times of ${redaction.redacted}` : "") +
