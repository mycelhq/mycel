...
export interface DeploymentQuery {
  project_id: string;
  /**
   * Narrow to ONE site. Absent means every site this tenant owns.
   *
   * Needed the moment an agency could have more than one — before that, "this project's live
   * deployment" was a complete question and now it is an ambiguous one.
   */
  site_id?: string;
...
export interface DomainStore {
  ...
  updateDeployment(
    ...
    projectId: string,
    /**
     * `source_key` is WRITE-ONCE, not freely patchable, and the difference is enforced in the stores
     * rather than by this type (`COALESCE(source_key, $n)` in Postgres, a delete in memory).
     *
     * It used to be absent here entirely, with a note at the call site saying "a deploy's input must
     * not be rewritable". That property is worth keeping and absence was the wrong way to keep it:
     * the key cannot be known before the insert, because it contains the deployment id, so
     * `proposeDeploy` has to write it afterwards. Write-once keeps the original guarantee — nothing
     * can repoint a stored deploy at different bytes after a person has been shown the first set —
     * while allowing the one write that has to happen. A second attempt is silently ignored rather
     * than refused, because the only caller that would make one is a retry of the same proposal.
     */
    patch: Partial<Pick<Deployment, "status" | "build_id" | "url" | "error" | "source_key">>,
  ): Promise<Deployment | undefined>;
  /**
   * Mark every other deployment OF THE SAME SITE superseded, atomically with nothing else.
   *
   * Called when one goes live. Without it a site accumulates rows that all claim to be `live`, and
   * "what is my URL" has several answers — which is how a customer is shown a URL a later deploy
   * replaced. Excludes `keepId` rather than taking a timestamp, because two deploys finishing in the
   * same millisecond would otherwise both survive.
   *
   * SCOPED BY SLUG, NOT BY PROJECT, and that change is the whole point of sites. Scoped by project
   * this retired an agency's FIRST client's site the moment their second one went live — silently,
   * because a superseded row is not an error anywhere. `project_id` is still passed and still in the
   * WHERE clause: it is the tenancy filter, and a slug arriving from a caller must never be able to
   * retire another tenant's deployment even if it somehow matched.
   */
  supersedeDeployments(projectId: string, slug: string, keepId: string): Promise<number>;
  /**
  ...
  updateCase(
    id: string,
    patch: Partial<Pick<Case, "stage" | "status" | "data" | "title" | "due_at" | "meeting_at" | "closed_at" | "client_id">>,
    event?: CaseEvent,
  ...
  /**
   * `ifUpdatedAt` makes the write a COMPARE-AND-SET: it applies only if the row still carries that
   * timestamp, and returns `undefined` when somebody else moved it first.
   *
   ...
   *
   * For playbooks that is not an abstract risk. `playbookSaveMeta` appends to `metadata.versions`,
   * and every entry now carries the TEXT IT REPLACED — so a lost update does not merely drop a
   * changelog line, it destroys the only copy of a procedure somebody could have restored. The
   * feature whose entire purpose is "you can always get it back" would silently stop being able to.
   *
   * Optional, so every existing caller is byte-for-byte unaffected: a write with no `ifUpdatedAt` is
   * the unconditional write it always was.
   */
  updateKnowledge(
    id: string,
...
export class InMemoryDomainStore implements DomainStore {
  ...
  async listDeployments(q: DeploymentQuery): Promise<Deployment[]> {
    ...
    return [...this.deployments.values()]
      .filter(
        (d) =>
          d.project_id === q.project_id &&
          (!q.status || d.status === q.status) &&
          // `?? project_id`, because a row written before sites existed has no site_id and its site
          // IS the project. Without this, asking for the founder's own site skips every deployment
          // made before this release.
          (!q.site_id || (d.site_id ?? d.project_id) === q.site_id),
      )
  ...
  async updateDeployment(
    ...
    const next = defined(patch);
    // WRITE-ONCE, matching the `COALESCE(source_key, …)` in the Postgres store. The two must agree
    // or a memory-backed test stops catching the thing the SQL is there to prevent: repointing a
    // stored deploy at different bytes after a person has been shown the first set.
    if (d.source_key !== undefined) delete (next as { source_key?: string }).source_key;
    Object.assign(d, next, { updated_at: now() });
    return d;
  }
  async supersedeDeployments(projectId: string, slug: string, keepId: string): Promise<number> {
    if (!projectId || !slug) return 0;
    let n = 0;
    for (const d of this.deployments.values()) {
      if (d.project_id !== projectId || d.slug !== slug || d.id === keepId) continue;
  ...
  async updateKnowledge(
    ...
    if (!k) return undefined;
    // Same contract as the SQL guard: a caller that names the row it read gets `undefined` when
    // somebody else moved it, rather than clobbering their write with a stale object.
    if (opts?.ifUpdatedAt !== undefined && k.updated_at !== opts.ifUpdatedAt) return undefined;
    ...
    /**
     * STRICTLY ADVANCING, which is what makes `ifUpdatedAt` a version stamp rather than a hint.
     *
     * `now()` here is millisecond ISO, so two writes inside the same millisecond produced the SAME
     * stamp — and the guard above then let the second one through, clobbering the first. Caught by
     * the test, not by reading it: the whole point of the guard is the case where two writers race,
     * which is exactly when they land in the same millisecond.
     *
     * Postgres does not need this: `now()` is the transaction's start time at microsecond
     * resolution, and the second transaction begins after the first commits, so the stamp advances
     * on its own. This keeps the two stores answering the same way.
     */
    const next = now();
    k.updated_at = next > k.updated_at ? next : new Date(Date.parse(k.updated_at) + 1).toISOString();
    return k;
  }
