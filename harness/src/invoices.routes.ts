...
export function mountInvoiceRoutes(app: Hono, deps: InvoiceRouteDeps): void {
  ...
  app.post("/v1/invoices/:id/status", async (c) => {
    ...
    if (to === "paid") {
       ...
       */
      /**
       * THE MOMENT, RECORDED — not the ask.
       *
       * `reachMilestone` is first-occurrence-wins and writes a row; it sends nothing. The ask that
       * hangs off it is due days later and is decided by `dueAsk`, for the reasons in the header of
       * milestones.ts: at this second the founder is looking at the screen, not their inbox.
       *
       * Best effort, and deliberately so. A growth signal that failed to record must never turn "I
       * marked it paid" into a 500 and leave the invoice owing.
       */
      await reachMilestone(getDomainStore(), {
        project_id: updated.project_id,
        kind: "first_client_paid",
        subject: updated.number ?? undefined,
      }).catch((e) => console.error(`[mycel] could not record the first-payment milestone:`, e));
   ...
   */
  /**
   * The client says "I've sent the transfer."
   *
   * ═══ WHY THIS ROUTE EXISTS ═══
   *
   * Most small firms are paid by bank transfer, and a transfer has no webhook. Between the client
   * paying and the founder noticing at the bank, the dunning ladder keeps firing — and being chased
   * for an invoice you already paid is the one automated-dunning failure a client never forgets.
   * This is the client's lever to stop it, offered in their own interest, so it actually gets used.
   *
   * ═══ WHAT IT IS NOT ═══
   *
   * Not a payment. `amount_paid` does not move; money is only ever counted by `recordPayment` and
   * `applyExternalPayment`. A claim buys the client a stand-down window while the founder checks,
   * and dunning treats a stale claim as chaseable again — so a false "I paid" buys a week, not
   * silence, and the eventual chase can say "you mentioned a transfer on the 12th; it has not
   * arrived" instead of pretending the conversation never happened.
   */
  app.post("/v1/portal/invoices/:id/payment-sent", async (c) => {
    ...
    if (inv.status === "paid" || inv.status === "void") {
      return c.json({ error: "nothing is outstanding on this invoice" }, 409);
    }
    ...
    const updated = await billing().updateInvoice(inv.id, {
      ...
      ...(note ? { payment_claim_note: note } : {}),
    });
    if (!updated) return c.json({ error: "not found" }, 404);
    return c.json({
      ok: true,
      // What the client needs to hear, and all they need to hear: we will not chase while we look.
      message: "Thanks — we've noted it and paused reminders while we check.",
    });
  });

  /**
   * The founder checked the bank and found nothing.
   *
   * Clearing the claim puts the invoice back on the ladder immediately rather than waiting out the
   * stale-claim window. Marking the invoice paid is the other exit, and needs no clearing — dunning
   * refuses paid invoices before it ever looks at claims.
   */
  app.post("/v1/invoices/:id/clear-payment-claim", async (c) => {
    const pid = writeProjectId(c);
    const inv = await billing().getInvoice(c.req.param("id"));
    if (!pid || !inv || inv.project_id !== pid) return c.json({ error: "not found" }, 404);
    if (!inv.payment_claimed_at) return c.json({ error: "no payment claim on this invoice" }, 409);
    const updated = await billing().updateInvoice(inv.id, {
      payment_claimed_at: null as unknown as string,
      payment_claim_note: null as unknown as string,
    });
    return c.json({ ok: true, invoice: updated });
  });
  ...
  app.get("/v1/portal/invoices/:id", async (c) => {
     ...
     */
    /**
     * ═══ THE CARD LINK, REPAIRED ON THE SCREEN WHERE IT IS MISSED ═══
     *
     * Until this, a checkout was minted in exactly one place: the moment an invoice transitioned to
     * `sent`. That call is best-effort by design — a Stripe that will not answer must not stop an
     * invoice going out — and there was no second attempt anywhere in the codebase. So three
     * ordinary situations produced an invoice that could never be paid by card, permanently:
     *
     *   · the business connected Stripe AFTER issuing its first invoices — which is the common
     *     order, because you send the first bill and then go and wire up how to take the money;
     *   · Composio, Stripe or the network was down for the second the invoice was issued;
     *   · the invoice was issued before the business enabled the Stripe rail at all.
     *
     * In every one of them the founder believes their client can pay and the client is looking at a
     * page with no button. It is the quietest possible revenue leak: nothing is red, nobody is
     * told, and the invoice simply ages into a chase.
     *
     * ═══ WHY A WRITE ON A READ IS THE RIGHT CALL HERE ═══
     *
     * `ensureStripeCheckout` persists `payment_link_url`, so this GET can write. That is normally
     * worth refusing, and it is not here, for three reasons: it is idempotent (a second view
     * returns the same URL and makes no call), it is a repair rather than a state change — the
     * invoice already means "this client owes this much" and the link is a rendering of that, not a
     * new fact — and the alternative is a client sitting in front of an unpayable bill.
     *
     * It is CHEAP when it does nothing. With the rail off it is one store read and returns; the
     * Composio round-trip only happens for a business that has actually asked to take cards and is
     * missing a link. And it is scoped to the founder's own verified connection inside
     * `ensureStripeCheckout` — a client's Stripe can never be charged to settle our invoice.
     *
     * ═══ FAIL SOFT, ALWAYS ═══
     *
     * A checkout that cannot be created must never cost the client the page. Bank transfer and cash
     * still print from `howToPay`, which is what most of these businesses are actually paid by.
     */
    const payable =
      inv.status !== "void" && invoiceTotals(inv).amount_due > 0
        ? ((await ensureStripeCheckout(inv)
            .then((url) => (url && url !== inv.payment_link_url ? billing().getInvoice(inv.id) : inv))
            .catch((e) => {
              console.warn(`[mycel] could not mint a card link for invoice ${inv.number} on portal view:`, e);
              return inv;
            })) ?? inv)
        : inv;

    const portal = toPortalInvoice(payable);
    const offer =
      portal.totals.amount_due > 0
        ? await howToPay(payable).catch((e) => {
