...
export interface ObservedEvent {
  ...
  status_hint?: string;
  /**
   * Meet / Zoom / Teams join link when the provider named one (hangoutLink, conference entry,
   * onlineMeeting.joinUrl). Absent when the event is in-person or the provider withheld it.
   * Mycel notes uses this so the founder does not re-paste a link already on the calendar.
   */
  meeting_url?: string;
}

/** HTTPS Meet / Zoom / Teams only — same hosts the join bot accepts. Never a bare string guess. */
export function meetingUrlFromText(raw: unknown): string | undefined {
  const s = str(raw);
  if (!s) return undefined;
  // Prefer an explicit URL in free text (description / location).
  const m = s.match(/https:\/\/[^\s<>"']+/i);
  const candidate = (m ? m[0] : s).replace(/[),.;]+$/, "");
  let u: URL;
  try {
    u = new URL(candidate);
  } catch {
  ...
  if (host === "meet.google.com") return candidate;
  if (host === "zoom.us" || host.endsWith(".zoom.us")) return candidate;
  if (
...
/** Google Calendar: hangoutLink, conference video entry, or a Meet/Zoom/Teams URL in location. */
export function meetingUrlFromGoogleEvent(r: Record<string, unknown>): string | undefined {
  const hangout = meetingUrlFromText(r.hangoutLink);
  if (hangout) return hangout;
  const conf = r.conferenceData && typeof r.conferenceData === "object" && !Array.isArray(r.conferenceData)
    ? (r.conferenceData as Record<string, unknown>)
    : undefined;
  const entries = Array.isArray(conf?.entryPoints) ? conf!.entryPoints : [];
  for (const ep of entries) {
    if (!ep || typeof ep !== "object" || Array.isArray(ep)) continue;
    const row = ep as Record<string, unknown>;
    if (str(row.entryPointType)?.toLowerCase() === "video") {
      const uri = meetingUrlFromText(row.uri);
      if (uri) return uri;
    }
  }
  return meetingUrlFromText(r.location) ?? meetingUrlFromText(r.description);
}

/** Outlook / Graph: onlineMeeting.joinUrl, onlineMeetingUrl, or a join URL in location. */
export function meetingUrlFromOutlookEvent(r: Record<string, unknown>): string | undefined {
  const online =
    r.onlineMeeting && typeof r.onlineMeeting === "object" && !Array.isArray(r.onlineMeeting)
      ? (r.onlineMeeting as Record<string, unknown>)
      : undefined;
  return (
    meetingUrlFromText(online?.joinUrl) ??
    meetingUrlFromText(r.onlineMeetingUrl) ??
    meetingUrlFromText(r.location) ??
    meetingUrlFromText(r.bodyPreview)
  );
}
...
export function normaliseGoogleCalendarEvents(data: unknown): Normalised<ObservedEvent> {
  ...
  for (const r of rows(data, "items", "events")) {
    ...
    const meeting_url = meetingUrlFromGoogleEvent(r);
    if (startDay) {
      out.push({
        external_id: id,
        title: str(r.summary),
...
export function normaliseOutlookEvents(data: unknown): Normalised<ObservedEvent> {
  ...
  for (const r of rows(data, "value", "events", "items")) {
    ...
    const meeting_url = meetingUrlFromOutlookEvent(r);
    if (r.isAllDay === true) {
