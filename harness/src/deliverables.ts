...
import { isLivePageUrl } from "./pages";
...
export function payloadFault(kind: DeliverableKind, v: { artifact_ids?: string[]; url?: string }): string | undefined {
  ...
  switch (DELIVERABLE_KINDS[kind].payload) {
    ...
    case "url":
      ...
      // Same door as wrap: https always, loopback http for a laptop kernel. `http://insecure.test` is not a delivery.
      return isLivePageUrl(url)
        ? undefined
        : `a "${kind}" deliverable's url must be https (or loopback http on a local kernel)`;
  }
}
...
export interface DeliverableStore {
  ...
  /**
   * Record that the founder has been told their client accepted this.
   *
   * Returns false when there was nothing to mark — already marked, not accepted, wrong tenant. The
   * caller uses that to avoid claiming a send it did not make. See founder-alerts.ts for why the
   * marker lives on the row rather than in a queue of its own.
   */
  markFounderNotified(projectId: string, id: string, at: string): Promise<boolean>;
...
export class InMemoryDeliverableStore implements DeliverableStore {
  ...
  async markFounderNotified(projectId: string, id: string, at: string): Promise<boolean> {
    const d = this.rows.get(id);
    // Tenant, state and the marker itself all checked: false means "nothing to mark", which the
    // caller reads as "do not claim you sent this".
    if (!d || d.project_id !== projectId || d.status !== "accepted" || d.founder_notified_at) return false;
    d.founder_notified_at = at;
    return true;
  }
...
export type ChangeComment = {
  ...
  file?: string;
  /**
   * ═══ WHICH SENTENCE, NOT JUST WHICH FILE ═══
   *
   * A client saying "the numbers look off" sends the founder back to read twelve pages. A client
   * clicking the cell and saying "this should be 12,400" is a two-second fix and an unambiguous
   * instruction to the revision run.
   *
   * `where` is the block's human label — "Row 4, column 2", "Heading 2", "Bullet 3" — produced by
   * the same parse the founder's editor uses, so both sides of the review are pointing at the same
   * piece of the document. `quote` is the text as it stood when they objected, which survives a
   * re-parse: block ids are positional and the next version renumbers them, but the sentence they
   * were looking at is still findable.
   */
  where?: string;
  quote?: string;
  note: string;
...
export function formatChangeBrief(request: string, comments?: ChangeComment[]): string {
  ...
  for (const c of (comments ?? []).slice(0, 20)) {
    ...
    if (!note) continue;
    const file = String(c.file ?? "").trim().slice(0, 200) || String(c.artifact_id ?? "").trim().slice(0, 80);
    const spot = String(c.where ?? "").trim().slice(0, 80);
    const quote = String(c.quote ?? "").trim().slice(0, 300);
    // Location first, then the words they were looking at, then what they want. A revision run
    // reading only the first line still knows exactly which sentence is in question.
    const at = [file, spot].filter(Boolean).join(" — ");
    const said = quote ? `${note}\n    (they were looking at: "${quote}")` : note;
    lines.push(at ? `On ${at}: ${said}` : said);
  }
