...
export function mountPortalApprovals(app: Hono, deps: PortalApprovalDeps): void {
  ...
  app.get("/v1/portal/approvals", async (c) => {
    ...
    /**
     * ONE QUERY, AND ONLY FOR THE ROWS THAT SURVIVE THE CHEAP FILTERS.
     *
     * This resolved a task per approval, sequentially, over every approval in the deployment — on
     * the page a paying client opens. `isClientFacing` and the status check need no database at all,
     * so they run first and the fetch is sized to what is left.
     *
     * The tenancy check below is unchanged and still runs per row. It has to: this loop is the only
     * place the list is filtered, and batching the READ must not turn into batching the CHECK.
     */
    const candidates = all.filter((a) => isClientFacing(a.action) && (!wanted || a.status === wanted));
    const owners = await store.getTasksByIds(candidates.map((a) => a.task_id));
