 ...
 */
/**
 * Actions that only LOOK. A read changes nothing, reaches nobody, and cannot be un-sent.
 *
 * ═══ MEASURED IN PRODUCTION, 27 AUG ═══
 *
 * 196 approvals raised, 195 expired, one ever approved. The top three by volume were
 * `search_people` (91), `get_profile` (9) and `get_company` (3) — every one of them a READ. They
 * fell through to "not an action this build recognises, so it comes to you", scored medium, and
 * queued.
 *
 * That default is right for an unrecognised WRITE and wrong for a read, and the cost of getting it
 * wrong is not a mildly annoying queue. It is that a founder who opens a queue of ninety-one
 * "approve this search?" cards stops opening the queue — and then the one card that was a refund
 * expires alongside them. Approval fatigue does not degrade the gate gracefully; it removes it.
 *
 * ═══ WHY A READ IS DIFFERENT IN KIND, NOT DEGREE ═══
 *
 * Every other category here is about consequence: money moved, a promise made, something published
 * that cannot be recalled. A read has no consequence to weigh. Asking a human to authorise one is
 * asking them to rubber-stamp, and a rubber stamp teaches them to stamp the next thing too.
 *
 * The controls that DO apply to reads are the ones already in the system and better suited to it:
 * the connection scope decides whose data can be read at all, and the per-run read ceiling decides
 * how much. Neither needs a person in the loop.
 *
 * Matched on the verb, not on a tool allowlist, so a capability added next month is covered without
...
const READ_ONLY = [
  ...
  "profile",
  "preview",
  // Not a prefix anyone would guess: `company_people` is a read (packages/linkedin verbs.ts
  // READ_LIVE) whose name is two nouns, so no verb stem catches it. Listed rather than matched on a
  // loose "people", because a future `invite_people` or `dm_people` would be a WRITE and a loose
  // needle would quietly wave it through — the one direction this list must never fail in.
  "company_people",
];
...
export function assessRisk(input: RiskInput): RiskVerdict {
  ...
  } else if (hit(hay, READ_ONLY)) {
    // A read. See READ_ONLY: no consequence to weigh, and the connection scope plus the per-run
    // read ceiling are the controls that actually fit it.
    v = { risk: "low", why: "This only looks something up — nothing is sent, changed or charged." };
  } else {
