...
export async function armWait(domain: DomainStore, a: ArmWaitArgs, now = new Date()): Promise<ArmResult> {
  ...
  await domain
    ...
    .catch(() => {});
  /**
   * A wait armed is a piece of work that stopped and is now depending on somebody else. Paired with
   * `wait_resumed` and `wait_dead` these three account for every parked engagement: armed is the
   * denominator, and the other two are the only two ways out. A gap between armed and the sum of the
   * other two is work sitting in the dark, which is exactly the state this product exists to remove.
   */
  capture(a.project_id, KERNEL_EVENT.waitArmed, {
    wedge: kase.wedge,
    resume_task_type: taskType,
    mode: wait.mode,
    // A seven-part join and a single missing receipt behave nothing alike, and averaging them
    // together would hide it.
    conditions: wait.conditions.length,
  });
  return { ok: true, wait };
}
...
async function evaluateOne(
  ...
  switch (c.kind) {
    ...
    case "client_reply": {
      ...
      if (!thread) {
        console.error(`[mycel] wait ${wait.id} names thread ${c.thread_id}, which no longer exists`);
        return { dead: "the conversation it was waiting on no longer exists" };
      }
      if (thread.project_id !== wait.project_id) {
        console.error(`[mycel] wait ${wait.id} names thread ${c.thread_id} from project ${thread.project_id}`);
        return { dead: "the conversation it was waiting on belongs to another business" };
      }
    ...
    case "request_resolved": {
      ...
      if (!req) {
        // See the note on `deliverable_settled` below: the id goes to the log, never to the founder.
        console.error(`[mycel] wait ${wait.id} names request ${c.request_id}, which is not in project ${wait.project_id}`);
        return { dead: "the question it was waiting on no longer exists" };
      }
    ...
    case "invoice_paid": {
      ...
      if (!inv || inv.project_id !== wait.project_id) {
        console.error(`[mycel] wait ${wait.id} names invoice ${c.invoice_id}, which is not in project ${wait.project_id}`);
        return { dead: "the invoice it was waiting on no longer exists" };
      }
    ...
    case "deliverable_settled": {
      ...
      if (!d) {
        /**
         * ═══ THIS SENTENCE IS READ BY THE FOUNDER, ON HOME ═══
         *
         * It rendered as: "every way out of this wait is closed: their answer on 'Calder Dental —
         * February visibility' (deliverable e24996dc9-e8df-4ec9-aa58-ee00d51f4ce3 is not in this
         * project)". Two faults in one clause. A UUID is never a thing to show a person — the same
         * defect `lib/client-name.ts` exists for, one process away. And "is not in this project" is
         * a developer's diagnosis: it describes our tenancy model, not what happened to their work.
         *
         * What happened is that the thing this engagement was waiting for is gone. That is what it
         * says now, and the surrounding sentence already tells them it needs a decision. The id is
         * still wanted when this fires — it goes to the log, where the person who can use it looks.
         */
        console.error(`[mycel] wait ${wait.id} names deliverable ${c.deliverable_id}, which is not in project ${wait.project_id}`);
        return { dead: "the work it was waiting on no longer exists" };
      }
    ...
    default: {
      ...
      console.error(`[mycel] wait ${wait.id} has an unknown condition kind:`, (unknown as { kind?: unknown })?.kind);
      return { dead: "this engagement is waiting on something this version does not understand" };
    }
  }
}
...
export async function resumeWait(
  ...
  await domain
    ...
    .catch(() => {});
  /**
   * THE PROOF THAT ANSWERING AN ASK DOES ANYTHING.
   *
   * Until today a resume could not happen at all for most task types — the wedges that raise every
   * ask in production declare no `waits_for`, so an answered question started nothing, forever. That
   * is fixed, and this is how we find out whether it stayed fixed: `request_raised` gives the asks,
   * the two answer events give the replies, and this one says whether the work actually restarted.
   * Without it the funnel stops one step short of the only outcome that matters.
   */
  capture(wait.project_id, KERNEL_EVENT.waitResumed, {
    wedge: wait.wedge,
    resume_task_type: wait.resume?.task_type,
    // How long the engagement sat parked. The number that turns "it resumed" into "it resumed in
    // time to be useful".
    parked_hours: Math.round((Date.parse(now.toISOString()) - Date.parse(wait.created_at)) / 3_600_000),
  });
    ...
    try {
      ...
      if (verdict.state === "unresolvable") {
        ...
        await domain
          ...
          .catch(() => {});
        /**
         * THE THIRD AND LAST EXIT. Armed = resumed + expired + broken, and this branch is the one
         * that means we shipped a wait nothing could ever satisfy — a bug on our side, not a slow
         * client. It is separated from expiry deliberately: they look identical on a dashboard that
         * counts "waits that did not resume", and only one of them is our fault.
         */
        capture(wait.project_id, KERNEL_EVENT.waitDead, { wedge: wait.wedge, how: "broken", why: verdict.reason });
        summary.broken++;
      ...
      if (verdict.state === "expired") {
        capture(wait.project_id, KERNEL_EVENT.waitDead, {
          wedge: wait.wedge,
          how: "expired",
          nudges: wait.nudge_count,
          // 6-of-7 and 0-of-7 are a phone call and a dead client. Same event, different business.
          satisfied: verdict.progress.satisfied,
          total: verdict.progress.total,
        });
