...
import type { Org, Project, Role, SignupRequest, StoredInvite, StoredMember, StoredSignupAccess } from "./identity";
...
const rowToProject = (r: any): Project => ({
  ...
  archived_at: r.archived_at ? iso(r.archived_at) : undefined,
  ...
  domain_provisioning: r.domain_provisioning ?? undefined,
...
export class IdentityPg {
  ...
  static async connect(url: string): Promise<IdentityPg> {
    ...
    await withSchemaLock(pool, async (client) => {
      ...
      await client.query(`
         ...
         *
         * A business carries invoices, released deliverables and a portal on its own subdomain. Its
         * customers hold links into it. Hard-deleting that to tidy a switcher would break somebody
         * else's records to solve somebody's clutter, so there is no delete and there should not be.
         *
         * What there was, until now, was nothing at all: a project could be created and never
         * removed. Every trial walkthrough left a permanent business in the switcher, for every
         * user, forever — thirteen of them on the founder's own account before this shipped.
         */
        ...
        -- The custom-domain SERVING state (certificate ARN, ACM validation record, distribution id,
        -- state machine position — see domains.aws.ts). One jsonb column rather than six scalar
        -- ones, because the record changes shape only as a unit and is read only as a unit; and it
        -- MUST be durable: a kernel restart mid-issuance that forgot the certificate ARN would mint
        -- a second certificate and strand the founder's published CNAME against the first.
        ALTER TABLE projects ADD COLUMN IF NOT EXISTS domain_provisioning jsonb;
        ...
        -- Who may sign up when the deployment is invite-only (MYCEL_SIGNUP_INVITE_ONLY). Two entry
        -- kinds share the shape: an allowlisted email (email set, no token) and a single-use link
        -- (token_hash set, expiry, email optional to pin it). Only the token's HASH is stored, as
        -- for invites: a signup link is a credential that creates an account.
        ...
        -- Asks, not grants. Landing / invite-gate "request access" writes here so a super-admin
        -- can see the address without fishing it out of mail. Email is the key: asking twice
        -- bumps last_seen_at. granted_at is set when the matching allowlist row is created.
  ...
  async loadAll(): Promise<{
    ...
    signupAccess: StoredSignupAccess[];
    signupRequests: SignupRequest[];
  }> {
    const [o, p, m, k, inv, sa, sr] = await Promise.all([
      ...
      this.pool.query(`SELECT * FROM signup_access`),
      this.pool.query(`SELECT * FROM signup_requests`).catch(() => ({ rows: [] })),
    ]);
    return {
      ...
      signupAccess: sa.rows.map((r: any) => ({
        id: r.id,
        email: r.email ?? undefined,
        note: r.note ?? undefined,
        invited_by: r.invited_by,
        token_hash: r.token_hash ?? undefined,
        // bigint arrives as a string from pg, same trap as `reset_expires` above.
        expires_at: r.expires_at === null || r.expires_at === undefined ? undefined : Number(r.expires_at),
        used_at: r.used_at ? iso(r.used_at) : undefined,
        created_at: iso(r.created_at),
      })),
      signupRequests: sr.rows.map((r: any) => ({
        email: r.email,
        source: r.source ?? undefined,
        about: r.about ?? undefined,
        created_at: iso(r.created_at),
        last_seen_at: iso(r.last_seen_at),
        granted_at: r.granted_at ? iso(r.granted_at) : undefined,
      })),
    };
  }
  ...
  /**
   * One org and its members, read FRESH — the plan facts a key mint needs.
   *
   * `loadProjectScope` covers the worker that has never heard of a tenant; this covers the worker
   * that has heard of it and is therefore worse off: the in-memory org it holds keeps the plan it
   * had at hydration, and `keyForOrg`'s vault comparison then agrees with the stale plan, so a
   * founder's upgrade changes nothing on that replica until it happens to restart. Observed on
   * app.mycelai.dev 2026-08-16: plan bumped to growth, deep-model runs refused with "key not
   * allowed to access model" until the worker was rolled by hand.
   *
   * Members ride along for the same reason they do in `loadProjectScope`: `orgIsUnlimited` scans
   * them, and a fresh org next to stale members would answer the super-admin question from a
   * different point in time than the plan question.
   */
  async loadOrgScope(orgId: string): Promise<{ org?: Org; members: StoredMember[] }> {
    const [o, m] = await Promise.all([
      this.pool.query(`SELECT * FROM orgs WHERE id=$1`, [orgId]),
      this.pool.query(`SELECT * FROM members WHERE org_id=$1`, [orgId]),
    ]);
    return { org: o.rows[0] ? rowToOrg(o.rows[0]) : undefined, members: m.rows.map(rowToMember) };
  }
  ...
  async upsertProject(p: Project): Promise<void> {
    await this.pool.query(
      `INSERT INTO projects (id, org_id, name, wedges, slug, custom_domain, domain_verify_token, custom_domain_verified_at, domain_provisioning, branding, archived_at)
       ...
       ON CONFLICT (id) DO UPDATE SET name=EXCLUDED.name, wedges=EXCLUDED.wedges, slug=EXCLUDED.slug,
         ...
         domain_provisioning=EXCLUDED.domain_provisioning, branding=EXCLUDED.branding,
         archived_at=EXCLUDED.archived_at`,
      [
        p.id, p.org_id, p.name, JSON.stringify(p.wedges ?? []),
        p.slug ?? null, p.custom_domain ?? null, p.domain_verify_token ?? null,
        p.custom_domain_verified_at ?? null,
        p.domain_provisioning ? JSON.stringify(p.domain_provisioning) : null,
        p.branding ? JSON.stringify(p.branding) : null,
        p.archived_at ?? null,
      ],
    );
  }
  ...
  async upsertSignupAccess(e: StoredSignupAccess): Promise<void> {
    await this.pool.query(
      `INSERT INTO signup_access (id, email, note, invited_by, token_hash, expires_at, used_at)
       VALUES ($1,$2,$3,$4,$5,$6,$7)
       ON CONFLICT (id) DO UPDATE SET used_at=EXCLUDED.used_at`,
      [e.id, e.email ?? null, e.note ?? null, e.invited_by, e.token_hash ?? null, e.expires_at ?? null, e.used_at ?? null],
    );
  }
  async deleteSignupAccess(id: string): Promise<void> {
    await this.pool.query(`DELETE FROM signup_access WHERE id=$1`, [id]);
  }

  async upsertSignupRequest(r: SignupRequest): Promise<void> {
    await this.pool.query(
      `INSERT INTO signup_requests (email, source, about, created_at, last_seen_at, granted_at)
       VALUES ($1,$2,$3,$4,$5,$6)
       ON CONFLICT (email) DO UPDATE SET
         source = COALESCE(EXCLUDED.source, signup_requests.source),
         about = COALESCE(EXCLUDED.about, signup_requests.about),
         last_seen_at = EXCLUDED.last_seen_at,
         granted_at = COALESCE(EXCLUDED.granted_at, signup_requests.granted_at)`,
      [r.email, r.source ?? null, r.about ?? null, r.created_at, r.last_seen_at, r.granted_at ?? null],
    );
  }
