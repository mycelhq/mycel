 ...
 *     engagement multiplier, an 08:00–19:00 sending window and a 45-second floor between
...
function worstCase(kinds: KindExposure[], anyUnknown: boolean): WorstCase {
  ...
  const excluded = [
    ...
    "connection pacing — per-connection budgets, an account-age ramp and an 08:00–19:00 sending window, none of which is a per-business number",
  ...
  /**
   * `email:send_reminder` is how the grant is stored, not how a founder decides whether fifty a day
   * is acceptable.
   *
   * The first pass at this produced "50 × send reminder (email)", which is better than a quoted
   * connector id and still not English: a multiplication sign, an imperative verb where a noun
   * belongs, and the connector in brackets like a footnote. Read aloud it is "fifty times send
   * reminder open-paren email".
   *
   * A COUNTABLE NOUN, because the sentence around it is counting: "50 email reminders". The verb
   * is dropped ("send reminder" → "reminder") since the sentence already says send, and the
   * connector becomes an adjective, which is where it belongs and where it is genuinely useful —
   * fifty emails and fifty LinkedIn messages are different amounts of exposure.
   */
  const humanAction = (id: string, count?: number): string => {
    const [connector, ...rest] = id.split(":");
    const raw = (rest.join(":") || connector).replace(/[_-]+/g, " ").trim();
    const noun = raw.replace(/^send\s+/i, "").trim() || raw;
    const plural = count === 1 || /s$/.test(noun) ? noun : `${noun}s`;
    return rest.length ? `${connector} ${plural}` : plural;
  };

  /** "a, b and c" — an Oxford-less list, because this is a sentence and not a legend. */
  ...
  for (const k of kinds) {
    ...
    for (const a of k.actions) {
      ...
      if (a.ceiling.unbounded) {
        unbounded = true;
        drivers.push(`${humanAction(a.action)} with no ceiling at all`);
        continue;
      }
      if ((a.ceiling.value ?? 0) > 0) {
        actions += a.ceiling.value ?? 0;
        drivers.push(`${a.ceiling.value} ${humanAction(a.action, a.ceiling.value)}`);
      }
    }
  }
  ...
  if (anyUnknown) {
    ...
    sentence = `On its worst day this business could start ${starts} jobs without you, and at least one of them could send without limit: ${listOf(drivers)}. That is not a bound anybody set — it is a missing ceiling.`;
  } else if (starts === 0) {
    sentence =
      "Nothing can happen without you. Nothing starts on its own, and nothing goes out without an approval.";
  } else if (actions === 0) {
    sentence = `Left alone for a day, it could start ${starts} jobs on its own — and send nothing. Every one drafts and stops at Approvals for you.`;
  } else {
    /**
     * The ceiling, said the way a founder would say it. This read "On its worst day this business
     * could start 20 runs without you and reach your customers 50 times with nobody looking:
     * 50 × "email:send_reminder"" — a threat model with a connector id in it, on the screen a
     * founder opens every morning.
     *
     * ONE DRIVER SAYS ITS OWN TOTAL. With a single kind of message the breakdown repeats the number
     * it is breaking down: "up to 50 messages — 50 email reminders". Naming the thing is the useful
     * half, so the total is dropped and the driver becomes the object of the sentence.
     */
    sentence =
      drivers.length === 1
        ? `Left alone for a day, it could start ${starts} jobs on its own and send clients up to ${listOf(drivers)}. Anything past that waits for you.`
        : `Left alone for a day, it could start ${starts} jobs on its own and send clients up to ${actions} messages — ${listOf(drivers)}. Anything past that waits for you.`;
  }
