 ...
 *
 * On cloud this is `starter` — the thing we sell — and it is paired with
 ...
 */
export const defaultPlan = (): Plan => (cloudMode() ? "starter" : "self_hosted");
...
export interface Limits {
  ...
  /**
   * Generated images a month. `null` is unlimited.
   *
   * A SEPARATE CEILING FROM `model_spend_usd_per_month`, because the two are enforced in different
   * places and one cannot see the other. Text spend is bounded by LiteLLM, which holds the org's
   * virtual key and refuses at the proxy. Image generation does not go through LiteLLM at all — it
   * is a Bedrock call the kernel makes with its own role, or a provider key — so it was bounded by
   * nothing whatsoever until this existed.
   *
   * COUNTED, NOT PRICED, and deliberately. An image is between $0.04 on Stability Ultra and $0.22 on
   * GPT Image 2, so a dollar ceiling would mean a founder's allowance silently quartering when an
   * operator changed a default. A count is the same promise on every provider and it is the unit a
   * founder can picture — "forty images" is a number you can hold, "$3.20 of image spend" is not.
   */
...
export const PLAN_LIMITS: Record<Plan, Limits> = {
  ...
  self_hosted: {
    ...
    // The operator's own AWS bill and their own decision.
  ...
  free: {
    ...
    // None. A generated image costs real money on somebody who has not paid one.
    images_per_month: 0,
    meeting_joins_per_month: 4,
    ...OS,
  },
  /**
   * LEGACY. Nothing new lands here; `defaultPlan()` is `starter` and the $49 aisle is not sold.
   * The row stays because live orgs may still be on it. Same grandfathering choice as `free`.
   *
   * Pipeline only: find, enrich, sequence, book. They fulfill the work themselves.
   */
  find: {
    seats: 1,
    projects: 1,
    ...
    // Enough for one site's heroes and dividers, not a stock library.
    images_per_month: 20,
  ...
  starter: {
    ...
    // A few sites, or one that gets iterated on properly.
    images_per_month: 60,
  ...
  growth: {
    ...
    // A working agency: several clients, revisions included.
  ...
  scale: {
    ...
    tasks_per_month: null,
    model_spend_usd_per_month: null,
    // Effectively unmetered for a real firm; still a ceiling a runaway loop hits.
...
/**
 * Has this org ACTUALLY PAID? Not "is it allowed to read its data" — has money moved.
 *
 * ═══ WHY `plan_status === "active"` IS NOT THAT QUESTION ═══
 *
 * An org created outside the cloud signup path gets `plan: "self_hosted"` and, from
 * `createOrgWithOwner`, `plan_status: "active"` — see the `cloud ? "none" : "active"` below. So a
 * row nobody has ever billed reads `active` to every caller that asks the obvious question.
 *
 ...
 * demo row can spend real money on an IP.
 *
 * `billing_ref` is the subscription. Its presence is the only evidence in this schema that anyone
 * agreed to pay, so it is what "paid" means here.
 *
 * A genuine self-hosted deployment is unaffected: it spends its own money on its own egress and
 * sends from its own domain, and neither of the paths this gates belongs to it.
 */
export function hasPaidPlan(org: { plan_status?: string | null; billing_ref?: string | null }): boolean {
  return (org.plan_status ?? "none") === "active" && Boolean((org.billing_ref ?? "").trim());
}
...
export interface Project {
  ...
  created_at: string;
  /**
   * Hidden from the switcher and from every list, and its clocks do not fire.
   *
   * Not a delete: the business keeps its invoices, its released work and its portal subdomain,
   * because its CLIENTS hold links into all three. Reachable by id so an old link still resolves,
   * and reversible.
   */
  archived_at?: string;
  /**
  ...
  /**
   * Where the SERVING half of the custom domain has got to — certificate, edge alias, the lot.
   * Ownership (`custom_domain_verified_at`) is a separate fact on purpose: it is proved by the
   * founder's DNS and never regresses, while provisioning is our work against AWS and can fail,
   * retry and resume. The shape and the state machine live in domains.aws.ts; this store only
   * keeps the record durable across restarts.
   */
...
/**
 * Permission to SIGN UP at all — the cloud deployment's front door, not membership of any org.
 *
 * Two kinds of entry share one table, exactly as their two failure modes share one question ("who
 * let this person in"): an allowlisted EMAIL (email set, no token — reusable until revoked, since
 * an address is not a secret) and a signup LINK (token_hash set, single-use, 30 days, optionally
 * pinned to an address). Only the token's hash is stored, same rule as invites and resets.
 */
export interface SignupAccessEntry {
  id: string;
  /** Lowercased. Present on an allowlist row; optional on a link, to pin who may spend it. */
  email?: string;
  note?: string;
  /** The super-admin who created it, or "control" for the ops token. */
  invited_by: string;
  created_at: string;
  /** Links only. An allowlist row does not expire; revoking it is the way it ends. */
  expires_at?: number;
  used_at?: string;
}
interface StoredSignupAccess extends SignupAccessEntry {
  token_hash?: string;
}

export type SignupAccessStatus = "allowlisted" | "link-pending" | "used" | "expired";

/**
 * Someone asked to be let in — landing hero, capacity band, or the invite gate.
 *
 * Distinct from SignupAccessEntry: that is a GRANT. This is an ASK. The Access settings page lists
 * asks so a super-admin can allowlist without fishing them out of mail. Same email asked twice
 * updates last_seen_at rather than stacking rows.
 */
export interface SignupRequest {
  email: string;
  /** `landing` | `signup` | a short placement like `hero`. */
  source?: string;
  about?: string;
  created_at: string;
  last_seen_at: string;
...
const SIGNUP_LINK_TTL_MS = 30 * 24 * 60 * 60 * 1000;

/**
 * Is this deployment invite-only? Read at call time, like `superAdminEmails`, so a test can flip it
 * and a redeploy takes effect on the next request. Unset means open signup — the self-host default.
 */
export function signupInviteOnly(): boolean {
  const v = (process.env.MYCEL_SIGNUP_INVITE_ONLY ?? "").trim().toLowerCase();
...
/**
 * Roles that may APPROVE work — the moment a person takes responsibility for something reaching a
 * customer. Everyone except `viewer`.
 *
 * This exists because the product SOLD a viewer as read-only ("Reads everything. Approves nothing.")
 * and the kernel did not enforce it: the approve/reject handler gated on tenancy alone, so a viewer
 * — an accountant given a look at the books, a client given sight of their own pipeline — could
 * approve a send. For a business selling itself as safe-by-default to teams, that is the difference
 * between a role and a label. A key-auth scope (no member behind it) is a product integration acting
 * as the org and keeps its existing authority; the gate is only meaningful for member sessions.
 */
const CAN_APPROVE: Role[] = ["owner", "admin", "operator"];
export const canApprove = (role?: Role | string): boolean => CAN_APPROVE.includes(role as Role);

// 7 days, and it SLIDES — `resolveSession` pushes the expiry forward on use, so an active founder
...
class IdentityStore {
  ...
  constructor() {
    ...
    // Stamped explicitly, because `defaultPlan()` answers `starter` under MYCEL_CLOUD and this
  ...
  async attach(pg: import("./identity.pg").IdentityPg): Promise<void> {
    this.pg = pg;
    const { orgs, projects, members, apiKeys, invites, signupAccess, signupRequests } = await pg.loadAll();
    ...
    // Same reasoning: a signup link handed out before a deploy must still admit its holder after.
    // `?? []` because a test's stub backend (and any backend built before this table) omits the key.
    for (const s of signupAccess ?? []) this.signupAccess.set(s.id, s);
    for (const r of signupRequests ?? []) this.signupRequests.set(r.email, r);
   ...
   */
  /**
   * `undefined` means REFUSED, and the caller cannot tell why — which cost a real debugging hour.
   *
   * There are two ways to get it: no such member, and over a ceiling. `/v1/me/prefs` mapped both to
   * a 413 "preferences are too large", so a member the store had never heard of reported as an
   * oversized write. Walking a fresh signup through onboarding produced exactly that: the account's
   * prefs were `null`, the message said they were too big, and the real cause was a kernel restart
   * that had not yet reloaded the member into this Map.
   *
   * The result type says which now, so the route can tell the truth.
   */
  setMemberPrefs(id: string, patch: Record<string, unknown>): Member | undefined {
    return this.setMemberPrefsResult(id, patch).member;
  }

  setMemberPrefsResult(
    id: string,
    patch: Record<string, unknown>,
  ): { member?: Member; reason?: "no_such_member" | "too_large" } {
    const m = this.members.get(id);
    if (!m) return { reason: "no_such_member" };
    ...
    if (Object.keys(next).length > PREFS_MAX_KEYS) return { reason: "too_large" };
    if (JSON.stringify(next).length > PREFS_MAX_JSON_CHARS) return { reason: "too_large" };
    m.prefs = next;
    ...
    return { member: this.publicMember(m) };
  }
  ...
  /**
   * ARCHIVED PROJECTS ARE NOT LISTED, and every switcher, picker and count reads this.
   *
   * `getProject` below deliberately still returns them: an archived business keeps its portal
   * subdomain and its released work, its clients hold links into it, and a link that 404s because
   * the founder tidied their sidebar is a worse outcome than a tidy sidebar is a good one.
   */
  listProjects(orgId: string, opts: { includeArchived?: boolean } = {}): Project[] {
    return [...this.projects.values()].filter(
      (p) => p.org_id === orgId && (opts.includeArchived || !p.archived_at),
    );
  }

  /**
   * Hide a business, or bring it back. `at = null` un-archives.
   *
   * There is no delete and there should not be one: a project carries invoices, released
   * deliverables and a client portal, so removing it to tidy a switcher would destroy somebody
   * else's records to solve somebody's clutter. What was missing was any way to remove it from
   * SIGHT — a project could be created and never removed, so every trial walkthrough left a
   * permanent business in the switcher. Thirteen of them on the founder's own account.
   */
  setProjectArchived(projectId: string, at: string | null): Project | undefined {
    const project = this.projects.get(projectId);
    if (!project) return undefined;
    const next: Project = { ...project, archived_at: at ?? undefined };
    this.projects.set(projectId, next);
    this.indexProject(next);
    if (this.pg) {
      void this.pg
        .upsertProject(next)
        .catch((e) => console.error("[mycel] persist project archive error:", e));
    }
    return next;
  }
  ...
  /**
   * This org, refreshed from Postgres — for the reads where stale is worse than slow.
   *
   * `loadProject` is a read-through on MISS: right for "have I ever heard of this tenant", wrong
   * for "what plan are they on right now", because a replica that hydrated an org keeps that org's
   * plan until it restarts. Key minting is the one call where that gap bills wrong or refuses a
   * model the founder just paid for (see loadOrgScope for the production incident), and it runs
   * once per task, so a single-row SELECT there is the cheapest correctness in this file.
   *
   * Updates the in-memory maps as a side effect, so `limitsFor`/`orgIsUnlimited` called after this
   * answer from the same instant. Falls back to memory when there is no Postgres (dev,
   * self-hosted single process) — in one process, memory IS fresh.
   */
  async freshOrg(orgId: string): Promise<Org | undefined> {
    if (!this.pg) return this.orgs.get(orgId);
    try {
      const { org, members } = await this.pg.loadOrgScope(orgId);
      if (org) {
        this.orgs.set(org.id, org);
        for (const m of members) this.members.set(m.id, m);
      }
      return org ?? this.orgs.get(orgId);
    } catch (e) {
      // Reported, never thrown — a database blip must not stop a run that a slightly stale plan
      // would have served correctly. The stale answer is the degrade, not the crash.
      console.error("[mycel] org refresh failed:", (e as Error).message);
   ...
   */
  /**
   * ARCHIVED PROJECTS ARE STILL ACCESSIBLE. Only the LISTS hide them.
   *
   * The first version of archiving read `listProjects` here, which excludes them — and that makes
   * an archived business unreachable by every route, INCLUDING THE ONE THAT UN-ARCHIVES IT. A
   * founder who tidied their switcher could never bring the business back, could never open its
   * invoices, and would have no way to find out why. A halt with no way back, which is the shape
   * this codebase keeps naming and keeps re-creating.
   *
   * Hiding is a presentation decision. It must not become an authorisation decision.
   */
  accessibleProjectIds(scope: AuthScope, requested?: string): Set<string> {
    const all =
      scope.kind === "key" && scope.project_id
        ? new Set([scope.project_id])
        : new Set(this.listProjects(scope.org_id, { includeArchived: true }).map((p) => p.id));
  ...
  resolveWriteProject(scope: AuthScope, requested?: string): string | undefined {
    ...
    /**
     * An EXPLICITLY requested project may be archived — un-archiving is a write, and so is fixing
     * an invoice on a business you have put away. The IMPLICIT fallback may not: defaulting a write
     * into a business the founder has hidden is the one outcome archiving is supposed to prevent.
     */
    const all = this.listProjects(scope.org_id, { includeArchived: true });
    if (requested && all.some((p) => p.id === requested)) return requested;
  ...
  claimDomain(projectId: string, domain: string): { record: { name: string; value: string } } | { error: string } {
    ...
    // A new claim is a new machine. Leaving the old certificate/distribution record in place would
    // let a founder who moves domains inherit a `live` state for a name nothing was ever done for.
    p.domain_provisioning = { state: "claimed" };
  ...
  markDomainVerified(projectId: string): Project | undefined {
    ...
    // The serving state machine (domains.aws.ts) starts from here. Recorded on the same write as
    // the verification so a crash between the two cannot leave a verified domain stuck "claimed".
    p.domain_provisioning = { ...(p.domain_provisioning ?? {}), state: "ownership_verified" };
    this.indexProject(p);
    if (this.pg) void this.pg.upsertProject(p).catch(() => {});
    return p;
  }
  ...
  /**
   * Record where the certificate/edge work has got to. Whole-object replacement rather than a
   * merge: `advanceCustomDomain` already holds the full record it built, and a field-wise merge
   * here would resurrect cleared fields (a dead cert ARN, a stale error) it deliberately dropped.
   */
  setDomainProvisioning(projectId: string, prov: DomainProvisioning): Project | undefined {
    const p = this.projects.get(projectId);
    if (!p) return undefined;
    p.domain_provisioning = prov;
    if (this.pg) void this.pg.upsertProject(p).catch(() => {});
    return p;
  }
  ...
  private signupAccess = new Map<string, StoredSignupAccess>();
  private signupRequests = new Map<string, SignupRequest>();
  ...
  // ---------------------------------------------------------------------------------------------
  // Signup access — who may create an account when the deployment is invite-only
  // ---------------------------------------------------------------------------------------------

  /** Every entry, newest first, with the state a table can print. `token_hash` never leaves. */
  listSignupAccess(): (SignupAccessEntry & { kind: "email" | "link"; status: SignupAccessStatus })[] {
    return [...this.signupAccess.values()]
      .sort((a, b) => b.created_at.localeCompare(a.created_at))
      .map((e) => ({
        ...this.publicSignupAccess(e),
        kind: e.token_hash ? "link" : "email",
        status: e.token_hash
          ? e.used_at
            ? "used"
            : (e.expires_at ?? 0) < Date.now()
              ? "expired"
              : "link-pending"
          : "allowlisted",
      }));
  ...
  /**
   * Grant someone the door. With an email it is an allowlist row; without one it mints a link and
   * returns the raw token exactly once — the product builds the URL and cannot recover it, exactly
   * as for team invites. `link: true` mints a link even when an email is given, pinning the token
   * to that address. `ttlMs` exists for tests; the product never passes it.
   */
  createSignupAccess(args: {
    email?: string;
    note?: string;
    invitedBy: string;
    link?: boolean;
    ttlMs?: number;
  }): { entry: SignupAccessEntry & { kind: "email" | "link"; status: SignupAccessStatus }; token?: string } {
    const email = args.email?.trim().toLowerCase() || undefined;
    if (email && !args.link) {
      // Re-allowlisting an address replaces the row rather than accumulating duplicates.
      for (const [id, e] of this.signupAccess) {
        if (!e.token_hash && e.email === email) this.signupAccess.delete(id);
      }
    }
    const token = email && !args.link ? undefined : `msia_${randomBytes(24).toString("base64url")}`;
    const entry: StoredSignupAccess = {
      id: randomUUID(),
      email,
      ...
      ...(token
        ? {
            token_hash: createHash("sha256").update(token).digest("hex"),
            expires_at: Date.now() + (args.ttlMs ?? SIGNUP_LINK_TTL_MS),
          }
        : {}),
    };
    this.signupAccess.set(entry.id, entry);
    if (email) this.grantSignupRequest(email);
    if (this.pg) void this.pg.upsertSignupAccess(entry).catch(() => {});
    return {
      entry: {
        ...this.publicSignupAccess(entry),
        kind: token ? "link" : "email",
        status: token ? "link-pending" : "allowlisted",
      },
      ...(token ? { token } : {}),
    };
  }

  deleteSignupAccess(id: string): boolean {
    if (!this.signupAccess.delete(id)) return false;
    if (this.pg) void this.pg.deleteSignupAccess(id).catch(() => {});
    return true;
  }

  /** What a signup link shows before anyone types a password. Public — the token is the credential. */
  peekSignupToken(token: string): { valid: boolean; email?: string } {
    const e = this.findSignupToken(token);
    return e ? { valid: true, ...(e.email ? { email: e.email } : {}) } : { valid: false };
  }

  /**
   * May this address create an account? One of: a super-admin's own email, an allowlisted email, or
   * a live token (which must match its pinned email, when it has one). Returns the entry to consume
   * so the caller can mark a link used only once the signup actually succeeds — everything here is
   * synchronous, so check → create → consume runs without a seam.
   */
  signupPermitted(email: string, token?: string): { ok: boolean; entry?: StoredSignupAccess } {
    const wanted = email.trim().toLowerCase();
    if (isSuperAdminEmail(wanted)) return { ok: true };
    const listed = [...this.signupAccess.values()].find((e) => !e.token_hash && e.email === wanted);
    if (listed) return { ok: true };
    if (token) {
      const e = this.findSignupToken(token);
      if (e && (!e.email || e.email === wanted)) return { ok: true, entry: e };
    }
    return { ok: false };
  }

  /** Spend a link. Called only after the signup it admitted has succeeded. */
  consumeSignupAccess(entry: StoredSignupAccess): void {
    entry.used_at = new Date().toISOString();
    if (this.pg) void this.pg.upsertSignupAccess(entry).catch(() => {});
  }

  private findSignupToken(token: string): StoredSignupAccess | undefined {
    const digest = createHash("sha256").update(token).digest("hex");
    const e = [...this.signupAccess.values()].find((x) => x.token_hash === digest);
    if (!e || e.used_at || (e.expires_at ?? 0) < Date.now()) return undefined;
    return e;
  }

  /** Allowlist, for the reason `publicInvite` is one: a spread would leak `token_hash` next. */
  private publicSignupAccess(e: StoredSignupAccess): SignupAccessEntry {
    return {
      id: e.id,
      email: e.email,
      note: e.note,
  ...
  // ---------------------------------------------------------------------------------------------
  // Signup requests — asked, not granted
  // ---------------------------------------------------------------------------------------------

  /**
   * Record an access request, and say whether we have heard from this address before.
   *
   * ═══ WHY `repeat` EXISTS, AND WHAT IT PREVENTS ═══
   *
   * The previous row was already read here — `prev` below — and the answer was thrown away. So the
   * caller sent a fresh confirmation email every single time an address was submitted, and a
   * founder testing the form twice in ten seconds got two emails. That is the visible half.
   *
   * The invisible half is the abuse case. The landing route rate-limits per IP, which stops one
   * machine spamming and stops nothing else: an attacker rotating addresses can submit a VICTIM's
   * inbox repeatedly and we will mail it, from our domain, at our reputation's expense, to somebody
   * who never asked. That is an email bomb with us as the amplifier, and it burns the sending
   * reputation the entire outbound engine depends on.
   *
   * Returning `repeat` lets the sender make the only correct decision: record the interest, and do
   * not send again. The address still hears from us the FIRST time, which is the only time they
   * asked.
   */
  recordSignupRequest(args: { email: string; source?: string; about?: string }): (SignupRequest & { repeat: boolean }) | { error: string } {
    const email = args.email.trim().toLowerCase();
    if (!email || !email.includes("@") || email.length > 320) {
      return { error: "a valid email is required" };
    }
    const now = new Date().toISOString();
    const prev = this.signupRequests.get(email);
    const row: SignupRequest = {
      email,
      source: (args.source?.trim() || prev?.source || undefined)?.slice(0, 40),
      about: (args.about?.trim() || prev?.about || undefined)?.slice(0, 500),
      created_at: prev?.created_at ?? now,
      last_seen_at: now,
      granted_at: prev?.granted_at,
    };
    this.signupRequests.set(email, row);
    if (this.pg) void this.pg.upsertSignupRequest(row).catch(() => {});
    // `!!prev` and not a time window: an address that asked a month ago and asks again has not been
    // forgotten, it is sitting in the founder's queue. A window here would be a slow-motion version
    // of the same bomb.
    return { ...row, repeat: !!prev };
  }

  /** Pending asks, newest last-seen first. Granted addresses drop off this list. */
  listSignupRequests(): SignupRequest[] {
    return [...this.signupRequests.values()]
      .filter((r) => !r.granted_at)
      .sort((a, b) => b.last_seen_at.localeCompare(a.last_seen_at));
  }

  grantSignupRequest(email: string): void {
    const row = this.signupRequests.get(email.trim().toLowerCase());
...
export type { StoredInvite, StoredMember, StoredSignupAccess };
