...
export function receiptScene(input: ReceiptDocumentInput, kit: BrandKit): Scene {
  ...
  b.text({ x: RIGHT, y: top + 20, text: "RECEIPT", size: 19.5, family: head, weight: "bold", fill: kit.accent, anchor: "end", tracking: 1 });
  let meta = top + 42;
  ...
  y += 24;
  b.text({ x: M, y, text: formatMoney(totals.amount_paid, inv.currency), size: 24.5, family: head, weight: "bold", fill: ink, anchor: "start" });
  y += 26;
  ...
  if (shown.length) {
    ...
    for (const p of shown) {
      const base = y + 14;
      b.text({ x: M + 10, y: base, text: p.paid_at.slice(0, 10), size: 9, family: body, weight: "normal", fill: ink, anchor: "start" });
      b.text({
        x: COL.method,
        y: base,
