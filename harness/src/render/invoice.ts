...
export function invoiceScene(input: InvoiceDocumentInput, kit: BrandKit): Scene {
  ...
  if (logo) {
    ...
    b.text({ x: M, y: top + 22, text: kit.display_name, size: 19.5, family: head, weight: "bold", fill: ink, anchor: "start" });
  }
  if (logo) {
  ...
  b.text({ x: RIGHT, y: top + 20, text: "INVOICE", size: 24.5, family: head, weight: "bold", fill: kit.accent, anchor: "end", tracking: 1 });
  let meta = top + 42;
  ...
  if (status === "paid" || totals.amount_due === 0) {
    b.text({ x: RIGHT, y: y + 4, text: "PAID IN FULL", size: 12.5, family: head, weight: "bold", fill: kit.accent, anchor: "end", tracking: 0.6 });
    y += 18;
  } else if (status === "overdue") {
    b.text({ x: RIGHT, y: y + 4, text: "OVERDUE", size: 12.5, family: head, weight: "bold", fill: "#b91c1c", anchor: "end", tracking: 0.6 });
    y += 18;
  }
  ...
  for (const l of shown) {
    ...
    b.text({
      x: M + 10,
      y: base,
      text: truncateToWidth(l.description || "—", fontFor(body, "normal"), 9, DESC_WIDTH),
      size: 9,
      family: body,
      weight: "normal",
      fill: ink,
    ...
    if (l.kind === "unit") {
      b.text({ x: COL.qty, y: base, text: formatQuantityMilli(l.quantity_milli ?? 0), size: 9, family: body, weight: "normal", fill: faint, anchor: "end" });
      b.text({ x: COL.unit, y: base, text: formatMoney(l.unit_amount ?? 0, inv.currency), size: 9, family: body, weight: "normal", fill: faint, anchor: "end" });
    }
    if (l.tax_bps) {
      b.text({ x: COL.tax, y: base, text: formatBps(l.tax_bps), size: 9, family: body, weight: "normal", fill: faint, anchor: "end" });
    }
    b.text({ x: COL.amount - 10, y: base, text: formatMoney(lineAmount(l), inv.currency), size: 9, family: body, weight: "normal", fill: ink, anchor: "end" });
    y += ROW_H;
  ...
  const totalRow = (label: string, value: string, emphasis = false) => {
    b.text({ x: LABEL_RIGHT, y, text: label, size: emphasis ? 12.5 : 9, family: body, weight: emphasis ? "bold" : "normal", fill: emphasis ? ink : faint, anchor: "end" });
    b.text({ x: RIGHT, y, text: value, size: emphasis ? 12.5 : 9, family: body, weight: "bold", fill: emphasis ? kit.accent : ink, anchor: "end" });
    y += emphasis ? 20 : 15;
  };
