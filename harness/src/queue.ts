...
import { ProvisioningUnavailable, provisionBackoffMs } from "./provisioning";
...
export async function startWorker(store: Store): Promise<{ stop: () => Promise<void> } | null> {
  ...
  runner = (await run({
    ...
    pollInterval: 2000,
    // Empty crontab: do not look for `/app/crontab`. Digest mail is EventBridge →
    // `POST /api/cron/digest` on cloud (Mon/Wed/Fri). A second clock here would double-send.
    crontab: "",
    taskList: {
      [TASK]: async (payload: unknown) => {
        ...
        try {
          ...
          /**
           * THE ONE FAILURE THIS QUEUE RE-ENQUEUES, and only because it can prove nothing happened.
           *
           * `maxAttempts: 1` above is right for a run that DID something. `ProvisioningUnavailable`
           * is raised only from between `setStatus("provisioning")` and `setStatus("running")` — no
           * turn, no action grant, no tool call, nothing sent — so the premise of that rule is
           * absent. See `provisioning.ts`; the production task it exists for died on `Total disk
           * limit exceeded` while twenty-eight orphaned sandboxes held the organisation cap.
           *
           * A NEW job rather than a graphile retry, deliberately: `maxAttempts: 1` is the guarantee
           * that nothing else in this system is ever retried automatically, and widening it would
           * make that guarantee conditional on a code path nobody reads when they add the next task.
           * The bound lives in the task's own event log, so a kernel restart between attempts cannot
           * reset it.
           */
          if (e instanceof ProvisioningUnavailable) {
            const delay = provisionBackoffMs(e.attempt);
            await utils?.addJob(
              TASK,
              { taskId },
              // A DIFFERENT job key, or graphile's dedupe would collapse this into the job that just
              // failed and it would never run.
              { jobKey: `${taskId}:provision:${e.attempt + 1}`, maxAttempts: 1, runAt: new Date(Date.now() + delay) },
            );
            return;
          }
          throw e;
