...
export interface RequestRouteDeps {
  ...
  /**
   * Store a multipart upload against a run, and answer with the artifact.
   *
   * Injected rather than reimplemented: `ingestUpload` in server.ts owns the size cap, the textual
   * /base64 decision, the basename strip that stops `../../etc/passwd`, and the S3 write. A second
   * copy here would be a second place for any of those to be forgotten.
   */
  uploadToTask(c: any, taskId: string, projectId: string): Promise<Response>;
  /**
...
export function mountRequestRoutes(app: Hono, deps: RequestRouteDeps): void {
  const { domain, store, accessible, writeProjectId, clientInProject, spawnFromThread, uploadToTask } = deps;
  ...
  /**
   * A LINK THE CLIENT CAN ACTUALLY OPEN.
   *
   * ═══ THE ASK EXISTED; THE WAY TO ANSWER IT DID NOT ═══
   *
   * `party-link` above refuses client-role asks with "they use the client portal", which assumed
   * something else was putting clients in front of that portal. Nothing was. Measured in
   * production: `mintPortalLink` had ONE non-test call site — a manual per-client route a founder
   * has to know exists — and across the whole system nine portal links had ever been issued, for
   * five clients, none since 27 August. Against forty-one open asks on fifteen clients.
   *
   * The email path does not cover it either. A thread is only created when the project has a
   * channel, and one project of twenty-one has one, so `thread_id` is null on all fifty-one asks
   * ever written and no ask has ever been emailed to anybody.
   *
   * So an ask was a question raised into a room with no door. This is the door: the same portal
   * link the client would have got by email, mintable on demand for any open client ask, so the
   * founder can paste it into whatever channel they already talk to that client on. It is
   * deliberately not conditional on a mailbox — the whole failure was making the client's ability
   * to answer depend on OUR outbound email being configured.
   *
   * Single-use and time-bound like every other portal link; the token is returned once and only
   * its hash is kept.
   */
  app.post("/v1/requests/:id/portal-link", async (c) => {
    const projectId = writeProjectId(c);
    if (!projectId || !accessible(c).has(projectId)) return c.json({ error: "not found" }, 404);
    const row = await requests().getRequest(projectId, c.req.param("id"));
    if (!row) return c.json({ error: "not found" }, 404);
    // The mirror of party-link's guard: that one is for everybody EXCEPT the client, this one is
    // for the client. A `party_role` other than "client" means someone else owes the answer.
    if (row.party_role && row.party_role !== "client") {
      return c.json({ error: "this ask is for a third party — use the party link" }, 409);
    }
    if (row.status !== "open") return c.json({ error: "that request is already closed" }, 409);
    if (!row.client_id) return c.json({ error: "this ask has no client to send it to" }, 409);
    const portal_link = mintPortalLink({ project_id: row.project_id, client_id: row.client_id });
    return c.json({ ok: true, portal_link });
  });
  ...
  /**
   * ═══════════════════════════════════════════════════════════════════════════════════════════════
   * THE FOUNDER ANSWERS THE ASK THEMSELVES
   * ═══════════════════════════════════════════════════════════════════════════════════════════════
   *
   * MEASURED, and it is the reason nothing in this product has ever finished: 51 asks raised, 3
   * resolved, 41 still open. Until this route existed the ONLY way to answer one was
   * `POST /v1/portal/requests/:id/respond` — a client-plane route, behind a portal that five
   * clients have ever logged into.
   *
   * So the run stops and says "send me the September bank statement". The founder HAS the September
   * bank statement; it is in their email, from the client, last Tuesday. And there was nowhere to
   * put it. Their own work sat blocked on a customer logging into a portal nobody had mentioned to
   * them, and the product asked again four hours later.
   *
   * A paying user who cannot unblock their own work churns in the first session, and correctly.
   *
   * ═══ IT DOES NOT PRETEND TO BE THE CLIENT ═══
   *
   * The portal route writes the thread message as `author: sc.client_id`, which is right there and
   * wrong here. `answered_by: "founder"` is recorded on the row, and the message — if there is a
   * thread — is written as the business.
   *
   * That distinction is not tidiness. "The client confirmed the closing balance" and "the business
   * supplied a statement for the client" are different facts about the same number, and a close that
   * cites the first when the second happened is citing evidence that does not exist. The resuming
   * run reads `answered_by` and can say which it had.
   *
   * ═══ NO THREAD MESSAGE, AND NO SPAWN ═══
   *
   * The portal route posts the client's words into the conversation and spawns a reply run, because
   * a customer answering is inbound contact. A founder filling in a blank is not: nothing arrived
   * from the client, so writing into their thread would manufacture a message they never sent, and
   * chasing it with a reply run would answer a person who has not spoken.
   *
   * The blocked work resumes the same way it always did — `resolveRequest` closes the row and the
   * wait sweep picks the engagement back up.
   */
  /**
   * The file the founder already has, attached to the ask that is waiting for it.
   *
   * Uploads land on the RUN that raised the ask, which is where artifacts live and where the
   * resuming episode will look for them. Twenty-two of the twenty-eight open document asks in
   * production carry one.
   *
   * The six that do not are kickoff asks — raised when an engagement opened, before any run — and
   * they are refused here in words rather than silently accepting a file with nowhere to belong.
   * Their answer can still be typed; it is a file they cannot take.
   */
  app.post("/v1/requests/:id/attachments", async (c) => {
    const set = accessible(c);
    for (const pid of set) {
      const row = await requests().getRequest(pid, c.req.param("id"));
      if (!row) continue;
      if (row.status !== "open") return c.json({ error: "that ask is already closed" }, 409);
      if (!row.task_id) {
        return c.json(
          { error: "this ask was raised when the engagement opened, before any run — type the answer instead of attaching a file" },
          409,
        );
      }
      return uploadToTask(c, row.task_id, pid);
    }
    return c.json({ error: "not found" }, 404);
  });

  app.post("/v1/requests/:id/respond", async (c) => {
    const set = accessible(c);
    const b = (await c.req.json().catch(() => ({}))) as { response?: string; artifact_ids?: unknown };
    const response = (b.response ?? "").trim();
    if (!response) return c.json({ error: "say what the answer is — that is the whole point" }, 400);
    ...
    for (const pid of set) {
      const existing = await requests().getRequest(pid, c.req.param("id"));
      if (!existing) continue;
      if (existing.status !== "open") return c.json({ error: "that ask is already closed" }, 409);
      if (existing.kind === "connection") {
        return c.json(
          { error: "this ask is cleared by connecting the app, not by typing an answer" },
          400,
        );
      }

      /**
       * Files are scoped to the PROJECT, not to the client — deliberately different from the portal
       * route. The founder is attaching their own copy of the client's document, which by definition
       * did not come through the client's own uploads.
       */
      ...
      if (existing.kind === "document" && rawIds.length) {
        for (const id of rawIds) {
          ...
          if (!t || t.project_id !== pid) return c.json({ error: "one of those files is not yours to attach" }, 400);
          artifactIds.push(id);
        }
      }

      const resolved = await requests().resolveRequest(pid, existing.id, response, artifactIds, "founder");
      // Lost the race with the client answering in the portal. Their answer stands.
      if (!resolved) return c.json({ error: "that ask is already closed" }, 409);

      // Same credit the portal path gives, for the same reason: the nudge that plausibly earned the
      // answer. Fail-soft — a ledger write must never cost the founder their answer.
      ...
      /**
       * The founder answering on the client's behalf, which the portal event cannot see. Both halves
       * are needed or the funnel reads as "clients never reply" when the real shape is "the founder
       * answered it themselves".
       */
      capture(resolved.project_id, KERNEL_EVENT.requestAnsweredByFounder, {
        kind: resolved.kind,
        with_files: artifactIds.length > 0,
        // How long the ask sat. The number that says whether this is a queue or a bottleneck.
        waited_hours: Math.round(
          (Date.parse(resolved.resolved_at ?? new Date().toISOString()) - Date.parse(resolved.created_at)) / 3_600_000,
        ),
      });
