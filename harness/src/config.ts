...
export function loadConfig(): MycelConfig {
  ...
  return {
     ...
     */
    /**
     * ═══ THREE HOURS, BECAUSE TIME IS NOT WHAT PROTECTS THE BILL ═══
     *
     * This was an hour, and it is a HARD CLAMP: `resolveHarnessProfile` runs every shape default and
     * every wedge override through it, so no service business could be given a longer run however
     * obviously its work needed one. A monthly close, a contract review across a dozen documents and
     * a site build are not the same job on a stopwatch.
     *
     * The instinct is that a bigger number is a bigger bill. It is not, and this codebase already
     * settled the argument twice:
     *
     *   · SPEND IS BOUNDED BY SPEND. `max_cost_usd` is checked by `shouldAbort` on every tick.
     *     models.ts says it outright — "the control that was actually doing this job all along" —
     *     after the cheap tier turned out not to be cheaper.
     *   · A WEDGED RUN IS CAUGHT BY THE STALL WATCHDOG, not by this. `STALL_MS` kills a run after
     *     fifteen minutes with no agent activity, which is the failure this number gets reached for
     *     and is much worse at: a stuck run hits fifteen minutes long before it hits three hours.
     *
     * So a total-runtime ceiling only ever kills runs that are ACTIVELY WORKING. At an hour that
     * means the longest, most valuable pieces of knowledge work are the only ones it can ever stop —
     * precisely inverted.
     *
     * It is a CEILING, not a default. Every shape still asks for what it asks for (`decide` 420s,
     * `operate` 1200s, `deliver` and `build` 3600s) and scheduled runs stay pinned at 30 minutes by
     * scheduler.ts. This only changes what a wedge is ALLOWED to request.
     */
    maxRuntimeCeilingS: Number(process.env.MYCEL_MAX_RUNTIME_S ?? 10_800),
