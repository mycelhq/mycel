...
import { designSystemFor, printPaletteFor, type PrintPalette } from "./design-systems";
...
export interface VisualIdentity {
  ...
  approved_at?: string;
  /**
   * ═══ THE LOOK THE FOUNDER ACTUALLY CHOSE ═══
   *
   * An id from `design-systems/systems/` — the full style system (tokens + prose) a delivering run
   * wears. `archetype` above only ever IMPLIED one through a seven-entry mapping, so a founder could
   * pick a feeling and never the look, and two businesses that both said "editorial" got a byte-
   * identical house style with no way to differ.
   *
   * Set, it wins. Absent, the archetype's mapping still applies, so nothing that exists today
   * changes. Same "the identity proposes, the founder disposes" rule `site.preset` already follows.
   */
...
export interface BrandKit {
  display_name: string;
  /**
   * WHAT THIS BUSINESS BILLS IN. ISO 4217, upper case.
   *
   * ═══ IT WAS £, EVERYWHERE, AND NOBODY CHOSE THAT ═══
   *
   * Three places fell back to `"GBP"` — the proposal, the ops rollup, the agency digest — and the
   * founder was never asked. So a Denver agency's first proposal quoted its own client in pounds,
   * which is not a small cosmetic wrong: it is the number on the page they are being asked to
   * agree to, in a currency they do not hold.
   *
   * A default was unavoidable somewhere. What was wrong was that it lived in three `??` operators
   * rather than on the business, so there was no single thing to change and nowhere to ask. It is
   * here because it belongs to the same object as the name on the letterhead and the address in the
   * footer: the facts about this business that every document repeats.
   *
   * USD when nothing is set, and the previous answer here was GBP — which reproduced, as a default,
   * the exact bug the paragraphs above describe. The comment said "a Denver agency's first proposal
   * quoted its own client in pounds", consolidated three `??` operators into one, and then left the
   * one at £. A single wrong default is easier to fix than three and is still wrong for almost
   * everybody who will ever sign up.
   *
   * Dollars because that is where the customers are, not because it is neutral — no default is. A
   * founder outside the US changes one field in Brand; a founder inside it never thinks about this
   * again, and the silent failure mode (a number on a contract in a currency the client does not
   * hold) stops being the common case.
   */
  currency: string;
  ...
  /** The firm's own site, and what was read off it. See `BrandKitConfig.measured` for the argument. */
  website?: string;
  measured?: {
    at: string;
    url?: string;
...
export interface BrandKitConfig {
  display_name?: string;
  /** ISO 4217. See `BrandKit.currency` — absent means the fallback, not "no currency". */
  currency?: string;
  accent?: string;
  support_email?: string;
  neutral?: string;
  ...
  identity?: VisualIdentity;
  /**
   * THE FIRM'S OWN WEBSITE, AND WHAT WE READ OFF IT.
   *
   * Not the portal we host for them (`Project.custom_domain`) — the site they already had when they
   * signed up. It is the only place in this product where a brand can be MEASURED rather than
   * chosen from a shelf: opendesign's first principle is that an AI naming a colour from memory
   * hallucinates plausible-but-wrong values, and the only fix is to read a real one.
   *
   * `measured` is filled by `measureSite` and is deliberately allowed to carry a `problem` instead
   * of a palette. A site that styles itself with JavaScript returns a shell, and saying so is worth
   * more than an empty palette that reads as "your brand has no colours".
   */
  website?: string;
  measured?: {
    at: string;
    ...
    problem?: string;
    /**
     * The founder has looked at what we measured and said yes. Until then it is evidence, not the
     * brand — the same "the identity proposes, the founder disposes" rule `design_system` follows,
     * and the reason a measurement cannot silently repaint work they are about to send a client.
     */
    accepted_at?: string;
  };
}
...
/**
 * The chosen system's print palette — and ONLY when a choice was actually made.
 *
 * `designSystemFor(undefined)` resolves to the house default system, which is correct for mounting
 * craft (a run must always have a style to read) and wrong here. A business that has configured no
 * identity at all has not picked a look, and dressing its documents in the default system's blue
 * would be a decision nobody made. It keeps the house default, exactly as before.
 *
 * So this returns nothing unless the identity names an archetype or a system, which makes the whole
 * change additive: every previously-configured business renders as it did, and the ones that DID
 * pick a look finally get it.
 */
function systemPalette(identity: VisualIdentity | undefined): PrintPalette {
  const named = identity?.archetype || (identity as { design_system?: string } | undefined)?.design_system;
  return named ? printPaletteFor(designSystemFor(identity)) : {};
}

/**
 * The colour read off the firm's own site, if they have confirmed it is theirs.
 *
 * Deliberately narrow: `accepted_at` AND a valid hex. `measureStyle` already filters neutrals and
 * requires a repeat, so anything reaching here has appeared more than once in colour-bearing CSS —
 * but `HEX` is re-checked because this value ends up in a style attribute and a PDF colour operator,
 * and it was produced by a regex over bytes we did not write.
 */
function measuredAccent(c: BrandKitConfig): string | undefined {
  const m = c.measured;
  if (!m?.accepted_at) return undefined;
  const first = (m.colors ?? []).find((x) => HEX.test(x));
...
export function resolveBrandKit(config: BrandKitConfig | undefined, projectName: string): BrandKit {
  ...
  return {
     ...
     */
    /**
     * EMPTY, not "Invoice". The fallback was a document TYPE, from when the only document was one.
     *
     * A monthly close reached a client with "Invoice" printed where the firm's name goes. A blank
     * masthead reads as unbranded; a wrong one reads as a mistake, and the mistake is on the first
     * line of the first thing they open. Every template draws this as text, so an empty string draws
     * nothing and the document falls back to being about its own title, which is the honest state
     * for a project that has not told us who it is.
     */
    display_name: (c.display_name || (isPlaceholderName(projectName) ? "" : projectName) || "").slice(0, 80),
    /**
     * Validated as a SHAPE, not against a list of codes.
     *
     * ISO 4217 is three letters, and a list here would be a list somebody has to maintain against a
     * standard that adds and retires codes — and the failure of an out-of-date list is refusing a
     * real currency, which is worse than accepting a fictional one. `formatMoney` already falls back
     * to printing the code beside the number for anything it has no symbol for, so an unknown code
     * renders as "MGA 12,500.00" rather than breaking.
     */
    currency: /^[A-Za-z]{3}$/.test((c.currency ?? "").trim()) ? c.currency!.trim().toUpperCase() : "USD",
    /**
     * ═══ AND THE COLOUR WE READ OFF THEIR OWN SITE, ONCE THEY CONFIRMED IT ═══
     *
     * The chain was: typed hex → the system they wear → the archetype's look → our default. A
     * measurement was not in it, so a founder who read their site, looked at the swatches and said
     * "that's us" got their colour on new DELIVERABLES and nowhere else — their client's portal, the
     * masthead on their invoices and the mark beside their name all still wore ours. Half a brand is
     * arguably worse than none: it is the product telling them it heard and then not acting on it.
     *
     * SECOND, not first. A hex they typed into a form is a direct statement; accepting a reading is
     * agreeing with one we proposed. When both exist the typed one is the more deliberate act and
     * still wins outright.
     *
     * UNACCEPTED READINGS ARE NOT IN THE CHAIN AT ALL — `accepted_at` is checked here for the same
     * reason `resolveStyle` checks it. A colour counted out of somebody else's stylesheet is
     * occasionally a third-party widget's, and repainting a client-facing portal on the strength of
     * that would be the loudest possible version of getting it wrong.
     *
     * Contrast is not a risk here and that is not luck: `PortalShell` puts the accent on a 3px rule
     * and a 6px square and keeps text on the audited tokens, precisely because a founder's colour is
     * validated as a colour and nothing more.
     */
    accent: HEX.test(c.accent ?? "")
      ? c.accent!.toLowerCase()
      : (measuredAccent(c) ?? systemPalette(c.identity).accent ?? look?.accent ?? DEFAULT_ACCENT),
    /**
     * ═══ THE SYSTEM SUPPLIES THE INK, NOT A SECOND TABLE OF HEXES ═══
     *
     * There were two mappings from a visual identity to a look: `ARCHETYPE_SYSTEM` points seven
     * archetypes at twenty-nine designer-authored `tokens.css` files, and this file kept its own
     * hand-typed list. The PDF renderer read this one — so the artifact a client receives was
     * dressed by the copy rather than by the system the founder actually picked.
     *
     * `printPaletteFor` reads the chosen system. It returns no ink for the four dark-themed systems,
     * whose `--fg` is nearly white because it belongs on a near-black surface, and `minimal-luxury`
     * maps to one of them — so that guard is load-bearing rather than defensive.
     *
     * Precedence is unchanged and is the rule `withBrandIdentity` already states: the founder's own
     * hex is IDENTITY and wins outright, the system they are wearing is the next answer, and the
     * global default is the last resort instead of the only one.
     */
    neutral: HEX.test(c.neutral ?? "")
      ? c.neutral!.toLowerCase()
      : (systemPalette(c.identity).ink ?? DEFAULT_NEUTRAL),
    ...
    /**
     * WHAT WE READ OFF THEIR OWN SITE, carried onto the kit rather than left on the raw config.
     *
     * `resolveStyle` reads the KIT, not `Project.branding`, so a field stored and not mapped here is
     * a field that exists in the database and reaches nothing — which is exactly what happened on
     * the first attempt, and what the route test caught: the accept wrote `accepted_at`, the pin
     * still said `default`, and every type checked.
     *
     * Colours are re-validated on the way out. They were written by a regex over somebody else's
     * stylesheet, and a value that reaches a style attribute and a PDF colour operator has to hold
     * to the same `HEX` rule the founder's own accent does.
     */
    ...(c.measured
      ? {
          measured: {
            at: String(c.measured.at ?? ""),
            url: c.measured.url,
            colors: (c.measured.colors ?? []).filter((x) => HEX.test(x)).slice(0, 3),
            fonts: (c.measured.fonts ?? []).map((x) => String(x).slice(0, 60)).slice(0, 3),
            problem: c.measured.problem,
            accepted_at: c.measured.accepted_at,
          },
        }
      : {}),
    ...(c.website ? { website: String(c.website).slice(0, 300) } : {}),
  };
}
