...
/**
 * The default per-ad daily-spend ceiling, in the account's major currency unit.
 *
 * Not a business rule — a guardrail against a typo becoming a charge. `placeAd` refuses a budget
 * above this; a founder who genuinely runs bigger campaigns manages them in their own Ads Manager,
 * where they already have the platform's own spend controls. $10,000/day is far above any sane
 * service-business test spend and far below a number that ruins someone before they notice.
 */
export const MAX_DAILY_BUDGET_MAJOR = 10_000;
...
function providerId(data: unknown): string | undefined {
  const d = asRecord(data) ?? {};
  // Composio nests the created resource differently per toolkit (`data`, `result`, `response`,
  // `campaign`), so we check every shape we've seen. A missed id here is not cosmetic: it is the
  // handle we pause the campaign with, and without it a live, spending ad has no off switch. When
  // this returns undefined, `placeAd` refuses to report a clean success (see below).
  const nested = asRecord(d.data) ?? asRecord(d.result) ?? asRecord(d.response) ?? asRecord(d.campaign);
  return (
    str(d.id) ??
    str(d.campaign_id) ??
    str(d.campaignId) ??
    str(nested?.id) ??
    str(nested?.campaign_id) ??
    str(nested?.campaignId) ??
...
export async function placeAd(args: {
  ...
  // A ceiling on a single click that spends real money every day until paused. This is the last
  // line between a fat-fingered "10000" and an ad platform charging it — the inputs only enforce a
  // floor, and the platform takes whatever it is handed. A founder who genuinely wants more raises
  // it deliberately; nobody types five figures a day by accident and means it.
  if (args.ad.daily_budget_major > MAX_DAILY_BUDGET_MAJOR) {
    return {
      ok: false,
      error: `That daily budget (${args.ad.daily_budget_major}) is above the ${MAX_DAILY_BUDGET_MAJOR}/day ceiling Mycel places by default. Lower it, or raise the ad in your own Ads Manager if you really mean to spend that much a day.`,
      status: 400,
      ad: args.ad,
    };
  }
  ...
  const now = new Date().toISOString();
  const resolvedId = providerId(created.data) ?? args.ad.provider_campaign_id;
  // Created and spending, but we could not read back the campaign id — so we have no handle to pause
  // it. That is the worst state an ad can be in, and it must NOT be dressed up as a clean success:
  // the ad is live (the platform made it), and its `error` carries the one instruction that can stop
  // the spend — pause it in the platform's own Ads Manager. `pauseAd` refuses rather than firing a
  // pause at an undefined id, which is how this used to fail silently while the money kept going out.
  const live = await savePaidAd(args.domain, {
    ...
    error: resolvedId
      ? undefined
      : `${TOOLKIT_LABEL[toolkit]} created this ad but didn't return a campaign id, so Mycel can't pause it for you. If you need to stop it, pause it in ${TOOLKIT_LABEL[toolkit]} Ads Manager.`,
    provider_campaign_id: resolvedId,
    placed_at: now,
    paused_at: undefined,
  });
...
export async function pauseAd(args: {
  ...
  // No campaign id means we never got a handle back when it was placed (see `placeAd`). Firing a
  // pause at an undefined id just fails opaquely at the provider while the ad keeps spending — so we
  // say the true thing instead: it can only be paused where it can be targeted, the Ads Manager.
  if (!args.ad.provider_campaign_id) {
    return {
      ok: false,
      error: `Mycel doesn't have a campaign id for this ad, so it can't pause it. Pause it in ${TOOLKIT_LABEL[toolkit]} Ads Manager.`,
      status: 409,
      ad: args.ad,
    };
  }
