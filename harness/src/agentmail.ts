...
async function call<T>(cfg: AgentMailConfig, path: string, init: RequestInit = {}): Promise<AgentMailCall<T>> {
  /**
   * ═══ THIS HAD NO DEADLINE, AND IT IS THE MAIL PATH ═══
   *
   * A bare `fetch` followed by an unguarded `await res.text()` — the exact shape `http.ts` was
   * written for after it took the outbound engine dark for a full day: "one stranger's web server
   * returned headers and then trickled its body forever". The `catch` here only ever fired on a
   * refused connection; a socket that opened and went quiet held the call on undici's 300s default.
   *
   * Every send, every inbox provision and every thread read goes through this function, inside a
   * customer's run, which has a runtime ceiling it would hit while reporting nothing at all.
   */
  const r = await fetchWithDeadline(`${cfg.baseUrl}${path}`, {
    ...init,
    headers: {
  ...
  if (r.status === 0) {
    // A network failure is not a 4xx. Saying "HTTP 0" would send a founder looking at their DNS, and
    // a timeout says so in its own words rather than as a generic reachability problem.
    return { ok: false, status: 0, detail: `could not reach AgentMail: ${r.detail ?? "network error"}` };
  }
  return {
    ok: r.ok,
    status: r.status,
    data: r.json as T | undefined,
    // The body of a failure, truncated. It is AgentMail's own error text, not a customer's mail.
    detail: r.ok ? "ok" : `AgentMail HTTP ${r.status}: ${r.text.slice(0, 300)}`,
  };
}

/**
 * ═══ THE PROVIDER TOLD US HOW TO FIX IT AND WE THREW IT AWAY ═══
 *
 * Asking for a username somebody already has returns AgentMail HTTP 403 with a body that is better
 * than anything we could write:
 *
 *   {"name":"IsTakenError","code":"resource_taken","message":"Inbox is taken",
 *    "fix":"…these usernames are currently available: hello7096, hello2689, hello8578.",
 *    "suggestions":["hello7096","hello2689","hello8578"]}
 *
 * The route turned that into a 502 carrying 300 characters of raw JSON. Two things wrong with it:
 * 502 is a server error, and this is the founder typing a name somebody else took — a 409, which a
 * form can render as a field error rather than an outage. And the suggestions, which are the whole
 * answer, arrived as an unreadable blob in the middle of an escaped string.
 *
 * One AgentMail account serves every tenant on this deployment, so "taken" is not rare: it is what
 * happens the second time anyone asks for `hello` or `billing`, which are the first names a founder
 * tries.
 */
export function inboxNameTaken(detail: string): { taken: boolean; suggestions: string[] } {
  if (!/resource_taken|IsTakenError|Inbox is taken/i.test(detail)) return { taken: false, suggestions: [] };
  // The body is embedded in `AgentMail HTTP 403: {…}` and may be truncated at 300 chars, so parse
  // the array out rather than JSON.parse-ing something that might have lost its closing brace.
  // No closing `]` required: `call()` truncates at 300 chars, so the array is often cut mid-way and
  // a strict match would drop every suggestion exactly when the founder most needs one.
  const raw = /"suggestions"\s*:\s*\[([^\]]*)/.exec(detail)?.[1] ?? "";
  const suggestions = [...raw.matchAll(/"([a-z0-9._-]{1,64})"/gi)].map((m) => m[1]).slice(0, 5);
 ...
 *
 * Found by provisioning the first customer inbox this system has ever had and then sending through
 * it. The send worked — HTTP 200, a real SES message id. Nothing came back: no thread, no message,
 * no task.
 *
 * The org webhook was real, enabled, and pointed at the right URL. It was also scoped:
 *
 *   "inbox_ids": ["gotomarket@agentmail.to", "mycel@agentmail.to"]
 *
 * Two of OUR OWN inboxes, registered by hand in August. Every inbox a founder provisions after that
 * is absent from the list, so its inbound mail fires no webhook at all.
 *
 * The consequence is the worst shape available and it is silent: we email a client a question, the
 * client replies, and the reply is delivered to an inbox nobody is listening to. The founder sees a
 * question that was asked and never answered, and concludes their client ignored them.
 *
 * The kernel side never had this problem — `connectionForInbox` routes an inbound message by its
 * inbox and refuses one it has no record of, so a webhook covering the whole organisation is
 * already safe. A per-inbox allowlist at the provider is a second registry that must be maintained
 * in lockstep with the first, and this is what it looks like when it is not.
 *
 * So: clear the filter rather than append to it. An org-wide webhook cannot fall behind.
 */
export async function ensureInboxDelivery(cfg: AgentMailConfig, inboxId: string): Promise<{ ok: boolean; detail: string }> {
  const list = await call<{ webhooks?: Array<{ webhook_id?: string; url?: string; inbox_ids?: string[]; enabled?: boolean }> }>(
    cfg,
    "/v0/webhooks",
  );
  if (!list.ok) return { ok: false, detail: `could not read webhooks: ${list.detail}` };
  const hooks = list.data?.webhooks ?? [];
  if (hooks.length === 0) return { ok: false, detail: "no webhook is registered, so no inbox can receive" };

  // Already covered: either an org-wide hook (no filter) or one that names this inbox.
  const covering = hooks.filter((h) => h.enabled !== false);
  if (covering.some((h) => !h.inbox_ids || h.inbox_ids.length === 0)) return { ok: true, detail: "covered org-wide" };
  if (covering.some((h) => (h.inbox_ids ?? []).includes(inboxId))) return { ok: true, detail: "already listed" };

  /**
   * ═══ AND IT CANNOT BE WIDENED IN PLACE ═══
   *
   * Measured against the live API, 6 September. `PATCH /v0/webhooks/{id}` returns 200 and changes
   * NOTHING: `inbox_ids: []`, `inbox_ids: null` and a full replacement list all came back 200 with
   * the filter untouched and `updated_at` still reading August. It accepts `{"nonsense_field":1}`
   * with a 200 too, so the endpoint ignores its body entirely. `PUT` is a 404 — the route does not
   * exist.
   *
   * The only thing that works is delete-and-create, and a created webhook returns a NEW signing
   * secret — which means rotating `AGENTMAIL_WEBHOOK_SECRET` and redeploying the kernel. That is an
   * operator action with a coordination window, not something to do inside a founder's click on
   * "Create mailbox".
   *
   * So this REPORTS rather than repairs. A scoped webhook is a deployment-level fault: it affects
   * every inbox at once, it is fixed once, and the fix is written down in the message. Silently
   * half-fixing it per provision would be worse than saying so.
   */
  const listed = covering.flatMap((h) => h.inbox_ids ?? []);
  return {
    ok: false,
    detail:
      `the AgentMail webhook is scoped to ${listed.length} inbox(es) and does not include ${inboxId}, ` +
      "so mail sent to this address will arrive and fire nothing. The scope cannot be edited — PATCH " +
      "is a no-op and PUT does not exist — so an operator must recreate the webhook with no " +
      "`inbox_ids`, put the new signing secret in `mycel/agentmail`, and redeploy the kernel.",
  };
}
