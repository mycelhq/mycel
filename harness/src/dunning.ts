...
import type { Client, Connection, Invoice, Schedule, TaskSource } from "./contract";
import { checkClientTouch, heldLine } from "./client-touch.pg";
import { chaseIntervalFor, chasePolicyFor, chasesExhausted, type ChasePolicy } from "./chase-policy";
 ...
 */
/**
 * How long a client's "I've sent the transfer" stands the ladder down.
 *
 * SEVEN DAYS: long enough for an international transfer and a weekend, short enough that a false
 * claim cannot buy a month of silence. After it, the invoice is chaseable again and the claim
 * travels with the chase as context rather than as a shield.
 */
export const CLAIM_STANDDOWN_DAYS = 7;
 ...
 */
// Moved to chase-policy.ts, which owns the clamping that enforces it. Re-exported because several
// modules and comments reach for it here.
export { MIN_CHASE_INTERVAL_DAYS } from "./chase-policy";
 ...
 */
/**
 * The DEFAULT ladder, for callers with no project in hand.
 *
 * The rungs used to be written here as literals, which made them the law rather than a default — a
 * founder asking us to ease off had nothing to change, because the number was in a compiled
 * function. `chase-policy.ts` owns them now and this delegates. Ranking surfaces (moves.ts) call it
 * without a policy on purpose: they are estimating "when could this next be chased" for a list, and
 * loading a policy per row to refine a sort key is not worth the reads.
 *
 * The sweep, which actually sends, resolves the project's real policy — see `chasePolicyFor`.
 */
export function chaseIntervalDays(daysOverdue: number, policy?: ChasePolicy): number {
  return chaseIntervalFor(daysOverdue, policy);
}
...
export function chaseTaskInput(
  ...
  gate?: LadderGate,
  offer?: PaymentOffer,
  client?: Pick<Client, "display_name" | "handles">,
): Record<string, unknown> {
  const totals = invoiceTotals(inv);
  const clientEmail = client ? clientEmailHandle(client) : undefined;
  return {
    invoice_id: inv.id,
    ...
    how_to_pay: offer
      ...
      : undefined,
    /**
     * WHO THIS CHASE IS TO — the recipient, travelling with the debt.
     *
     * ═══ THE PRODUCTION FAILURE THIS FIELD EXISTS FOR ═══
     *
     * Holy-grail run 1 (evals/product/HOLY-GRAIL-RUN-1.md, breakage 2): the input carried the
     * invoice number, the amounts, the due date and `how_to_pay` — everything about the DEBT and
     * nothing about the PERSON. The client row held `handles: ["admin@quillandstone.example"]`
     * the whole time; the agent asked `brain:ask` for "client email contact", got nothing usable,
     * filed a knowledge gap and honestly declined: `{"step":"final_notice","channel":"none",
     * "message":"Draft not sent: client email address and payment instructions are unavailable."}`.
     * An invoice 26 days overdue, a correct rung, a working mailbox — and no address to write to.
     *
     * The same NEVER-INVENT rule as everywhere else in this input: when the client row has no email
     * handle, `email` stays absent and `email_missing` says why in a sentence the agent can act on
     * (surface it to the founder), because an agent told nothing will either guess an address or —
     * as run 1 showed — burn a run discovering the absence the harness already knew about.
     */
    client: client
      ? {
          ...
          ...(clientEmail
            ...
            : {
                email_missing:
                  "no email address is on file for this client — do not guess or invent one; " +
                  "tell the founder to add the client's email to the client record so reminders can be addressed",
              }),
        }
      : undefined,
    note: inv.note,
...
/**
 * The client's email address as the CLIENT ROW knows it, or undefined when it holds none.
 *
 * `Client.handles` is the normalized identity list inbound matching already trusts (emails, phone
 * numbers, ids — see contract.ts), so "the first handle containing @" is a read of existing truth,
 * not a parse and not a validation. Deliberately NOT a fallback chain through metadata or
 * preferences: an address good enough to send a dunning email to is exactly one good enough to
 * resolve that client's replies, and those must be the same string or the reply lands nowhere.
 */
export function clientEmailHandle(client: Pick<Client, "handles">): string | undefined {
  return (client.handles ?? []).find((h) => typeof h === "string" && h.includes("@"));
}
...
export async function parkAfterChaseSend(task: {
  ...
  /**
   * THE FOUNDER'S OWN LADDER, not ours.
   *
   * This is the point where the interval stops being an estimate and becomes a promise to somebody's
   * customer about when they will next hear from us, so it is the point that pays for the read. A
   * bookkeeper with a decade of goodwill and a contractor owed for materials want different answers
   * and both are right about their own business.
   *
   * Fails soft to the default: a store blip must never silently stop the collections a business
   * depends on. See chase-policy.ts.
   */
  const policy = await chasePolicyFor(projectId, dunningWedge() ?? "", {
    queryRecords: (q) => getDomainStore().queryRecords(q),
  });
  const days = chaseIntervalDays(overdue, policy);
 ...
 */
/** The pool the touch budget reads. No database means no budget — see nudges.ts. */
function touchDb(): { query: (t: string, v?: unknown[]) => Promise<{ rows: any[]; rowCount: number | null }> } {
  const url = (process.env.MYCEL_DATABASE_URL ?? "").trim();
  if (!url) return { query: async () => ({ rows: [], rowCount: 0 }) };
  return getPool(url);
}

export type ChaseRefusal =
  /**
   * The client has already heard from another wedge today. The invoice is still owed and the next
   * sweep offers it once there is a gap — the fix is "wait", where every other refusal here asks
   * the founder to act. See client-touch.ts.
   */
  | "held"
  | "not_configured"
  /**
  ...
  | "nothing_outstanding"
  /**
   * The client told us, from their portal, that they have sent the transfer — and the claim is
   * fresh. Chasing through it is the one automated-dunning failure a client never forgets: a
   * reminder for money they have already paid, from the business that told them it was handling
   * this. The founder confirms at the bank (mark paid) or finds nothing (clear the claim); a claim
   * left alone goes stale in CLAIM_STANDDOWN_DAYS and the ladder resumes on its own, so a false
   * "I paid" buys a week, not silence.
   */
  | "payment_claimed"
  | "paced"
  /**
 ...
 *     email an hour later.
 *
 * The input is built from a SNAPSHOT of `inv` taken before the claim — not from `inv` itself,
 * which the in-memory store's `claimInvoiceForChase` mutates in place. `chaseTaskInput` derives
 * `last_chased_days_ago` from `last_chased_at`, and `next_step` returns `hold` when that is
 * under 2; building it from the stamped row tells the workflow this invoice was chased zero
 * days ago (by this very run) and every chase declines itself. That is not hypothetical: this
 * function said "built from `inv`" while calling `chaseTaskInput` after the claim, and holy-grail
 * run 1 delivered ZERO dunning emails in four simulated weeks because of it. See the snapshot
 * comment in the body.
 *
...
export async function chaseHands(
  ...
  /**
   * ═══ TWO DIFFERENT PROBLEMS, AND THEY WERE ONE SENTENCE ═══
   *
   * This said "this business has no mailbox connected" in both cases, and for one of them that is
   * FALSE in a way the founder can see. Found by running `simulate.ts`: the simulated agency creates
   * an email connection, hangs a channel on it, and receives its clients' first messages through it
   * — and was then told it had no mailbox and should connect the email account it invoices from.
   *
   * Telling somebody to do a thing they have already done is the most frustrating message a product
   * can produce. They go and look, find the mailbox they connected sitting there, and conclude the
   * product is broken rather than incomplete — which is the more expensive conclusion, because a
   * broken thing gets abandoned and an incomplete one gets finished.
   *
   * The real states are different and so is the fix. Nothing connected: connect a mailbox. Something
   * connected that cannot send: it receives fine, and the sending half is missing.
   */
  const messaging = all.filter((c) => c.kind === "email" || c.kind === "agentmail" || c.kind === "composio");
  if (messaging.length) {
    const named = messaging.find((c) => c.name)?.name;
    return {
      ok: false,
      message:
        `Not chasing ${inv.number}: ${named ? `"${named}"` : "your mailbox"} can receive mail but is not set up to ` +
        `send it, so the reminder could be written and nothing would leave. Finish setting up sending on that ` +
        `mailbox and this will start on its own.`,
    };
  }
...
export async function startChase(
  ...
  // THE CLIENT SAID THEY HAVE PAID. Before capability and pacing, because those questions are
  // about whether we CAN send, and this one is about whether we should say anything at all.
  if (inv.payment_claimed_at) {
    const ageMs = Date.now() - Date.parse(inv.payment_claimed_at);
    if (Number.isFinite(ageMs) && ageMs < CLAIM_STANDDOWN_DAYS * 24 * 60 * 60 * 1000) {
      return {
        ok: false,
        reason: "payment_claimed",
        message: `The client says they sent payment${inv.payment_claim_note ? ` (“${inv.payment_claim_note}”)` : ""} — reminders are paused while you check the bank. Mark it paid, or clear the claim if nothing arrived.`,
      };
    }
    // Stale: the transfer never landed. The ladder resumes — and the chase input carries the claim,
    // so the message can say "you mentioned a transfer on the 12th; it has not arrived" instead of
    // pretending the conversation never happened. Ignoring what a client told you is the same
    // failure as asking twice for a statement they already sent, pointed the other way.
  }
  ...
  /**
   * ═══ AND THE CLIENT'S PATIENCE, WHICH IS SHARED WITH EVERY OTHER WEDGE ═══
   *
   * Directly beneath `cannot_send` because the argument is identical and already written above: a
   * chase whose message cannot land is a paid model call producing a paragraph nobody reads, and
   * refusing after the claim buries the invoice for a ladder interval. A chase the client will not
   * receive TODAY because geo-monitor and books-keeper already wrote is the same waste.
   *
   * The ladder is our patience about this invoice. This is the client's patience about our
   * business, and no single wedge can see it — seven declare client-facing task types and none
   * knows the others exist.
   *
   * A chase outranks a nudge and a check-in (money has a deadline outside our control), so in
   * practice this holds far less often than they do. When it does hold, the invoice is still owed
   * and the next sweep offers it once there is a gap.
   */
  if (inv.client_id) {
    const verdict = await checkClientTouch(touchDb(), { taskType: "chase_invoice", clientId: inv.client_id }, now.getTime());
    if (!verdict.allow) {
      const line = heldLine("chase_invoice", inv.client_id, verdict);
      if (line) console.log(line);
      return { ok: false, reason: "held", message: verdict.detail };
    }
  }
  ...
  /**
   * The WHOLE row, snapshotted before the claim — not just `last_chased_at`.
   *
   * ═══ THE PRODUCTION FAILURE THIS SNAPSHOT EXISTS FOR ═══
   *
   * Holy-grail run 1 (evals/product/HOLY-GRAIL-RUN-1.md, breakage 1). The release path above
   * shielded ITSELF from the in-memory store's mutation — `previouslyChasedAt` is read before the
   * claim — but `chaseTaskInput(inv, …)` was still called AFTER `claimInvoiceForChase` had stamped
   * the very object `inv` points at. So every run's input said `last_chased_days_ago: 0` — chased
   * zero days ago, by this very run — and the wedge's own `next_step` 48h rule answered `hold`, on
   * a 5-day and a 47-day overdue invoice alike. Verbatim: "it was last chased 0 days ago, so it
   * must not be chased again within 48 hours". On a memory-store kernel (demo, e2e, the eval
   * harness) the invoice-chaser could NEVER send; Postgres returns a fresh row from the claim and
   * hid it, which is exactly why the test pins the memory store.
   *
   * A copy of the row rather than one field, so the next field the claim (or a future stamp)
   * mutates does not reopen the same hole one property over.
   */
  const preClaim: Invoice = { ...inv };
  const overdue = chaseDaysOverdue(inv, today);
  ...
  /**
   * The recipient, resolved here — where the claim already proved this run owns the invoice —
   * rather than inside `chaseTaskInput`, which stays pure and synchronous for `proposeMoves`.
   * A client lookup that throws degrades to "no contact in the input", never to a failed chase:
   * the run can still write the words and say what is missing, which beats a spawn_failed that
   * buries the invoice for a ladder interval. See breakage 2 in HOLY-GRAIL-RUN-1.md.
   */
  const clientRow = claimed.client_id
    ? await getDomainStore()
        .getClient(claimed.client_id)
        .catch((e) => {
          console.error(`[mycel] could not read client ${claimed.client_id} for chase on invoice ${inv.id}:`, e);
          return undefined;
        })
    : undefined;
  ...
  try {
    const taskId = await deps.spawnTask({
      ...
      input: {
        // `preClaim`, NOT `inv` and NOT `claimed` — both of those carry the stamp this run just
        // wrote on the in-memory store, and an input built from them is the self-poisoning
        // `last_chased_days_ago: 0` of holy-grail run 1. See the snapshot above.
        ...chaseTaskInput(
          preClaim,
          today,
          confidence,
          questionGate,
          await howToPay(preClaim).catch((e) => {
