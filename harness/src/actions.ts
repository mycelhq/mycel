...
//
// EVERY KIND HERE EXECUTES. There is no stub shelf. This header used to promise stripe, sms,
// whatsapp and calendar as "structured stubs you wire and they light up", and those kinds were
// deleted from `ConnectionKind` when the decision was made to broker that whole class through
// Composio rather than hand-roll four providers. The promise outlived them by some months, which
// is the failure `contract.ts` describes at the type: a menu advertising dishes the kitchen cannot
// cook. Payments, SMS, WhatsApp and calendar are all reachable — as `composio`, with real OAuth,
// which is why there is no longer anything to wire.
...
import { looksLikeEmail } from "@mycel/deliverability";
import { refusalSaid, suppressionFor } from "./outreach/suppression";
import {
...
import {
  ...
  sendLinkedInMessage,
  reactToLinkedInPost,
  endorseLinkedInSkill, followLinkedInPerson,
  viewLinkedInProfile,
 ...
 */
/**
 * A seeded-read override for the product eval only (`MYCEL_COMPOSIO_MOCK=1`).
 *
 * The hole this fills: `read_bank_transactions` (and the other brokered reads) execute through
 * `readComposio`, which calls the real Composio API against a real tenant app. A test therefore has
 * NO way to inject the month of transactions a monthly-close run must reconcile — so the run always
 * hits the honest refusal ("no bank feed is connected") and never produces a real deliverable to
 * judge. This is Layer 2 of the holy-grail eval: give a mock connection a memory.
 *
 * Mirrors `MYCEL_LINKEDIN_MOCK` exactly — off by default, gated by an env flag the eval stack alone
 * sets, and it only fires when the connection ITSELF carries `config.mock_reads`. A prod connection
 * has no `mock_reads`, so even with the flag on (which prod never sets) a real connection is
 * untouched and still reads through the real broker. The returned shape is byte-identical to a real
 * `readComposio` success: `{ ok: true, data: … }`.
 *
 * `mock_reads` is keyed by the READ SLUG the agent calls (`capability` here — e.g. `list_transactions`
 * or `read_bank_transactions`). The value is handed back as `data` as-is, so a seed can hold either a
 * bare `{ transactions: [...] }` or a `{ data: [...] }` envelope; the caller reads whatever it seeded.
 */
function mockComposioRead(conn: Connection, capability: string): ReadResult | undefined {
  if (process.env.MYCEL_COMPOSIO_MOCK !== "1") return undefined;
  const cfg = (conn.config ?? {}) as Record<string, unknown>;
  const seed = cfg.mock_reads as Record<string, unknown> | undefined;
  if (!seed || typeof seed !== "object") return undefined;
  // Case-insensitive on the slug, because the read path compares slugs case-insensitively
  // (`isReadTool`) and a seed author should not have to match casing to get their data back.
  const key = Object.keys(seed).find((k) => k.toLowerCase() === capability.toLowerCase());
  if (key === undefined) return undefined;
  const value = seed[key];
  // A nested `{ data: … }` is unwrapped so a seed can mirror a real tool envelope OR hand the payload
  // directly; both end up as `data`, exactly the field a real success populates.
  const data =
...
async function readComposio(
  ...
  const mocked = mockComposioRead(conn, capability);
  ...
  try {
    /**
     * A CAPABILITY THE TOOLKIT DOES NOT COVER, ADDRESSED AT THE VENDOR'S OWN API.
     *
     * Composio's Xero toolkit is four tools and none of them reads a transaction, an invoice list or
     * a report — see `proxyRequest`. A connection may therefore map a capability to an API PATH
     * instead of a tool slug:
     *
     *   "read_paths": { "read_bank_transactions": "/api.xro/2.0/BankTransactions" }
     *
     * and the broker's OAuth carries the request to the real endpoint. The token still never enters
     * this process; `isReadTool` still governs what may be named. This is a different way to phrase
     * the request, not a wider grant.
     *
     * Checked BEFORE the tool path so an explicit path always wins: a connection that declares one
     * has been configured by someone who knew the toolkit was insufficient, and silently preferring
     * a same-named tool would take that decision back off them.
     */
    const paths = (conn.config as { read_paths?: unknown }).read_paths;
    const configured =
      paths && typeof paths === "object" && !Array.isArray(paths)
        ? (paths as Record<string, unknown>)[capability]
        : undefined;
    /**
     * The path may come from the CALLER as well as from the connection.
     *
     * `readCapability` knows which `ProviderRead` it is running and passes that read's `path`; a
     * connection may override with `read_paths` when a founder's tenant needs a different endpoint.
     * The connection wins, because it is the per-tenant fact and the provider table is the default.
     */
    const declaredPath = configured ?? params.path;
    if (typeof declaredPath === "string" && declaredPath.trim()) {
      const r = await proxyRequest(cfg, {
        connectedAccountId: cc.connected_account_id,
        endpoint: declaredPath.trim(),
        method: "GET",
        // Only string-ish values become query parameters. An object here would serialise as
        // "[object Object]" and filter on nothing, which reads as an empty period rather than a bug.
        parameters: Object.entries(args)
          .filter(([, v]) => typeof v === "string" || typeof v === "number" || typeof v === "boolean")
          .map(([name, v]) => ({ name, value: String(v), in: "query" as const })),
      });
      return {
        ok: r.successful,
        detail: r.error ?? (r.successful ? undefined : "the provider refused the request"),
        data: r.data,
      };
    }
...
const LINKEDIN_EXECUTORS: Record<string, LinkedInExecutor> = {
  ...
  // The third warm-up. Built and tested in the package since d0d6bd6, lost from this table in the
  // extraction to @mycel/linkedin — so the verb sat in WARMUP_GATED declared dispatchable with no
  // dispatcher, and the playbooks suite flagged it every run. Same rules as its siblings: explicit
  // action only, never a sequence step, inert behind MYCEL_LINKEDIN_WARMUP until the INFERRED
  // endpoint is confirmed from a live capture. The skill must arrive as a urn read off their own
  // profile — the executor refuses names, so it cannot endorse a skill nobody listed.
  async endorse_skill(conn, payload) {
    const memberUrn = str(payload.profile_id ?? payload.member ?? payload.urn);
    if (!memberUrn) return { ok: false, detail: "an endorsement needs a member `profile_id` (urn)" };
    const skillUrn = str(payload.skill_urn ?? payload.skill);
    if (!skillUrn) return { ok: false, detail: "an endorsement needs a `skill_urn` read from their profile" };
    return endorseLinkedInSkill(conn, memberUrn, skillUrn);
  },
...
/**
 * Every recipient this payload names, checked against the business's suppression list.
 *
 * Returns the refusal, or undefined to proceed. ONE suppressed address refuses the whole send rather
 * than quietly dropping that recipient: a chase addressed to a finance team and a person who
 * unsubscribed is not a chase we may send to three of the four, and silently removing somebody would
 * mean the founder believes a message went to a person it did not.
 */
async function suppressedRecipient(
  projectId: string,
  payload: Record<string, unknown>,
): Promise<ActionResult | undefined> {
  const raw = payload.to ?? payload.recipient ?? payload.email;
  const list = (Array.isArray(raw) ? raw : [raw]).filter((v): v is string => typeof v === "string");
  for (const address of list) {
    if (!looksLikeEmail(address)) continue;
    const stopped = await suppressionFor(projectId, address);
    if (!stopped) continue;
    // `refusalSaid` is plain English with no field names and none of our nouns — this reaches a
    // founder's screen, and a refusal is when somebody is already frustrated.
    return { ok: false, code: "suppressed", detail: refusalSaid(stopped) };
  }
  return undefined;
}
...
export async function executeAction(
  ...
  /**
   * ═══ THE SUPPRESSION CHECK, AND IT IS THE FIRST THING THIS FUNCTION DOES ═══
   *
   * Before the platform guard, before the secret, before any decision about which executor runs. The
   * invariant is the one `growth/lib/email/suppression.ts` states and this kernel had no equivalent
   * of: there is exactly one function that sends, and its first operation is "may we". No `force`, no
   * `skipSuppression`, no override parameter, no second send path.
   *
   * NOT GATED ON `isMessagingSend`, deliberately. That reads the capability NAME, and a rule that
   * depends on somebody spelling their action `send_x` rather than `notify_x` is a rule with a way
   * round it that nobody meant to leave. Anything carrying a recipient address is checked, whatever
   * it calls itself and whichever connection kind it runs on.
   *
   * A connection with no project is not checked and cannot be: the list is per-tenant, and an
   * unscoped connection has no list to consult. `connections.ts` already refuses those for every
   * path that matters; this states plainly that suppression is one of the things tenancy buys.
   */
  if (conn.project_id) {
    const refusal = await suppressedRecipient(conn.project_id, payload);
...
async function postWebhook(
  ...
  /**
   * ═══ A STRANGER'S SERVER COULD HOLD A RUN OPEN FOREVER ═══
   *
   * This posted to a URL the FOUNDER configured — an arbitrary third party — with no signal and an
   * unguarded body read. `http.ts` documents the outage that shape caused verbatim: "one stranger's
   * web server returned headers and then trickled its body forever", 798 ticks started, none
   * finished, nothing in the logs. A webhook endpoint that accepts a connection and stops talking is
   * not a hypothetical; it is what a server behind a dying process does.
   *
   * The run has a runtime ceiling and a cost ceiling, so "give up and say so" is always the right
   * answer here, and `timedOut` makes the report say which happened.
   */
  ...
  if (r.status === 0) return { ok: false, detail: r.detail ?? "the webhook could not be reached" };
  return { ok: r.ok, detail: `HTTP ${r.status}`, data: r.json };
}
