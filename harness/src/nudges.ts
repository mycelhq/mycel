 ...
 */
/**
 * The pool the touch budget reads, or a stand-in that allows everything.
 *
 * Resolved here rather than threaded through `NudgeDeps` because `getRequestStore()` is already a
 * global in this file — passing one dependency and not the other would be the inconsistency, not
 * the fix.
 *
 * NO DATABASE MEANS NO BUDGET. Every in-memory test and every developer machine runs without
 * MYCEL_DATABASE_URL, and a politeness check that threw there would fail the suite to protect a
...
export type NudgeRefusal =
  /**
   * The client has already heard from another wedge today. Not a misconfiguration and not our
   * pacing — the request is still outstanding and the next sweep offers it once there is a gap.
   * Its own refusal because the fix is "wait", while every other one here asks the founder to act.
   */
  | "held"
  | "not_configured"
  | "no_wedge"
  ...
  | "too_soon"
  /**
   * The business has no mailbox connected, so a reminder has nowhere to go.
   *
   * Its own refusal because the fix is one click by the founder, and because the alternative — what
   * production actually did — is to spawn the run anyway. Seven of them at 22:30 tonight: each
   * booted a sandbox, was told by `compile` "no mailbox is connected to this business", wrote a
   * perfectly good reminder, correctly answered `channel: "none"`, and reported success. Seven
   * sandboxes to produce seven messages that could never leave, four times a day.
   */
  | "no_channel"
  | "paced"
  | "exhausted"
...
export async function startNudge(
  ...
  /**
   * ═══════════════════════════════════════════════════════════════════════════════════════════════
   * A REMINDER WITH NOWHERE TO GO DOES NOT GET A SANDBOX
   * ═══════════════════════════════════════════════════════════════════════════════════════════════
   *
   * `compile` already knows this — it prints "not equipped to run … no mailbox is connected to this
   * business" — and then the run proceeds anyway. For a job whose ENTIRE output is an outbound
   * message that is pure waste: the draft is written, `channel: "none"` comes back, and the message
   * is discarded. Measured tonight: seven runs, seven sandboxes, nothing sent.
   *
   * That advisory-not-blocking behaviour is right for a job that PRODUCES something a founder can
   * still use — a report is worth writing even if it must be delivered by hand. A chase is not: its
   * only artefact is the sending.
   *
   * Checked before the pacing ladder deliberately, so an unsendable reminder does not consume one of
   * the client's `MAX_NUDGES`. The ask stays open at full budget, and the moment a mailbox is
   * connected the next sweep sends it.
   */
  const canSend = resolveCapability("send_email", await domain.listConnections(), r.project_id);
  if (!canSend.ok) {
    return { ok: false, reason: "no_channel", message: canSend.detail };
  }
  ...
  /**
   * ═══ THE CLIENT'S PATIENCE IS SHARED ACROSS EVERY WEDGE ═══
   *
   * `nudgeIsDue` above is OUR pacing: have we waited long enough since we last nudged about THIS
   * request. It knows nothing about the geo-monitor report and the invoice chase that may have gone
   * to the same person this morning. Seven wedges each declare `nudge_client_request`, no wedge
   * knows another exists, and they all reach one human.
   *
   * Checked BEFORE the claim, deliberately. `claimRequestForNudge` is a compare-and-set that stamps
   * `last_nudged_at`, so claiming and then declining would burn the interval on a nudge nobody
   * received — the request would go quiet for a day having said nothing.
   *
   * An override does NOT relax this, matching the comment below on the budget guard: the ladder is
   * our patience and a founder may spend it, but this is the client's and it is not the founder's
   * to spend either.
   */
  if (r.client_id) {
    const verdict = await checkClientTouch(touchDb(), {
      taskType: "nudge_client_request",
      clientId: r.client_id,
    }, now.getTime());
    if (!verdict.allow) {
      const line = heldLine("nudge_client_request", r.client_id, verdict);
