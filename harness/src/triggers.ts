...
import { isShowroomOrg } from "./showroom";
...
export async function startRunFromTrigger(args: {
  ...
  if (orgId) {
     ...
     */
    /**
     * A SHOWROOM ORG NEVER STARTS WORK — here as well as on `POST /v1/tasks`.
     *
     * Guarding only the HTTP route would have been a guard on the door of a building with an open
     * side entrance: schedules and triggers reach `store.createTask` through this function, not
     * through the route, so a demo tenant with a schedule on it would have quietly run work nobody
     * asked for.
     *
     * It also lets the demo LOOK alive. The first version disabled the demo's schedules to stop
     * exactly that, and the console then greeted every visitor with "Your business is paused" —
     * safe, and the worst possible opening line for a shop window. With the refusal here instead,
     * the schedules stay on, the business reads as running, and nothing runs.
     */
    // 402 rather than 403: this path's status union is the one the caller already handles, and
    // "allowed to exist, not allowed to run" is exactly what the other 402s here mean.
    if (isShowroomOrg(orgId)) return { ok: false, status: 402, reason: "showroom" };
