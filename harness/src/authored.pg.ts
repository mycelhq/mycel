  ...
  await withSchemaLock(pool as never, async (client) => {
    await client.query(`
      CREATE TABLE IF NOT EXISTS authored_wedges (
        ...
        -- See AuthoredWedge.notices in authored.ts. jsonb, not text[]: read whole, never queried.
    ...
    `);
    /**
     * ═══ `notices` WAS ADDED TO THE CREATE AND NOWHERE ELSE ═══
     *
     * `CREATE TABLE IF NOT EXISTS` does nothing at all to a table that already exists, so declaring
     * a new column inside it gives that column to fresh databases only. Production had this table
     * with three rows in it, so the column was never created there and never would have been.
     *
     * `deliverables.pg.ts` carries this same warning verbatim, two files away, above the same fix.
     * The rule is not subtle and it is still the easiest one in this codebase to miss: a column
     * added after the first deploy needs a line HERE, not an edit up there.
     *
     * Found by `src/db/verify.ts` against production — a declared column that was not in
     * `information_schema` — which is the second real fault that check has turned up.
     */
    await client.query(
      `ALTER TABLE authored_wedges ADD COLUMN IF NOT EXISTS notices jsonb NOT NULL DEFAULT '[]'::jsonb`,
    );
  });
}
...
export class PostgresAuthoredStore implements AuthoredStore {
  ...
  async createDraft(r: NewAuthoredWedge): Promise<AuthoredWedge> {
    ...
    const res = await this.pool.query(
      /**
       * ELEVEN COLUMNS, ELEVEN EXPRESSIONS. It was eleven and ten.
       *
       * `notices` was added to the column list and to `SET` on the conflict branch, and no value was
       * added to `VALUES` and no argument to the array below. Postgres answers that with "INSERT has
       * more target columns than expressions" — so this statement could not succeed even on a
       * database that had the column, and writing a drafted service failed in production every time.
       *
       * Nothing caught it. `authored-pg.test.ts` asserts against the SQL as a STRING — a fake
       * `Queryable` records statements and never parses one — which is the right shape for checking
       * that the tenant is in the WHERE clause and is blind to whether the statement is valid SQL at
       * all. `test/insert-arity.test.ts` now counts the two sides across every store.
       */
      `INSERT INTO authored_wedges
         (id, project_id, slug, title, manifest, skills, knowledge, status, described_as, notices, source_task_id)
       VALUES ($1,$2,$3,$4,$5::jsonb,$6::jsonb,$7::jsonb,'drafted',$8,$9::jsonb,$10)
       ON CONFLICT (project_id, slug) DO UPDATE
         SET title=EXCLUDED.title, manifest=EXCLUDED.manifest, skills=EXCLUDED.skills,
             knowledge=EXCLUDED.knowledge, described_as=EXCLUDED.described_as, notices=EXCLUDED.notices,
      ...
      [
        ...
        r.described_as ?? "",
        // `?? []` rather than `?? null`: the column is NOT NULL, and an authored wedge with nothing
        // to warn about has an empty list of notices rather than an absent one.
        JSON.stringify(r.notices ?? []),
        r.source_task_id ?? null,
      ],
    );
