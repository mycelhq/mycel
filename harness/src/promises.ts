...
export interface SendPromise {
   ...
   *
   * ═══ VALUES ARE STRINGS HERE AND COMPARED AGAINST BOOLEANS AND NUMBERS TOO ═══
   *
   * This is why the mechanism was stuck on one task type.
   *
   * The comparison used to require the run's field to be a `string`, which works for `chase_invoice`
   * because its "I decided to say nothing" signal is a string enum (`step: "hold"`). Every other
   * candidate in the catalogue signals it with a boolean or a number — `drafted: false`,
   * `proposed: 0` — and for those the comparison skipped silently, so no run could be excused and
   * every legitimate no-op would have FAILED. Declaring a promise on them was not merely unhelpful,
   * it was unsafe, and leaving them undeclared was the correct response to that.
   *
   * A JSON manifest is the wrong place to demand real booleans: `"unless": { "drafted": false }`
   * reads as a flag meaning "no unless" rather than a value to match. So the manifest stays strings
   * and the comparison coerces, which is a widening — `"hold"` still matches `"hold"`.
   */
...
export function excusedFrom(promise: SendPromise, output: Record<string, unknown> | null): string | undefined {
  ...
  for (const [field, values] of Object.entries(unless)) {
    ...
    /*
      STRINGS, BOOLEANS AND NUMBERS — and deliberately nothing else.

      `null` and `undefined` are not excuses: a field the run never set is a run that did not decide,
      and "we could not tell what it decided" resolving to "it decided to do nothing" is the exact
      swallow this module exists to close.

      AN ARRAY IS NOT AN EXCUSE EITHER, and that one is not theoretical. `String([1])` is `"1"` and
      `String([])` is `""`, so without this guard a field holding a one-element array would match a
      scalar excuse value — and `artifacts: []` / `artifacts: [x]` is a real shape in this catalogue.
      "It produced one artifact" is not "it decided not to send".

      That leaves the three scalar kinds a JSON schema puts in a flag, which is every "I did nothing"
      signal here: `step: "hold"`, `drafted: false`, `proposed: 0`.
    */
    if (typeof actual !== "string" && typeof actual !== "boolean" && typeof actual !== "number") continue;
    const asText = String(actual);
    /*
      `!== undefined`, NOT a truthiness check on the found value.

      The original was `if (hit)`, and `hit` is the matched EXCUSE VALUE — so an excuse of `""`
      matched and was then discarded as falsy, silently. A wedge writing `"unless": { "reason": [""] }`
      — "excused when it gave no reason" — got a promise that could never be kept. Found while trying
      to sabotage this function and discovering the sabotage made no difference, which is the tell
      that two things were cancelling out.
    */
    const hit = values.find((v) => v.toLowerCase() === asText.toLowerCase());
    if (hit !== undefined) return `it decided the right answer here was "${hit}"`;
  }
  return undefined;
}
