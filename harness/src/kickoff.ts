...
import { declaredWaitFor } from "./wait-declaration";
...
export interface KickoffSpec {
  ...
  /** Ignition's production run, when the wedge declares one. See WedgeManifest.fulfillment. */
...
export function fulfillmentOf(
  ...
  const production_task_type =
    typeof o.production_task_type === "string" && /^[a-z][a-z0-9_]{0,63}$/.test(o.production_task_type)
      ? o.production_task_type
      : undefined;
  ...
  if (
    ...
    !intake_asks.length &&
    !money_plan &&
    !deliverable_shapes?.length &&
    !production_task_type
  ) {
    return undefined;
  }
  return { client_connections, intake_asks, money_plan, deliverable_shapes, production_task_type };
}
...
export async function runKickoffPlaybook(args: {
  ...
  /**
   * ═══ THE THREAD IS CREATED BEFORE EITHER LOOP, BECAUSE BOTH ASKS NEED SOMEWHERE FOR A FILE ═══
   *
   * This used to sit between the two loops, so only intake asks carried a `thread_id`. That was
   * exactly right when a connection ask had one way to be satisfied — OAuth — and is wrong now that
   * it has two. `blocked.tsx` only shows the uploader when `thread_id` is set, so a connection ask
   * without one can offer the client nothing but the button, and if the button cannot work for them
   * the engagement stops there with no other move available.
   */
  ...
  // ② Connection invites — kind `connection`, toolkit on the row, portal OAuth clears it. The
  // thread rides along so the same row can accept the file instead. See `fallback` on the ask.
