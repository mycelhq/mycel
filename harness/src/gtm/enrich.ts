...
/**
 * The person's portrait, from whichever field the provider populated.
 *
 * FullEnrich has moved this field around across shapes, so rather than bind to one spelling we try
 * the several places a face legitimately arrives: the top-level `profile_picture_url` / `photo_url` /
 * `picture` the contact object carries, and the same keys under the professional-network social
 * profile. First https string wins — these land in an `img src`, so an http or data URL is dropped.
 */
export function photoFromRow(row: unknown, social?: Record<string, unknown>): string | undefined {
  ...
  for (const c of candidates) {
    if (typeof c === "string" && c.trim().startsWith("https://")) return c.trim();
  }
  return undefined;
}

/** Pull LinkedIn URL + role + company logo + portrait from a FullEnrich contact row (free with any enrich). */
export function parseRichProfile(row: unknown): {
  ...
  return {
    ...
    location,
    photo_url: photoFromRow(r, social),
...
export function emailProvenance(
  ...
  if (pattern) {
    attempts.push({
      ...
      at,
      note: pattern.note,
      ...(pattern.guess ? { guess: pattern.guess } : {}),
    });
  }
  if (fire) {
    attempts.push({
...
/**
 * The `try/catch` was already right about network errors and said nothing about time: a bare
 * `fetch` with no signal, then `res.text()`. A provider that trickles a body forever is not an
 * error, so nothing here would ever have caught it. See http.ts.
 */
async function call(path: string, init: RequestInit): Promise<FullEnrichCall> {
  ...
  const r = await fetchWithDeadline(
    ...
    { redact: (m) => redact(m, key) },
  );
  if (r.status === 0) return { ok: false, status: 0, detail: `fullenrich ${r.detail ?? "unreachable"}` };
  return { ok: r.ok, status: r.status, json: r.json, detail: r.ok ? undefined : `fullenrich ${r.status}` };
}
...
export async function enrichEmails(
  ...
  const patternByKey = new Map<string, PatternHop>();
  for (const t of wanted) {
    const hop = await patternHop(t);
  ...
  for (const t of wanted) {
    ...
    try {
      const data: Record<string, unknown> = {
        ...
        ...(rich.photo_url ? { photo_url: rich.photo_url } : {}),
