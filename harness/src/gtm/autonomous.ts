...
export interface GtmAudience {
  ...
  calendar_url?: string;
  /** Scheduler they wired: calcom, calendly, hubspot, … */
  booking_provider?: string;
  /**
   * Signing secret they paste into that scheduler.
   *
   * Minted once (`openssl rand -hex 32` equivalent) and stored here so rotating the product
   * control token does not silently invalidate the webhook they already configured.
   */
...
export async function saveAudience(
  ...
  const merged: GtmAudience = {
    ...
    booking_provider: patch.booking_provider ?? existing?.booking_provider,
    booking_hook_secret: patch.booking_hook_secret ?? existing?.booking_hook_secret,
...
function filtersToQuery(f: AudienceFilters): {
  query?: string;
  title?: string;
  location?: string;
  industries?: string[];
  ...
  const industries = (f.company_industries ?? []).map((s) => s.trim()).filter(Boolean);
  return {
    query,
    title,
    location,
...
/**
 * A NAME, not a description of the filters.
 *
 * `audienceLabel` joins every filter with a middot, which is right for a sentence explaining what
 * the loop is looking for and wrong for a title. It produced:
 *
 *   Auto: Founders of web apps · apps seeking more exposure through SEO · software ·
 *   mobile applications · France · 2026-09-02
 *
 * on a card whose whole job is to be told apart from the campaign above it at a glance. The date is
 * already on the row, and "Auto:" is already a badge.
 *
 * So: the first title, the place, and nothing else. Long enough to recognise, short enough to read.
 */
function campaignTitle(f: AudienceFilters): string {
  const who = (f.titles ?? []).map((s) => s?.trim()).find(Boolean) ?? f.keywords?.trim() ?? "";
  const where = f.location?.trim() ?? "";
  const name = [who, where].filter(Boolean).join(" · ") || "New audience";
  // Truncate on a word so a clipped title still reads as words rather than as a broken string.
  if (name.length <= 42) return name;
  return `${name.slice(0, 42).replace(/[\s·]+\S*$/, "")}…`;
}
  ...
  try {
    ...
    try {
      const proposed = await proposeCampaign(store, domain, {
        ...
        project_id,
        connection_id: conn.id,
        name: campaignTitle(audience.filters),
        prospects,
      ...
      /**
       * AFTER enrolment, never before: `enrolProspects` refuses to attach anybody once the approval
       * is decided, so approving first produced a correctly-approved campaign with nobody in it.
       */
      await approveOnStandingConsent(
        store,
        proposed.campaign,
        "autopilot is on for this audience — you pre-authorised this loop when you enabled it",
      );
    } catch (e) {
