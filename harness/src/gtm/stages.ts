...
export const STAGES = [
   ...
   *
   * `booked` and `met` were one stage, and the gap between them is where most deals actually die: a
   * no-show, a reschedule that never lands, a calendar invite nobody accepted. A pipeline that
   * cannot tell "we are meeting on Thursday" from "we met and they are thinking" reports both as
   * progress and neither as a thing anybody has to do.
   *
   * Terminal for the sequencer like the three before it — a human owns everything from `replied`.
   */
  "met",
  /**
   * The meeting was booked and did not happen.
   *
   * NOT `lost`. A no-show is the most recoverable state in the pipeline and the one most likely to
   * be abandoned, because nothing surfaces it: `booked` looks like progress for ever. This is the
   * stage that makes "chase the reschedule" a thing the product can see.
   */
  "no_show",
  "won",
  "lost",
