...
import { mayEmail, recordEmailSend } from "../outreach/inbox-health";
import { inboxIdFor } from "../agentmail";
...
import { campaignEnvelope, campaignIsLive, loadCampaign, CROSS_CHANNEL_ACTIONS, type Campaign, type SequenceStep } from "./campaign";
...
export async function advanceSequences(
  ...
  /**
   * ═══ THE CAMPAIGN IS CONSULTED BEFORE THE BATCH IS PICKED, NOT INSIDE THE LOOP ═══
   *
   * This used to sort and `.slice(0, BATCH)` here, and `advanceCase` read the campaign afterwards.
   * That is a starvation bug, and it is not hypothetical — the founder's own outbound engine lost
   * two days to exactly this shape: 478 live enrolments, 316 of them on one paused campaign, ten
   * slots per tick, 948 denials, and counters reading `claimed=10 executed=10`. Busy, achieving
   * nothing.
   *
   * The mechanism is what makes it so reliable a trap. Cases the campaign will refuse are deferred
   * and re-dated, so they accumulate the OLDEST `due_at` values in the set — which means the
   * oldest-first sort, the fairness rule, hands them the entire batch every single tick. The better
   * the queue's manners, the more completely it starves.
   *
   * So paused campaigns are removed from the population BEFORE the sort. Their cases keep their
   * stage and their `due_at` and resume the moment the campaign does; they simply are not due work
   * while nobody wants them sent.
   */
  const ids = [...new Set(eligible.map((k) => String((k.data as Record<string, unknown>).campaign_id)))];
  for (const id of ids) {
    if (!campaigns.has(id)) campaigns.set(id, await loadCampaign(domain, opts.project_id, id));
  }

  const due = eligible
    .filter((k) => campaignIsLive(campaigns.get(String((k.data as Record<string, unknown>).campaign_id))))
  ...
  // Reported, because "nothing went out" and "nothing went out because you paused it" are the same
  // silence otherwise — and the second one is a founder's own decision they may have forgotten.
  const heldByPause = eligible.length - eligible.filter((k) =>
    campaignIsLive(campaigns.get(String((k.data as Record<string, unknown>).campaign_id))),
  ).length;
  if (heldByPause > 0) {
    summary.note = `${heldByPause} prospect${heldByPause === 1 ? "" : "s"} held: their campaign is paused`;
  }

  // Connections that answered "the account is stopped" during THIS tick. The cap that was missing
  // when one dead endpoint produced 308 identical failed Voyager calls every five minutes for ten
  // hours: the first refusal is information, the next twenty-four are noise and egress.
  const stopped = new Map<string, string>();

  for (const kase of due) {
    summary.processed++;
    try {
      const { outcome, stop } = await advanceCase(store, domain, kase, now, campaigns);
      if (stop) stopped.set(stop.connection_id, stop.detail);
    ...
    if (stopped.size) {
      // Every remaining case in this tick runs on the LinkedIn account that just said it cannot act,
      // and the answer would be identical for each of them. They keep their `due_at`, so the moment
      // the account is reconnected the next tick drains the backlog in order.
      summary.note = `stopped this tick: ${[...stopped.values()][0]}`;
      break;
    }
  }
  return summary;
...
interface Stepped {
  ...
  task_id?: string;
  /**
   * The account is stopped, not the prospect.
   *
   * Set when a dispatch came back with a named LinkedIn stop (`linkedin_session_dead`,
   * `linkedin_endpoint_gone`, `linkedin_unhealthy`, `challenged`). Every remaining case in this tick
   * that runs on the same connection would get the identical answer, so the tick stops instead of
   * proving it twenty-four more times. See `LINKEDIN_STOP_CODES`.
   */
  stop?: { connection_id: string; detail: string };
}

/**
 * Codes that mean "this ACCOUNT cannot act", as opposed to "this prospect did not work out".
 *
 * `linkedin_backoff` is deliberately here too: while a connection is backing off, every further
 * dispatch on it is a refusal that costs a store round-trip and buys nothing.
 */
const LINKEDIN_STOP_CODES = new Set([
  "challenged",
  "linkedin_session_dead",
  "linkedin_endpoint_gone",
  "linkedin_unhealthy",
  "linkedin_backoff",
  // The three invitation refusals are account-level by construction: an exhausted weekly window, a
  // full pending pile and a restriction all belong to the ACCOUNT, so every remaining invite case on
  // this connection would get the identical answer. Before these were here, a tick walked the whole
  // prospect list and asked LinkedIn once per prospect — which is how one refused evening became
  // three hundred refusals and three hundred engagement flags on an account that was fine.
  // `linkedin_invite_duplicate` is deliberately NOT here: that one really is about one prospect.
  "linkedin_invite_quota",
  "linkedin_invite_pending_cap",
  "linkedin_invite_restricted",
]);
...
async function advanceCase(
  ...
  /**
   * THE BELT. `advanceSequences` already removed paused campaigns from the population, so this
   * should be unreachable from the tick — but this function is ALSO the one `takeTouch` calls when
   * a founder presses "send this one now", and a paused campaign has to refuse there too.
   *
   * Parked rather than closed, and with a short interval: the founder is expected to un-pause, and
   * a case that closed itself because somebody paused for an hour would be a prospect quietly
   * dropped.
   */
  if (!campaignIsLive(campaign)) {
    await park(
      ...
      campaign.paused_reason
        ? `this campaign is paused — ${campaign.paused_reason}`
  ...
  /**
   * ═══ EMAIL GETS ITS OWN CEILING, RATHER THAN NONE ═══
   *
   * The exemption above is right and the replacement was `{ allowed: true }`, which meant cold
   * outreach email from this kernel had NO ceiling of any kind: no warm-up, no daily cap, no
   * spacing, no working hours. A brand-new mailbox could open a hundred conversations on its first
   * afternoon, which reads to every receiver exactly like a compromised account.
   *
   * `mayEmail` is the deliverability ramp shared with `growth/`, applied HERE — where the campaign
   * is — and not at the transport, because the same connection also carries a dunning chase to an
   * existing client and a receipt. Those go at 3am on a Sunday if that is when the client paid, and
   * an overdue invoice that stopped being chased because a cold mailbox hit its cap would be a far
   * worse bug than the one this fixes.
   */
  const touch = CROSS_CHANNEL_ACTIONS.has(step.action) ? null : touchFor(step.action);
  let verdict: PacingVerdict;
  if (touch) {
    verdict = await assertSendAllowed(domain, conn.id, touch);
  } else if (kase.project_id && sendingAddress(conn)) {
    const window = await mayEmail({ project_id: kase.project_id, address: sendingAddress(conn)!, now: new Date(now) });
    verdict = {
      allowed: window.allowed,
      ...(window.allowed ? {} : { reason: window.reason }),
      remaining: window.remaining,
      budget: window.ramp.cap,
      nextAfterMs: window.nextAfterMs,
    };
  } else {
    // No project or no `from` address: nothing to pace against, and refusing here would park every
    // campaign on a connection whose config we simply cannot read. Said plainly rather than silently.
    verdict = { allowed: true, remaining: 0, budget: 0, nextAfterMs: 60_000 };
  }
  if (!verdict.allowed) {
...
/**
 * The address a mail connection sends FROM, which is the mailbox the ramp is about.
 *
 * `config.from` for a generic email connection, the AgentMail inbox otherwise. Undefined for a
 * LinkedIn connection, which is how the caller knows this is not an email send.
 */
function sendingAddress(conn: Connection): string | undefined {
  if (conn.kind !== "email" && conn.kind !== "agentmail") return undefined;
  const from = conn.config?.from ?? conn.config?.address ?? inboxIdFor(conn);
  const s = typeof from === "string" ? from.trim() : "";
  return s.includes("@") ? s.toLowerCase() : undefined;
}
...
async function dispatchStep(
  ...
  if (!result.ok) {
    ...
    return result.code && LINKEDIN_STOP_CODES.has(result.code)
      ? {
          outcome: "parked",
          stop: { connection_id: dispatchConn.id, detail: result.detail ?? "this LinkedIn account is stopped" },
        }
      : { outcome: "parked" };
  }
  ...
  /**
   * ═══ THE MAILBOX'S OWN COUNT, AFTER A SUCCESSFUL SEND AND NEVER BEFORE ═══
   *
   * Counting on intent rather than on success would let a run of transport failures eat a mailbox's
   * whole daily allowance without one message reaching anybody — and the warm-up would then advance
   * on volume that was never delivered, which is precisely the fiction the earned-week rule exists
   * to refuse. So it is counted here, past the failure branch above.
   */
  if (CROSS_CHANNEL_ACTIONS.has(step.action)) {
    const from = sendingAddress(dispatchConn);
    if (from && campaign.project_id) {
      await recordEmailSend({ project_id: campaign.project_id, address: from, now: new Date(now) }).catch(() => {
        // A counter that failed to write must not fail a send that already happened. The next
        // decision reads a slightly stale count, which errs toward sending less.
      });
    }
  }
