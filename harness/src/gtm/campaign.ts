...
import { SEQUENCE_LIVE_SET } from "../linkedin/verbs";
import { evaluateCondition, factsUsed, ConditionError } from "./conditions";
import { CAMPAIGN_COLLECTION, gtmWedge, STAGES, type Stage } from "./stages";
...
export const DEFAULT_SEQUENCE: SequenceStep[] = [
  ...
  // After LinkedIn silence — email nudge (needs enrich + AgentMail/Gmail). Stops on reply.
  { from: "dm2", action: "send_email", advance_to: "em1", wait_days: 3, only_if: "!replied" },
];

/**
 * The same sequence for people the founder is ALREADY connected to.
 *
 * No invitation step, because there is nothing to invite: a first-degree connection is past that
 * gate. Running `DEFAULT_SEQUENCE` over a warm list burns the scarcest budget in the system —
 * invitations — on people who would only ever have been refused ("you're already connected"), and
 * the case then sits at `invited` waiting for an acceptance that can never arrive.
 *
 * `enrollProspect` marks cases on an invite-less sequence `connected` at enrolment for the same
 * reason: the fact is already true, and the first DM's `only_if: "connected AND !replied"` would
 * otherwise never fire.
 */
export const WARM_SEQUENCE: SequenceStep[] = [
  { from: "queued", action: "view_profile", advance_to: "connected" },
  { from: "connected", action: "send_message", advance_to: "dm1", only_if: "connected AND !replied" },
  { from: "dm1", action: "send_message", advance_to: "dm2", wait_days: 4, only_if: "connected AND !replied" },
  { from: "dm2", action: "send_email", advance_to: "em1", wait_days: 3, only_if: "!replied" },
];
 ...
 */
/** Sequence verbs a campaign envelope may auto-approve. Same set as `SEQUENCE_LIVE` — one table. */
export const AUTO_APPROVABLE = SEQUENCE_LIVE_SET;

/** Non-LinkedIn actions a GTM sequence may name (email nudge after DM silence). */
export const CROSS_CHANNEL_ACTIONS = new Set(["send_email"]);
...
export const KNOWN_FACTS = [
  ...
  "opted_out",
  "has_thread",
  // The `invite_accepted` trigger, observable at last. Distinct from `connected` — see `factsFor`.
  "invite_accepted",
] as const;
...
export interface Campaign {
  ...
  task_id: string;
  /**
   * RUNNING, OR STOPPED BY THE FOUNDER.
   *
   * ─── THE GAP THIS CLOSES ───
   *
   * There was no way to stop a campaign. `Campaign` carried steps, an approval, an expiry and a
   * calendar link, and `advanceCase` checked that the campaign existed, had a step, passed cadence
   * and had a connection — it never asked whether the founder still wanted it running. The only
   * stops were revoking the approval envelope or waiting for `expires_at`.
   *
   * For a product whose entire pitch is "nothing reaches anyone until you approve it", *stop this*
   * having no button is the sharpest contradiction in the repo. A founder who realises the copy is
   * wrong, or that a prospect list was bad, has minutes before the next tick.
   *
   * ─── ABSENT MEANS ACTIVE ───
   *
   * Optional, so every campaign written before this field existed keeps running exactly as it did.
   * A required field would have needed a migration over a jsonb blob, and the failure mode of
   * getting that wrong is every live campaign stopping at once.
   *
   * ─── PAUSED IS NOT CANCELLED ───
   *
   * A paused campaign keeps its cases, its stages and its `due_at`s. Resuming picks up where it
   * stopped rather than restarting anyone — which matters because the alternative is a prospect who
   * gets touch two twice, and the whole point of the pause was that something was wrong.
   */
  status?: "active" | "paused";
  /** Why it was stopped, in the founder's own words. Shown wherever the pause is. */
  ...
  created_at: string;
  /**
   * Where a prospect books time with the founder (Cal.com, Calendly, whatever they use).
   *
   * Read only by humans and by the reply-drafting skill — the sequencer NEVER pastes it into a
   * message, because a booking link in an unsolicited first DM is the single clearest bot tell in
   * outbound. It exists so that the moment someone answers, the handoff has one obvious CTA
   * instead of the founder digging their own link out of another tab.
   */
  calendar_url?: string;
}
...
export interface ProspectDraft {
  ...
  /**
   * The thing that happened which makes this person worth writing to now.
   *
   * Accepted by `POST /v1/gtm/campaigns/:id/enrol`, dropped on the floor by every version before
   * this one — the field was in the request body, in the API docs and in the caller, and no type
   * between the route and the case had a home for it. It is written onto the case so the whole rest
   * of the relationship can see it: the reply drafter, the engagement writer, and the founder
   * reading the pipeline three weeks later wondering why they messaged this person at all.
   */
  signal?: string;
}
...
export function validateSteps(steps: SequenceStep[]): void {
  ...
  for (const s of steps) {
    ...
    // LinkedIn capabilities OR declared cross-channel GTM actions (email after LinkedIn silence).
    if (!capability(s.action) && !CROSS_CHANNEL_ACTIONS.has(s.action)) {
      throw new CampaignError(`"${s.action}" is not a declared LinkedIn capability or GTM channel action`);
    }
...
export async function proposeCampaign(
  ...
  input: {
    ...
    ttl_ms?: number;
    now?: Date;
    /** Booking link, shown to the founder at the handoff. Never sent by the sequencer. */
    calendar_url?: string;
  },
  ...
  const campaign: Campaign = {
    ...
    ...(input.calendar_url?.trim() ? { calendar_url: input.calendar_url.trim() } : {}),
  };
...
/**
 * Stop, or restart, a campaign.
 *
 * A read-modify-write over the same record `saveCampaign` writes, because the campaign IS its record
 * — there is no second row to keep in step. Scoped through `loadCampaign`, so a campaign in another
 * tenant is not found rather than not permitted.
 *
 * PAUSING KEEPS EVERYTHING. Cases hold their stage and their `due_at`; the approval envelope is
 * untouched. Resuming picks up exactly where it stopped, which is the difference between a pause and
 * a cancel: the alternative would re-send touch two to everybody who had already had it, and the
 * reason somebody pauses is usually that something already went out wrong.
 */
export async function setCampaignStatus(
  ...
  const campaign = await loadCampaign(domain, projectId, campaignId);
  if (!campaign || !projectId) return undefined;
  const next: Campaign = {
    ...campaign,
    status,
    ...(status === "paused"
      ? { paused_at: new Date().toISOString(), ...(reason?.trim() ? { paused_reason: reason.trim().slice(0, 300) } : {}) }
      : // Cleared on resume so a later screen cannot show a stale "paused because…" beside a
        // campaign that is plainly running.
        { paused_at: undefined, paused_reason: undefined }),
  };
  await domain.upsertRecord({
    ...
    data: { ...next, campaign_id: campaignId } as unknown as Record<string, unknown>,
  });
  return next;
}
...
/**
 * Is this campaign allowed to spend a touch right now?
 *
 * One predicate, exported, because the sequencer needs it BEFORE it picks its batch and `advanceCase`
 * needs it again as a belt — and two hand-written spellings of "is it paused" is how one of them
 * ends up checking `!== "paused"` and the other `=== "active"`, which disagree for a legacy row.
 */
export const campaignIsLive = (c: Pick<Campaign, "status"> | undefined): boolean =>
  !!c && c.status !== "paused";
...
export async function enrollProspect(
  ...
  // A sequence with no invitation step is a WARM one: these people are already first-degree, and
  // `connected` is a fact about them that is true before we do anything. Without this the first DM's
  // `only_if: "connected AND !replied"` never fires and the whole warm campaign parks silently at
  // `queued` — busy-looking, and sending nothing.
  const warm = !campaign.steps.some((s) => s.action === "send_invite");
  return domain.createCase({
    ...
    data: {
      ...
      name: p.name,
      copy: p.copy ?? {},
      // Kept for the life of the relationship, not just the first message. Everything downstream —
      // the reply drafter, `draft_engagement`, and the founder reading their own pipeline a month
      // later — asks "why did we contact this person", and until now the case could not answer.
      ...(p.signal?.trim() ? { signal: p.signal.trim() } : {}),
      touch_count: 0,
      ...(warm ? { connected: true } : {}),
    },
...
/**
 * ═══ APPROVE A CAMPAIGN ON THE FOUNDER'S STANDING AUTHORITY ═══
 *
 * A founder who turned autopilot ON pre-authorised this loop to find people and reach them. That
 * switch IS the consent, it is what starts the timer, and turning it off is what stops it. Asking
 * again per campaign asked the same question twice and blocked on the second: nothing sends until
 * it is clicked, so an unattended loop produced campaigns that sat until they expired.
 *
 * ═══ CALLED AFTER ENROLMENT, AND THAT ORDER IS NOT OPTIONAL ═══
 *
 * `enrolProspects` refuses to attach anybody once the approval is not `pending` — "the approved
 * list cannot grow afterwards", which is right: a founder approves a campaign against a list they
 * were shown. The first version of this approved inside `proposeCampaign`, before enrolment, and
 * the result was a correctly-approved campaign with ZERO people in it. Approve last.
 *
 * `approved`, not `auto_approved`: `campaignAutoApproval` refuses an `auto_approved` campaign
 * envelope by design, so that status would satisfy the queue and then silently stop every send.
 * `approved` is also the honest word — a human did decide, once, in advance. `policy_reason` says
 * so, and the row keeps the full record of what ran.
 */
export async function approveOnStandingConsent(
  store: Store,
  campaign: Campaign,
  reason: string,
): Promise<void> {
  await store.setApproval(campaign.approval_id, "approved", reason);
  await emitEvent(store, campaign.task_id, "approval.resolved", {
    approval_id: campaign.approval_id,
    action: `gtm:campaign:${campaign.id}`,
    decision: "approved",
    reason,
    by: "autopilot",
  }).catch(() => {});
