...
//
// Deliberately NOT maximally conservative: a GTM add-on that sends one invitation a day produces no
// results, and a customer paying for outcomes will simply do it by hand instead. A connected free
// account starts at the published free ceiling (80 invites/week, 25 messages/day) and only drops
// on evidence — poor acceptance, flags, or a LinkedIn account we *know* is under a week old.
import type { DomainStore } from "./domain";
// The vocabulary — what a touch is, what a tier is — lives with the LinkedIn core in
// @mycel/linkedin (its host seam), because the core must compile without the kernel. The SEMANTICS
// (the ceilings, ramps, windows and multipliers below) stay here, and the kernel implements the
// package's `Pacing` interface with them in linkedin/host.ts.
import type { TouchKind, AccountTier } from "@mycel/linkedin";
...
const CEILING: Record<AccountTier, Ceiling> = {
  // 2026 practitioner consensus: ~100 invites/week soft cap on free+Premium (Premium does not
  // buy more invites). We used to sit at 80 ("a bit below"). That left unused outcome on healthy
  // accounts; the daily-spread guard below is what keeps 100 from dumping in two days.
  free: { invitesPerWeek: 100, messagesPerDay: 25, requestsPerWeek: 5, inmailPerMonth: 0 },
  premium: { invitesPerWeek: 100, messagesPerDay: 50, requestsPerWeek: 15, inmailPerMonth: 15 },
  // Sales Nav: industry upper band is 150–200 with strong trust. Cap at 150 and let engagement
  // earn toward it — opening at 200 is the edge, not "a bit below".
  sales_navigator: { invitesPerWeek: 150, messagesPerDay: 80, requestsPerWeek: 25, inmailPerMonth: 50 },
  recruiter: { invitesPerWeek: 150, messagesPerDay: 100, requestsPerWeek: 30, inmailPerMonth: 150 },
};

/**
 * A brand-new LinkedIn account is the most fragile thing in this system.
 *
 * This ramp is for when we KNOW the LinkedIn account itself is young — not for a member session we
 * just attached. Missing `account_age_days` is treated as established (see `assertSendAllowed`):
 * the founder connected an existing account on a dedicated country line, and first-week warmup
 * of 1 invite / 3 messages a day is unusable dogfood.
 *
 * Under 30 days LinkedIn applies a stricter informal cap until trust signals accumulate. An
 * account that opens at full volume on day two is the classic way to lose one, so the ramp
 * starts at a quarter and reaches full allowance at eight weeks — only when age is known.
 */
...
/** Missing age means an existing member we attached, not a day-old LinkedIn. Fully ramped. */
export const UNKNOWN_ACCOUNT_AGE_DAYS = 365;
 ...
 *
 * Below a very small sample nobody has evidence of anything. A connected free account starts at
 * full allowance so the documented ceilings actually reach the HUD; evidence (acceptance, flags)
 * is what moves the number after that — not a permanent 0.6 haircut on day one.
 */
export function engagementMultiplier(e: Engagement): number {
  if (e.sent < 20) return 1;

  const accept = e.accepted / e.sent;
  const reply = e.replied / Math.max(1, e.sent);
  // ATTEMPTS, NOT SENDS, IN THE DENOMINATOR. `flagged` counts invitations LinkedIn refused, and a
  // refused invitation never reaches `sent` — so dividing by `sent` alone divides refusals by the
  // successes that happened *instead of* them, and the "rate" it produces can exceed 1 (the
  // founder's account read 305/6). Attempts is the population the flags actually came from, so this
  // is a rate in the ordinary sense, and it can only ever be LOWER than what shipped before.
  const flagRate = e.flagged / (e.sent + e.flagged);
...
export interface PacingInput {
  ...
  used: Partial<Record<TouchKind, number>>;
  /**
   * When the current invite (or other) window started, ISO. Used to refuse front-loading the whole
   * weekly invite budget into day one — the classic automation signature.
   */
  windowStartedAt?: Partial<Record<TouchKind, string>>;
...
/**
 * How many invites may have gone out by now if the weekly budget is spread across the window.
 *
 * LinkedIn's soft weekly cap is real; dumping it on Monday is what looks like a bot. Allow a small
 * front-load buffer (≈ one safe day of ~20) so a founder who opens Pipeline mid-week is not stuck
 * at 1 invite, then refuse further invites until the calendar catches up.
 */
export function inviteSpreadCap(budget: number, windowStartedAt: string | undefined, now: Date = new Date()): number {
  const safeDaily = Math.min(25, Math.max(8, Math.ceil(budget / 5)));
  if (!windowStartedAt) return safeDaily; // first touch of a window — one day's worth only
  const started = Date.parse(windowStartedAt);
  if (!Number.isFinite(started)) return safeDaily;
  const elapsedMs = Math.max(0, now.getTime() - started);
  const elapsedDays = Math.floor(elapsedMs / 86_400_000) + 1; // day 1 = first calendar day of window
  return Math.min(budget, safeDaily * elapsedDays);
}
...
export function budgetFor(kind: TouchKind, input: PacingInput): number {
  ...
  const base =
    ...
    : kind === "profile_view" ? 200
    : 100;

  // Documented free ceilings (~100/week, 25/day) are the target. Engagement can earn a 1.15 reply
  // bonus — cap at `base` so a free account never inherits Sales Nav volume. Profile views stay
  // well below LinkedIn's own pain (~80–150/day).
  const raw = Math.floor(base * ageRamp(input.accountAgeDays) * engagementMultiplier(input.engagement));
  return Math.max(1, Math.min(base, raw));
}
 ...
 */
export function withinSendingWindow(localHour: number, _localDay: number): boolean {
  // Every day of the week. Weekends are not a ban signal — a real person messages on a Saturday —
  // so the only guard is the hour: 08:00–19:00 local. Outreach at 3am is the actual bot signature,
  // and that is all this needs to stop. (`_localDay` stays in the signature for callers/tests; it no
  // longer gates.)
...
export function evaluate(kind: TouchKind, input: PacingInput): PacingVerdict {
  ...
  // Invites: refuse front-loading the weekly budget. Spacing alone is not enough if a burst of
  // approvals lands — this is the calendar belt that makes a higher weekly ceiling safe.
  if (kind === "invite") {
    const spread = inviteSpreadCap(budget, input.windowStartedAt?.invite, new Date());
    if (used >= spread) {
      return {
        allowed: false,
        reason:
          `spreading invitations across the week (${used} so far — next batch unlocks as the window rolls). ` +
          "Dumping the weekly allowance in a day is how accounts get restricted.",
        remaining,
        budget,
        nextAfterMs: 6 * 60 * 60 * 1000,
      };
...
/**
 * The account's own local clock, from the offset on its config.
 *
 * Lifted out of `assertSendAllowed` because the polling path below needs exactly the same answer,
 * and two independent copies of "what time is it where this founder lives" is how one of them ends
 * up reading UTC and driving automation against a member's session at 04:00 local.
 */
export function localClock(config: Record<string, unknown> | undefined, now: Date): { hour: number; day: number } {
  const offset = typeof config?.utc_offset === "number" ? config.utc_offset : 0;
...
export async function assertSendAllowed(
  ...
  try {
    ...
    if (cfg.linkedin_challenge) {
      return {
        allowed: false,
        reason: "LinkedIn challenged this session — reconnect before anything else goes out",
        remaining: 0,
        budget: 0,
        nextAfterMs: 24 * 60 * 60 * 1000,
      };
    }
    // The same stop, for the permanent failures that are NOT a challenge — a rejected session, or an
    // endpoint LinkedIn retired (410). Read here so every scheduler that already consults pacing
    // skips the account without learning a second concept. See packages/linkedin/src/health.ts.
    const unhealthy = cfg.linkedin_unhealthy as { detail?: string } | undefined;
    if (unhealthy?.detail) {
      return { allowed: false, reason: unhealthy.detail, remaining: 0, budget: 0, nextAfterMs: 24 * 60 * 60 * 1000 };
    }
    const state = (cfg.pacing ?? {}) as PacingState;
    const now = new Date();
    const local = localClock(cfg, now);

    return evaluate(kind, {
      tier: (cfg.tier as AccountTier) ?? "free",
      accountAgeDays: typeof cfg.account_age_days === "number" ? cfg.account_age_days : UNKNOWN_ACCOUNT_AGE_DAYS,
...
// ── Polling: reads are automation too ────────────────────────────────────────────────────────────
//
// Everything above paces things a stranger RECEIVES. The return path (did they accept? did they
// answer?) is made of reads, and it is tempting to treat a read as free. It is not: a poller is a
// request pattern against a real member's session, from a residential proxy, and a request pattern
// is exactly what LinkedIn's automation detection scores. A GET at 03:40 on a Sunday is a bot
// signature in the same way an invitation at 03:40 on a Sunday is.
//
// So the return path gets the same three disciplines the send path has, and no others invented for
// it: the working-hours window, the "this account is in trouble, stop touching it" pause, and
// jitter. What it does NOT get is a budget counter — a poll delivers nothing to anybody, so
// charging it against the weekly invitation allowance would spend the scarcest thing LinkedIn gives
// an account on a read nobody sees.

/**
 * May this account be polled right now?
 *
 * Fails CLOSED for the same reason `assertSendAllowed` does. A poller whose safety check degrades
 * to "poll anyway" on a store blip is a poller that runs unbounded during exactly the incident when
 * the fleet is already behaving strangely.
 */
export async function assertPollAllowed(domain: DomainStore, connectionId: string): Promise<PacingVerdict> {
  try {
    const conn = await domain.getConnection(connectionId);
    if (!conn) return { allowed: false, reason: "unknown connection", remaining: 0, budget: 0, nextAfterMs: 0 };
    const cfg = (conn.config ?? {}) as Record<string, unknown>;
    if (cfg.linkedin_challenge) {
      return {
        allowed: false,
    ...
    const engagement = ((cfg.pacing ?? {}) as PacingState).engagement ?? { sent: 0, accepted: 0, replied: 0, flagged: 0 };
    const { hour, day } = localClock(cfg, new Date());

    if (!withinSendingWindow(hour, day)) {
      return {
        allowed: false,
        reason: "outside this account's working hours — a poller that runs at 3am is the same bot signature a send is",
        remaining: 0,
        budget: 0,
        nextAfterMs: 60 * 60 * 1000,
      };
    }
    // The account is already in the state that pauses sending. Reading its inbox on a timer while
    // it is under suspicion adds request volume to an account we have just decided to leave alone.
    if (engagement.sent >= 20 && engagementMultiplier(engagement) <= 0.2) {
      return {
        allowed: false,
        reason: "this account is paused for engagement — nothing automated touches it, reads included",
        remaining: 0,
        budget: 0,
        nextAfterMs: 24 * 60 * 60 * 1000,
      };
    }
    return { allowed: true, remaining: 0, budget: 0, nextAfterMs: 0 };
  } catch (e) {
    return { allowed: false, reason: `pacing state unavailable: ${(e as Error).message}`, remaining: 0, budget: 0, nextAfterMs: 60_000 };
  }
}

/**
 * When the next poll of this kind becomes due — jittered, for the same reason sends are.
 *
 * A read every 3600s exactly, from every account in the fleet, is a more obvious fingerprint than
 * the reads themselves. ±40% around the base, matching `nextIntervalMs`, and never faster than five
 * minutes however small a base someone configures.
 */
export function pollIntervalMs(baseMs: number, rand: () => number = Math.random): number {
  return Math.max(5 * 60_000, Math.floor(baseMs * (0.6 + rand() * 0.8)));
}
