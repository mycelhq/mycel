...
export interface ImprovementFilter {
  ...
  target?: ImprovementProposal["target"];
  limit?: number;
}
...
export interface KnowledgeStore {
  ...
  /** Increment `uses` for rules that made it into a prompt, or that first held a move at rank time. Distinguishes "wrong" from "never read". */
  ...
  createImprovementProposal(p: NewImprovementProposal): Promise<ImprovementProposal>;
  getImprovementProposal(id: string, projectId: string): Promise<ImprovementProposal | undefined>;
  listImprovementProposals(projectId: string, filter?: ImprovementFilter): Promise<ImprovementProposal[]>;
  setImprovementProposalStatus(id: string, projectId: string, status: ImprovementStatus): Promise<ImprovementProposal | undefined>;
}
...
export class InMemoryKnowledgeStore implements KnowledgeStore {
  ...
  private improvements = new Map<string, ImprovementProposal>();
  ...
  async createImprovementProposal(p: NewImprovementProposal): Promise<ImprovementProposal> {
    const now = nowIso();
    const row: ImprovementProposal = { ...p, status: "proposed", created_at: now, updated_at: now };
    this.improvements.set(row.id, row);
    return { ...row, evidence: [...row.evidence], metadata: { ...row.metadata } };
  }
  async getImprovementProposal(id: string, projectId: string): Promise<ImprovementProposal | undefined> {
    const row = this.improvements.get(id);
    return row && projectId && row.project_id === projectId
      ? { ...row, evidence: [...row.evidence], metadata: { ...row.metadata } }
      : undefined;
  }
  async listImprovementProposals(projectId: string, filter: ImprovementFilter = {}): Promise<ImprovementProposal[]> {
    ...
    return [...this.improvements.values()]
      .filter((p) => p.project_id === projectId && (filter.wedge === undefined || p.wedge === filter.wedge) &&
        (filter.status === undefined || p.status === filter.status) &&
        (filter.target === undefined || p.target === filter.target))
      .sort((a, b) => b.created_at.localeCompare(a.created_at))
      .slice(0, Math.max(1, Math.min(filter.limit ?? 100, 500)))
      .map((p) => ({ ...p, evidence: [...p.evidence], metadata: { ...p.metadata } }));
  }
  async setImprovementProposalStatus(id: string, projectId: string, status: ImprovementStatus): Promise<ImprovementProposal | undefined> {
    const row = this.improvements.get(id);
    if (!row || !projectId || row.project_id !== projectId) return undefined;
    row.status = status;
 ...
 *
 * The Postgres backend's own `close` is a no-op (the pool is shared process-wide and ended once by
 * `closeAllPools`), so this exists to drop the singleton — a process that keeps a handle to a store
 * whose pool has been ended answers reads with a confusing "cannot use a pool after calling end"
 * rather than a clean shutdown.
 */
export async function closeKnowledgeStore(): Promise<void> {
  const s = cached as (KnowledgeStore & { close?: () => Promise<void> }) | null;
  cached = null;
  await s?.close?.();
}
