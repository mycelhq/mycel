...
export async function readCapability<T extends { external_id: string }>(args: {
  ...
  for (const b of binding.bound) {
    ...
    for (const read of b.provider.reads ?? []) {
      ...
      /**
       * A read is a tool slug OR an API path — see `ProviderRead`. `named` is what the gate checks,
       * what the failure says, and what the run reports, so both doors are one thing from here down.
       *
       * A path is named by the CAPABILITY rather than by the path itself: the connection grants
       * `read_invoices`, not `/api.xro/2.0/Invoices`, because a founder consented to us reading their
       * invoices and not to a particular URL we happen to use this month.
       */
      const named = read.slug ?? args.capability;
      if (!read.slug && !read.path) {
        out.failures.push(`${b.provider.label}: a read declares neither a tool nor a path`);
        continue;
      }
      // THE READ GATE. The provider table's wish list is not permission; the connection is.
      if (!isReadTool(b.connection, named)) {
        out.ungranted.push(`${b.connection.name}/${named}`);
        continue;
      }
      const res = await args.execute(b.connection, named, {
        arguments: { limit: args.limit ?? 100, ...(read.arguments ?? {}) },
        ...(read.path ? { path: read.path } : {}),
      });
      if (!res.ok) {
        out.failures.push(`${b.connection.name}/${named}: ${res.detail ?? "no detail"}`);
        continue;
      }
      out.reached = true;
