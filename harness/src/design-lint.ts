...
//
// Vendored from nexu-io/open-design (Apache-2.0), `apps/daemon/src/lint-artifact.ts`, with its two
// wire types inlined from `packages/contracts/src/api/artifact-lint.ts`. Their upstream tests came
// with it (`design-lint.test.ts`) and are what makes this a port rather than a paraphrase.
//
// WHY THIS AND NOT MORE PROSE. `craft/presenting-work.md` and the harvested design skills tell a run
// what good looks like, and a model that has read all of it still reaches for indigo `#6366f1`, a
// purple hero gradient, a rocket emoji as a feature icon and "10× faster" as a metric — because
// those are the median of what it was trained on. Advice does not beat a prior; a check does.
//
// The seven P0 rules are the tells that make a document read as machine-made at a glance, before
// anybody has assessed a single number in it. They are deterministic, so they are a GATE rather than
// a suggestion: a finding here is a regression, not a style preference.
//
// PURE. HTML string in, findings out. No network, no filesystem, no config — the same property the
// contact parser has, and for the same reason: the part that must be exactly right should be
