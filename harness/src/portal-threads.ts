...
export interface PortalThreadDeps {
  ...
  /** For `businessDisplayName` — the shape lives in a `draft_shape` run's artifact. */
  store: Store;
}
...
export function mountPortalThreads(app: Hono, deps: PortalThreadDeps): void {
  ...
  app.get("/v1/portal/business", async (c) => {
    ...
    return c.json({
      /**
       * The portal always shows a client SOMETHING, which is not the kit's job to invent.
       *
       * `resolveBrandKit` returns an empty display name when the project has not said who it is —
       * honest, and right for a document, where a blank masthead reads as unbranded and a wrong one
       * reads as a mistake. (It used to fall back to the literal string "Invoice", which then
       * appeared where the firm's name goes on a monthly close.)
       *
       * A portal is different: it is a page a client is looking at right now, and a blank heading
       * there is not restraint, it is a broken page. So the surface supplies the fallback, and it is
       * generic rather than a document type — nobody's bookkeeper is called Invoice.
       *
       * ── BUT "Your account" WAS REACHED ONE STEP TOO EARLY ──
       *
       * Observed by opening a real client link: the header read "Your account" and the greeting read
       * "Answer below and Your account keeps moving", to a client of a business whose founder had
       * typed "Halden Freight Recruitment" into onboarding twenty minutes before. The name existed;
       * only the brand-kit FIELD was empty, and this jumped straight from that to a placeholder.
       *
       * `businessDisplayName` asks the kit, then the shape, then the project. The placeholder stays
       * as the last resort it was always meant to be.
       */
      display_name: (await businessDisplayName(deps.store, sc.project_id, kit.display_name)) || "Your account",
      accent: kit.accent,
