...
import { creditSkillsForPayment } from "./skill-payment";
...
export async function recordManualPayment(input: ManualPaymentInput): Promise<ManualPaymentOutcome> {
  ...
  if (totals.amount_due === 0 && canTransition(invoice.status, "paid")) {
    ...
    if (done) {
       ...
       */
      // The same moment, from the other path. A payment reconciled from a provider and a payment a
      // founder marked by hand are the same fact about their business, and recording it in only one
      // place would mean the milestone existed for Stripe customers and for nobody else.
      await reachMilestone(getDomainStore(), {
        project_id: done.project_id,
        kind: "first_client_paid",
        at: paidAt,
        subject: done.number ?? undefined,
