 ...
 */
/**
 * Money as a person writes it, from minor units — and WITHOUT DIVIDING.
 *
 * `${amount_due / 10 ** exponent} ${currency}` rendered "6900 USD" — no separator, no decimals, a
 * currency CODE — in the subtitle of a row whose right-hand column already said "$6,900.00". The
 * same number, twice, in two formats, on one line.
 *
 * The first fix used `Intl.NumberFormat` on `minor / 10 ** exponent`, which displays correctly and
 * still divides. `contract.ts` is unambiguous about that: "Never a float, anywhere, for one reason
 * that is not negotiable." Display is a weaker case than arithmetic, but a divide-by-a-power-of-ten
 * in a money path is the thing the next person copies. So this is string work on the integer, the
 * same approach as `cloud/lib/money.ts` — separators inserted into a digit string, a decimal point
 * placed by the currency's own exponent, nothing computed.
 *
 * `signals.money_at_stake` still carries the exact minor units for anything that needs to compute.
 */
const ZERO_DECIMAL = new Set(["BIF","CLP","DJF","GNF","JPY","KMF","KRW","MGA","PYG","RWF","UGX","VND","VUV","XAF","XOF","XPF"]);
const THREE_DECIMAL = new Set(["BHD","IQD","JOD","KWD","LYD","OMR","TND"]);
const MONEY_SYMBOL: Record<string, string> = { USD: "$", GBP: "£", EUR: "€", JPY: "¥", AUD: "A$", CAD: "C$", NZD: "NZ$" };

function moneyExponent(currency: string): number {
...
export function moneyText(amountMinor: number, currency: string): string {
  ...
  const e = moneyExponent(code);
  const n = Math.trunc(amountMinor || 0);
  const negative = n < 0;
  const digits = String(negative ? -n : n).padStart(e + 1, "0");
  ...
  const symbol = MONEY_SYMBOL[code];
  if (symbol) return `${negative ? "-" : ""}${symbol}${body}`;
  return `${negative ? "-" : ""}${code} ${body}`;
}
  ...
  return assemble(
    {
      ...
      why:
        ...
        `${moneyText(totals.amount_due, inv.currency)} outstanding` +
        (inv.last_chased_at
    ...
    [
      { term: "money", points: money, because: `${moneyText(totals.amount_due, inv.currency)} is outstanding` },
...
export function takeability(
  ...
  if (kind === "rewrite_losing_arm") {
    /**
     * ═══ TAKEABLE, AND IT WAS NOT UNTIL THE DEPLOY GREW A GATE ═══
     *
     * This block used to refuse, and the refusal was correct at the time. `product-builder` declares
     * `build_feature`, so clicking would have spawned a real run with a real contract — and
     * `orchestrator.ts` called `startDeploy` the moment that run completed. The page went to S3,
     * CloudFront invalidated, and it was live. There was no step between the run finishing and a
     * stranger reading the founder's front door, so a take button here would not have meant "start
     * work", it would have meant "publish a new homepage", sitting next to a button that only drafts
     * an email. Two very different decisions wearing one control is how a founder learns the wrong
     * thing about what a click does.
     *
     * The refusal named its own precondition: "an approval gate on the deploy, so the run produces a
     * page a human confirms before it replaces the live one." That gate exists. A completed build now
     * leaves its deployment at `awaiting_review` (`proposeDeploy`), the bytes sit in S3, and CodeBuild
     * is not asked for anything until somebody answers `POST /v1/deployments/:id/publish`.
     *
     * So the click means what the button next to it means: START THE WORK. The consequence boundary
     * is guarded in exactly the way `chase_invoice`'s is — that one drafts an email and holds it in
     * `/approvals`; this one builds a page and holds it in the deployments list. Taking the move moves
     * the navigation and not the boundary, which is the promise the whole take button rests on.
     *
     * ═══ WHAT TAKING IT STILL SPENDS ═══
     *
     * A `deep`-tier run with a $5 ceiling, and no CodeBuild minutes until the review is answered. The
     * old refusal also cited "the strength of a ranking function nobody has audited yet", and that is
     * a fair objection to acting automatically on the ranking — which is not what this is. A take is
     * a person reading the evidence and choosing; `canStart` in `autonomy.ts` is what decides whether
     * a sweep may do it unattended, and that is governed by the autonomy settings rather than by this
     * flag. The row states the evidence and the founder decides twice: once to start the build, once
     * to publish what it produced.
     */
    return { takeable: true };
  }
