 ...
 *
 * The template's `/` asks a stranger for exactly one thing: send us a message. So a conversion is
 * "a visitor who was shown an arm and then sent an inquiry the kernel accepted, in the same
 * browser". Each clause earns its place:
 *
 ...
 *   - *an inquiry the kernel accepted* — not a click, not a form submission, not a validation
 *     failure, not a tripped honeypot. It is recorded in the Server Action after this endpoint
 *     answered ok (`app/contact-actions.ts`), so what is counted is a lead the founder can act on.
 ...
 *
 * ═══ THIS DEFINITION CHANGED, AND THE OLD ONE WAS MEASURING A BUG ═══
 *
 * It used to be "reached the client portal after seeing the marketing page", because the template's
 * only call to action was `PortalCta` → `/portal`. That call to action was itself the defect: the
 * largest button on a founder's public page told strangers to sign in to an account they did not
 * have. Four conditions in the tenant's middleware existed to strip a portal pageview back down to
 * something lead-shaped, and none of them could fix what was underneath — an arm "won" by sending
 * more people to a door that turned them away.
 *
 * Consequence for anyone reading a rate that spans the change: conversions recorded before it
 * describe a different act, and the two must not be pooled. The arm labels are unchanged, so this
 * will not announce itself — check the window against the deploy date before trusting a verdict
 * that straddles it.
 *
 * The honest weakness, stated so nobody has to rediscover it: this measures INTENT, not revenue. A
 * headline that gets more strangers to write in is not certainly a headline that wins more
 * business. It is much closer than a pageview was — an inquiry is a real person asking a real
 * business to do real work — but an arm that wins here has still won a proxy.
 */
export const CONVERSION_METRIC = "sent an inquiry after seeing the marketing page";
