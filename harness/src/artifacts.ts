...
//   inline — content is stored in the Store (zero setup). Fine for text and small files.
...
// ═══ LARGE FILES ═══
//
// A PDF pack, a workbook, a set of renders — those do not belong in a Postgres row. The inspector
// and the portal both go through `withContent`, which reads from this backend; if the writer used
// S3 and the reader is still `inline`, the file looks empty. Every process that reads or writes
// artifacts must share one MYCEL_ARTIFACTS setting. Above `INLINE_CONTENT_MAX_BYTES`, operators
// should be on fs or s3 — this module will not silently split the backend (that is how files vanish).
...
/** Bytes above this should live in the object store, not the database row. */
export const INLINE_CONTENT_MAX_BYTES = 512 * 1024;

/** Where this payload ought to live. Does not switch the backend — that is an operator choice. */
export function contentBelongsInline(sizeBytes: number): boolean {
  return Number.isFinite(sizeBytes) && sizeBytes >= 0 && sizeBytes <= INLINE_CONTENT_MAX_BYTES;
}
