...
//   2. Exceptions       — "…except Brightline, who asked us to".
...
export interface RuleProvenance {
  ...
  after?: string;
  /**
   * WHY they changed it, in their words, when they said.
   *
   * `before`/`after` show WHAT differs and are silent on what mattered about it. Two paragraphs
   * differ in a hundred ways and the founder cared about one of them; "too formal for this client"
   * is the only thing that says which, and it is the one input to this whole system that cannot be
   * inferred from anything else we hold.
   *
   * Never invented. Absent means they did not say, which is the common case and is fine — the
   * contrast still teaches. What is not fine, and was the state of this field until it existed, is
   * asking the founder for the sentence and then having nowhere to put it.
   */
  reason?: string;
  at: string;
}
...
export interface Rule {
   ...
   * "Brightline wants Friday summaries" is not a house rule, and applying it to everyone is exactly
...
export function distillFromApprovalEdit(args: {
  ...
  proposed: Record<string, unknown>;
  edited: Record<string, unknown>;
  /** The founder's one-line explanation, when the console asked and they answered. */
  reason?: string;
  task_id?: string;
  approval_id?: string;
  at?: string;
  ...
  /**
   * `action` is a noun at every call site — "deliverable", "email". Templating it straight into a
   * verb slot produced "When you deliverable, write ...", which is the sort of sentence that makes a
   * founder distrust everything else on the page, and it is inlined into the agent's prompt.
   */
  const doing = `you produce a ${args.action}`;
  return diffPayload(args.proposed, args.edited).map((ch) => {
    ...
    if (ch.change === "removed") {
      ...
      return {
        ...
        kind: "never" as const,
        text:
          `Do not include \`${ch.field}\` when ${doing} — the founder removed it before this went out.` +
    ...
    if (ch.change === "added") {
      return {
        ...
        text:
          `Always set \`${ch.field}\` when ${doing} — the founder had to add it before this went out.` +
          (reason ? ` They said: "${reason}"` : ""),
      };
    }
    return {
      ...
      text:
        `When ${doing}, write \`${ch.field}\` the founder's way (below), not the way you drafted it.` +
        (reason ? ` They said: "${reason}"` : ""),
    };
  });
}
 ...
 *     prompt as a standing instruction it has no way to decline. "Brightline wants Friday
 *     summaries" applied to everyone is how a system that "learns" actively makes a business worse,
 *     and the rule text quotes Brightline by name while doing it.
 ...
 * work FOR A NAMED CLIENT is about that client. A correction to a chase message sent to Brightline
 * quotes Brightline's balance and Brightline's excuses; it is not house style, however much it also
