...
declare module "hono" {
  interface ContextVariableMap {
    ...
    /**
     * Third-party counterparty plane (`/v1/party/*`) — one request only. Separate from `client`
     * so a candidate session cannot widen into the client portal.
     */
    party: import("./party").PartyScope;
  }
}
 ...
 *
 * Async because a member session is now a read-through against Postgres: the session store is no
 * longer a single-process Map, so validating a token can miss the local cache and has to ask the
 * durable store. API keys stay a synchronous in-memory lookup, so a key request pays nothing.
 */
export async function resolveAuth(token: string): Promise<AuthScope | null> {
  if (!token) return null;
  const id = getIdentityStore();
  // Member tokens are prefixed, so we don't waste a lookup — but either resolver is safe.
  if (token.startsWith("msess_")) return (await id.resolveSession(token)) ?? null;
  return id.resolveApiKey(token) ?? (await id.resolveSession(token)) ?? null;
}
...
export async function requireApiKey(c: Context, next: Next): Promise<Response | void> {
  const scope = await resolveAuth(bearer(c));
