...
async function initSchema(pool: Queryable): Promise<void> {
  ...
  await withSchemaLock(pool as never, async (client) => {
    ...
    await client.query(`
      ...
      -- Who answered: the client in the portal, or the business on their behalf. Provenance, not
      -- permissions — see ClientRequest.answered_by. Null on rows that predate the founder route.
      ALTER TABLE client_requests ADD COLUMN IF NOT EXISTS answered_by text;
...
export function rowToRequest(r: any): ClientRequest {
  return {
    ...
    answered_by: (r.answered_by ?? undefined) as ClientRequest["answered_by"],
...
export class PostgresRequestStore implements RequestStore {
  ...
  async resolveRequest(
    ...
    response: string,
    artifactIds?: string[],
    answeredBy: "client" | "founder" = "client",
  ): Promise<ClientRequest | undefined> {
    const res = await this.pool.query(
      `UPDATE client_requests
          SET status='resolved', response=$3, response_artifact_ids=$4::jsonb,
              answered_by=$5, resolved_at=now(), updated_at=now()
        WHERE id=$1 AND project_id=$2 AND status='open'
        RETURNING *`,
      [id, projectId, response, JSON.stringify(artifactIds?.length ? artifactIds.slice(0, 20) : null), answeredBy],
    );
    return res.rows[0] ? rowToRequest(res.rows[0]) : undefined;
  }
