 ...
 *     `{ type: "json_schema", schema, retryCount }`, i.e. OpenCode has NATIVE structured output
 *     with retries. That is a better home for a wedge's `output_schema` than a sentence in the
 *     prompt, and is wired in `opencode.ts`/`runtime.ts` for the strict-output profiles.
...
export const SHAPE_DEFAULTS: Record<HarnessShape, ShapeDefaults> = {
  ...
  decide: {
    tier: "fast",
    // Four minutes. Longer than the old flat 300s default is NOT needed here — a decision that has
  ...
  build: {
    tier: "deep",
    // Thirty minutes, clamped by MYCEL_MAX_RUNTIME_S (1800 by default, so this is the ceiling).
...
export function resolveHarnessProfile(args: ResolveHarnessArgs): HarnessProfile {
  const { task, wedge, plan, ceilings } = args;
  ...
  const tier = resolveTier(requested, plan);
  ...
  return {
    ...
    tier_clamped: wasClamped(requested, plan),
    max_runtime_s,
    max_cost_usd,
