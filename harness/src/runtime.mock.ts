...
// The shape matches `runOpenCodeTask` so the orchestrator can destructure one result from either.
// A mock resolves no connections, so it has no capability gaps to report — `undefined`, which
// `decideFate` reads as "nothing was missing", and that is the truth for a run that asked for
// nothing.
export async function runMockTask(
  ...
  /** Always absent: a mock writes no files, so there is none to malform. See `malformedArtifacts`. */
...
function onboardingFixture(task: Task): Record<string, unknown> | null {
  ...
  if (task.task_type === "draft_shape") {
    ...
    const preferred = (
      ["dunning", "receipts", "outreach", "business_shaping", "app_building"] as const
    )
