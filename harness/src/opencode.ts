...
export class OpenCodeClient {
  ...
  async startPrompt(
    ...
    model: string,
    /**
     * Per-message harness controls.
     *
     * NOTE: `format: {type:"json_schema", ...}` is NOT part of the 1.17.6 API. The body schema
     * (SessionPromptData) is `{messageID?, model?, agent?, noReply?, system?, tools?, parts}` and
     * nothing else; the string "json_schema" does not appear anywhere in the 1.17.6 SDK. OpenCode
     * parses request bodies with non-strict zod objects, so an extra `format` key is silently
     * STRIPPED — a strict-output run believes it has native schema enforcement with retries and in
     * fact has nothing. It is still sent (harmless, and correct for a later pin that adds it), but
     * nothing may rely on it until it is verified against the pinned binary.
     */
