 ...
 *
 * NO DATABASE MEANS NO BUDGET. The in-memory suite and every developer machine run without
...
export type CheckInRefusal =
  /**
   * The client has already heard from another wedge today. The engagement is still quiet and the
   * next sweep offers it again once there is a gap. See client-touch.ts.
   */
  | "held"
  | "not_configured"
  | "no_carrier"
...
export async function startCheckIn(
  ...
  /**
   * ═══ THE CLIENT'S PATIENCE IS SHARED, AND A CHECK-IN SPENDS IT LAST ═══
   *
   * The cooldown below is OUR pacing on THIS engagement. It knows nothing about the report, the
   * chase or the nudge another wedge may have sent this morning — seven wedges reach one human and
   * none knows the others exist.
   *
   * A check-in is the lowest-priority touch there is, so it is the one most often held, and that is
   * exactly right: its entire premise is that nothing else was happening, which is false the moment
   * another wedge has already written. "Just checking in" on the same day as an invoice chase reads
   * as a business that is not paying attention to itself.
   *
   * BEFORE the claim. `claimCaseMarker` stamps `last_checked_in_at`, so claiming and then declining
   * would spend the cooldown on a check-in nobody received.
   */
  if (k.client_id) {
    const verdict = await checkClientTouch(touchDb(), { taskType: "check_in_case", clientId: k.client_id }, now.getTime());
    if (!verdict.allow) {
      const line = heldLine("check_in_case", k.client_id, verdict);
