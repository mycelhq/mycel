...
POST     /v1/intake/form          a website contact form → a client, a thread and a task
...
## Wiring an existing website's contact form

Most service businesses already have a site. `POST /v1/intake/form` turns a submission on it into
the same thing an inbound email produces: a client record, a thread, and a task that starts work. It
runs the same normaliser as channel inbound, so a lead from a Squarespace form and a lead from an
inbox arrive identically shaped — and it dedupes on retried deliveries, per project.

**It is server-to-server.** It takes the founder key, so it is called from your site's form handler,
never from the browser — a key in a contact form is a key on every visitor's machine. Anything that
posts server-side will do: a WordPress hook, a Netlify or Vercel function, a Zapier webhook step.

### The payload

`fields` is required and is the form exactly as your site collected it — every key is kept in the
task's metadata, so a question you added to the form last week is in front of the agent this week
without anything here changing.

The sender comes from `from.handle` or, failing that, `fields.email`. The body comes from `body` or
`fields.message`. Anything else is optional.

```bash
curl -X POST "$KERNEL/v1/intake/form" \
  -H "authorization: Bearer $MYCEL_API_KEY" -H 'content-type: application/json' \
  -d '{
        "channel_id": "chn_…",
        "from":    { "handle": "sam@hartsbakery.co.uk", "name": "Sam Hart" },
        "subject": "Bookkeeping enquiry",
        "fields":  { "email": "sam@hartsbakery.co.uk",
                     "message": "We need someone for the August books.",
                     "turnover": "£240k", "software": "Xero" }
      }'
```

### Where the work lands

`channel_id` is the recommended form: the channel already carries the project, the wedge and the
task type, so a business that changes which service handles enquiries changes it in one place rather
than in its website's code.

Without a channel, name the trade directly — `wedge` and `task_type`, plus an `X-Mycel-Project`
header. Both must exist and the wedge must be enabled for that project, or you get a 400/403 saying
which. `GET /v1/wedges` lists what is installed and the task types each one accepts.

Replays answer `200` with the original task rather than creating a second one; a first delivery
answers `201`. Like every other inbound path, nothing reaches the person who wrote in until it has
