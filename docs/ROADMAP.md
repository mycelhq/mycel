...
businesses — e-commerce bookkeeping — against the live kernel. All 20 capability checks pass, and
it's now a regression test (`harness/test/stress-bookkeeping.test.ts`, wedge in
`wedges/books-keeper/`). Cases, schedules, ungated reads, exact integer-cent reconciliation,
...
and *what inputs* — never the logic. Examples: `books-keeper/reconcile` (integer cents, tax-inclusive
sales tax, balanced to the cent rather than to the dollar) and `invoice-chaser/next_step` (the dunning
...
deterministic workflows all carried over unchanged. It replaced `geo-monitor`, which was making the
