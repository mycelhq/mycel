...
| `MYCEL_LINKEDIN_QID_PROFILE` | unset | GraphQL query id for a profile read. Unset = that candidate is skipped, never guessed |
| `MYCEL_LINKEDIN_PROFILE_DECORATION` | unset | Named response shape for the dash profile finder. Optional; a rotated one costs one 400, not the read |
| `MYCEL_LINKEDIN_LEGACY_PROFILEVIEW` | unset | `1` re-enables `/identity/profiles/{id}/profileView`, which answered **410 Gone** from 2026-08-18 |

**Profile reads are a LADDER, not an endpoint** (`packages/linkedin/src/profile.ts`). `profileView`
was retired under us and took every LinkedIn read with it, so the read now tries the dash finder,
then GraphQL (only with a known query id), then the profile page as a browser document, latches onto
whichever answers, and fails permanently with a devtools capture list when none do. To find out which
one LinkedIn serves *your* account today, run `packages/linkedin/scripts/verify-profile.ts` — see
`packages/linkedin/README.md`.
