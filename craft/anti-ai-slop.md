<!--
  VENDORED from nexu-io/open-design `craft/anti-ai-slop.md` (Apache-2.0), which adapted it from
  referodesign/refero_skill (MIT). Kept in `craft/` rather than on the skill shelf because it is
  short, it is universal, and it applies to every deliverable — not only to briefs that happen to
  mention design.

  The references to a "daemon" and `lint-artifact.ts` upstream are OURS too: `harness/src/
  design-lint.ts` is a verified port of that linter, running their 97 tests. So the rules below
  marked as enforced really are enforced here, and the ones marked guidance really are not.
-->
