...
- **HOW you add a section the template does not ship — the mechanism, not just the exhortation.**
  `app/page.tsx` renders `marketing.sections` through an exhaustive `switch`, so for a long time
  "author a bespoke section" was advice the code could not accept, and the file's own header said so.
  Both are fixed. The seam is `{ kind: "custom", id }` plus `components/sections/`:

  1. Write the component — `components/sections/<id>.tsx`. Compose `Section` / `Eyebrow` /
     `PortalCta` from `components/marketing.tsx` where they fit; build from scratch where they do not.
  2. Register it in `components/sections/index.ts` (`customSections`).
  3. Place it in `content/marketing.ts` → `marketing.sections` as `{ kind: "custom", id: "<id>" }`,
     in the position the identity's section plan wants. Add `eyebrow` for the house label, or omit
     it for a section whose effect is that it is unlabelled.

  An id with nothing registered is dropped before render, so a half-finished section is a missing
  section rather than a hole in the page. Read `components/sections/index.ts` — it documents the
  contract and the prop surface.

- **THE KERNEL NOW FAILS A BUILD THAT ONLY RE-SKINNED THE SEED.** After the app is proved to compile
  and boot, the kernel hashes the workspace against the exact template it seeded. A run whose only
  changes are `app/globals.css`, `content/**`, `components/ui/**` (what `npx shadcn add` writes — not
  your authorship) or config is reported as a FAILURE, naming the files, and comes back to you for
  another attempt. The bar is at least one NEW source file of your own, three substantive files, and
  ~2KB of authored code. That is a floor, not a target; it exists because "compiles and serves a 200"
  was previously satisfied by a copy swap. If you find yourself about to finish having written no new
  component, you have not done the job — go back to the identity's `signature_motif` and build it.
...
## The one file you must not decapitate

`app/globals.css` starts with the Tailwind import and the token blocks. EXTEND it — add tokens,
add utilities at the bottom — but NEVER replace the top of the file or remove the tailwind import.
A globals.css without that import still "builds": the page answers 200, tsc passes, and the deployed
site is raw unstyled HTML with a 1.5KB stylesheet. That exact failure shipped once. The verify now
measures the compiled stylesheet and fails the run under 10KB, so you will not get away with it —
keep the header intact and you will never meet that error.
